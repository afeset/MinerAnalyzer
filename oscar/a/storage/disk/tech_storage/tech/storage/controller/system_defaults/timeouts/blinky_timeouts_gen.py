...
from a.api.yang.modules.tech.common.qwilt_tech_storage_controller.tech.storage.controller.system_defaults.timeouts.timeouts_data_gen import TimeoutsData





class BlinkyTimeouts(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-controller"
    #leaves
    
    ourXmlTagGetLogicalDiskStatus="get-logical-disk-status"
    
    ourXmlTagGetPhysicalStatus="get-physical-status"
    
    ourXmlTagActivateLogicalDisk="activate-logical-disk"
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagGetLogicalDiskStatus)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.getLogicalDiskStatus = newValue.asInt64()
                self.myCandidateData.setHasGetLogicalDiskStatus()
                
            else:
                self.setGetLogicalDiskStatusDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagGetPhysicalStatus)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.getPhysicalStatus = newValue.asInt64()
                self.myCandidateData.setHasGetPhysicalStatus()
                
            else:
                self.setGetPhysicalStatusDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagActivateLogicalDisk)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.activateLogicalDisk = newValue.asInt64()
                self.myCandidateData.setHasActivateLogicalDisk()
                
            else:
                self.setActivateLogicalDiskDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkytimeouts').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.controller.system_defaults.timeouts.blinky_timeouts_gen import BlinkyTimeouts', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagGetLogicalDiskStatus)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagGetPhysicalStatus)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagActivateLogicalDisk)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkytimeouts-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.controller.system_defaults.timeouts.blinky_timeouts_gen import BlinkyTimeouts', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = TimeoutsData()
        
        res = self.setGetLogicalDiskStatusDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-getlogicaldiskstatus-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setGetLogicalDiskStatusDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setGetPhysicalStatusDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-getphysicalstatus-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setGetPhysicalStatusDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setActivateLogicalDiskDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-activatelogicaldisk-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setActivateLogicalDiskDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setGetLogicalDiskStatusDefaultValue (self, setHas):
        for logFunc in self._log("set-getlogicaldiskstatus-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.getLogicalDiskStatus = 0
    ...
    def setGetPhysicalStatusDefaultValue (self, setHas):
        for logFunc in self._log("set-getphysicalstatus-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.getPhysicalStatus = 0
    ...
    def setActivateLogicalDiskDefaultValue (self, setHas):
        for logFunc in self._log("set-activatelogicaldisk-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.activateLogicalDisk = 0
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = TimeoutsData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkytimeouts-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkytimeouts-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_storage_controller.tech.storage.controller.system_defaults.timeouts.timeouts_data_gen import TimeoutsData", 
        "moduleYangNamespacePrefix": "qt-strg-ctrl", 
        "validationPoint": null, 
        "yangName": "timeouts", 
        "namespace": "timeouts", 
        "logGroupName": "blinky-timeouts", 
        "className": "BlinkyTimeouts", 
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-controller-system-defaults-timeouts-blinky-timeouts-gen", 
        "importStatement": "from a.storage.disk.tech_storage.tech.storage.controller.system_defaults.timeouts.blinky_timeouts_gen import BlinkyTimeouts", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-controller", 
        "dataClassName": "TimeoutsData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

...
from a.storage.disk.tech_storage.tech.storage.controller.blinky_controller_gen import BlinkyController

class BlinkyControllerList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyControllerList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyControllerList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkycontrollerlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.storage.disk.tech_storage.tech.storage.controller.blinky_controller_gen import BlinkyController', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyController.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkycontrollerlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.storage.disk.tech_storage.tech.storage.controller.blinky_controller_gen import BlinkyController', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-controller-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-controller-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-controller").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyControllerList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-controller").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyController. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyController(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-controller-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyController not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-controller").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyController. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyController(self, key):
        for logFunc in self._log("get-blinky-blinky-controller").debug1Func(): logFunc("getBlinkyController(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyController(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-controller-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyController-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyController(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyController", 
        "containerLogGroupName": "blinky-controller", 
        "namespace": "controller", 
        "containerImportStatement": "from a.storage.disk.tech_storage.tech.storage.controller.blinky_controller_gen import BlinkyController", 
        "logGroupName": "blinky-controller-list", 
        "className": "BlinkyControllerList", 
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-controller-blinky-controller-list-gen", 
        "keyLeaf": {
            "varName": "controller", 

...
class BlinkyStorage(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage"
    #leaves
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkystorage').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyStorage._validationPointId, BlinkyStorage._actionPointId)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkystorage').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.blinky_storage_gen import BlinkyStorage', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        logger('is-key-path-registered-blinkystorage-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.blinky_storage_gen import BlinkyStorage', keyPathRegistered, keyPath, keyDepth)
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkystorage-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkystorage-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_storage.tech.storage.storage_data_gen import StorageData", 
        "moduleYangNamespacePrefix": "qt-strg", 
        "validationPoint": null, 
        "yangName": "storage", 
        "namespace": "storage", 
        "logGroupName": "blinky-storage", 
        "className": "BlinkyStorage", 
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-blinky-storage-gen", 
        "importStatement": "from a.storage.disk.tech_storage.tech.storage.blinky_storage_gen import BlinkyStorage", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage", 
        "dataClassName": "StorageData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

...
from a.api.yang.modules.tech.common.qwilt_tech_storage_disk.tech.storage.disk.physical.physical_data_gen import PhysicalData
...
class BlinkyPhysical(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyphysical').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyPhysical._validationPointId, BlinkyPhysical._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagImplementation)):
            if trxElement.getNewVal():
                ...
                if not DiskPhysicalImplementationType.isValidValue(newValue.asEnum()):
                ...
                self.myCandidateData.implementation = DiskPhysicalImplementationType.getByValue(newValue.asEnum())
                self.myCandidateData.setHasImplementation()
                
            else:
                self.setImplementationDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagController)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyphysical').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.disk.physical.blinky_physical_gen import BlinkyPhysical', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkyphysical-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.disk.physical.blinky_physical_gen import BlinkyPhysical', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = PhysicalData()
    ...
    def setImplementationDefaultValue (self, setHas):
        ...
        self.myCandidateData.implementation = DiskPhysicalImplementationType.kDirectory
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = PhysicalData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyphysical-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyphysical-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_storage_disk.tech.storage.disk.physical.physical_data_gen import PhysicalData", 
        "moduleYangNamespacePrefix": "qt-strg-dsk", 
        "validationPoint": null, 
        "yangName": "physical", 
        "namespace": "physical", 
        "logGroupName": "blinky-physical", 
        "className": "BlinkyPhysical", 
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-disk-physical-blinky-physical-gen", 
        "importStatement": "from a.storage.disk.tech_storage.tech.storage.disk.physical.blinky_physical_gen import BlinkyPhysical", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", 
        "dataClassName": "PhysicalData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

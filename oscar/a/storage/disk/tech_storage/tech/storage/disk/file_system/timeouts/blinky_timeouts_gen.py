...
from a.api.yang.modules.tech.common.qwilt_tech_storage_disk.tech.storage.disk.file_system.timeouts.timeouts_data_gen import TimeoutsData
...
class BlinkyTimeouts(BlinkyContainer):
    ...
    ourXmlTagActivate="activate"
    
    ourXmlTagGetStatus="get-status"
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagActivate)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.activate = newValue.asInt64()
                self.myCandidateData.setHasActivate()
                
            else:
                self.setActivateDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagGetStatus)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.getStatus = newValue.asInt64()
                self.myCandidateData.setHasGetStatus()
                
            else:
                self.setGetStatusDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkytimeouts').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.disk.file_system.timeouts.blinky_timeouts_gen import BlinkyTimeouts', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagActivate)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagGetStatus)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkytimeouts-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.disk.file_system.timeouts.blinky_timeouts_gen import BlinkyTimeouts', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        ...
        res = self.setActivateDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-activate-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setActivateDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setGetStatusDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-getstatus-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setGetStatusDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setActivateDefaultValue (self, setHas):
        for logFunc in self._log("set-activate-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.activate = 0
    ...
    def setGetStatusDefaultValue (self, setHas):
        for logFunc in self._log("set-getstatus-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.getStatus = 0
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_storage_disk.tech.storage.disk.file_system.timeouts.timeouts_data_gen import TimeoutsData", 
        ...
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-disk-file-system-timeouts-blinky-timeouts-gen", 
        "importStatement": "from a.storage.disk.tech_storage.tech.storage.disk.file_system.timeouts.blinky_timeouts_gen import BlinkyTimeouts", 

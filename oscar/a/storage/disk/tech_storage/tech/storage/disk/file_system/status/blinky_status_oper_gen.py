...
from a.api.yang.modules.tech.common.qwilt_tech_storage_disk.tech.storage.disk.file_system.status.status_oper_data_gen import StatusOperData
...
class BlinkyOperStatus(BlinkyOperNode):
    ...
    _kCallpointName = "tech-storage-disk-file-system-status-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-storage-disk-file-system-status-callpoint"
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "uuid"):
                    for logFunc in self._log("set-oper-data-requested-fields-uuid-requested").debug3Func(): logFunc(
                        "uuid requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setUuidRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "operational-status"):
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "file-system-type-raw"):
                    for logFunc in self._log("set-oper-data-requested-fields-filesystemtyperaw-requested").debug3Func(): logFunc(
                        "file-system-type-raw requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setFileSystemTypeRawRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "file-system-type"):
                    for logFunc in self._log("set-oper-data-requested-fields-filesystemtype-requested").debug3Func(): logFunc(
                        "file-system-type requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setFileSystemTypeRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "expected-uuid"):
                    for logFunc in self._log("set-oper-data-requested-fields-expecteduuid-requested").debug3Func(): logFunc(
                        "expected-uuid requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setExpectedUuidRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "operational-status-reason"):
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "uuid"):
            if operData.isUuidRequested():
                 if operData.hasUuid():
                     value.setString(operData.uuid)
                 else:
                     value.setEmpty()
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "file-system-type-raw"):
            if operData.isFileSystemTypeRawRequested():
                 if operData.hasFileSystemTypeRaw():
                     value.setString(operData.fileSystemTypeRaw)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "file-system-type"):
            if operData.isFileSystemTypeRequested():
                 if operData.hasFileSystemType():
                     value.setEnum(operData.fileSystemType.getValue())
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "expected-uuid"):
            if operData.isExpectedUuidRequested():
                 if operData.hasExpectedUuid():
                     value.setString(operData.expectedUuid)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_storage_disk.tech.storage.disk.file_system.status.status_oper_data_gen import StatusOperData", 
        ...
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-disk-file-system-status-blinky-status-oper-gen", 
        "importStatement": "from a.storage.disk.tech_storage.tech.storage.disk.file_system.status.blinky_status_oper_gen import BlinkyOperStatus", 
        "callpointName": "tech-storage-disk-file-system-status-callpoint", 

...
from a.api.yang.modules.tech.common.qwilt_tech_storage_disk.tech.storage.disk.disk_data_gen import DiskData


from a.storage.disk.tech_storage.tech.storage.disk.system_defaults.blinky_system_defaults_gen import BlinkySystemDefaults

from a.storage.disk.tech_storage.tech.storage.disk.raid_array.blinky_raid_array_gen import BlinkyRaidArray

from a.storage.disk.tech_storage.tech.storage.disk.file_system.blinky_file_system_gen import BlinkyFileSystem

from a.storage.disk.tech_storage.tech.storage.disk.physical.blinky_physical_gen import BlinkyPhysical
...
class BlinkyDisk(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkydisk').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyDisk._validationPointId, BlinkyDisk._actionPointId)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkydisk').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.disk.blinky_disk_gen import BlinkyDisk', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkydisk-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.disk.blinky_disk_gen import BlinkyDisk', keyPathRegistered, keyPath, keyDepth)
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkydisk-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkydisk-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_storage_disk.tech.storage.disk.disk_data_gen import DiskData", 
        "moduleYangNamespacePrefix": "qt-strg-dsk", 
        "validationPoint": null, 
        "yangName": "disk", 
        "namespace": "disk", 
        "logGroupName": "blinky-disk", 
        "className": "BlinkyDisk", 
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-disk-blinky-disk-gen", 
        "importStatement": "from a.storage.disk.tech_storage.tech.storage.disk.blinky_disk_gen import BlinkyDisk", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", 
        "dataClassName": "DiskData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {
    ...
    "descendants": [
        ...
        }, 
        {
            ...
            "importStatement": "from a.storage.disk.tech_storage.tech.storage.disk.file_system.blinky_file_system_gen import BlinkyFileSystem"
        }, 
        {

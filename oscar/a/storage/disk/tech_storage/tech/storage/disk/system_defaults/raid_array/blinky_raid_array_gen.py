...
from a.api.yang.modules.tech.common.qwilt_tech_storage_disk.tech.storage.disk.system_defaults.raid_array.raid_array_data_gen import RaidArrayData
...
class BlinkyRaidArray(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk"
    #leaves
    
    ourXmlTagOsDevice="os-device"
    
    ourXmlTagImplementation="implementation"
    
    ourXmlTagRaidType="raid-type"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyraidarray').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyRaidArray._validationPointId, BlinkyRaidArray._actionPointId)
        ...
        newNode=BlinkyRaidArray(logger)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagOsDevice)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.osDevice = newValue.asString()
                self.myCandidateData.setHasOsDevice()
                
            else:
                self.setOsDeviceDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagImplementation)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                if not RaidArrayImplementationType.isValidValue(newValue.asEnum()):
                    self._log("trx-element-update-candidate-illegal-enum-implementation-failed")\
                        .error("illegal enum value %s for implementation", newValue.asEnum())
                    return ReturnCodes.kGeneralError
                self.myCandidateData.implementation = RaidArrayImplementationType.getByValue(newValue.asEnum())
                self.myCandidateData.setHasImplementation()
                
            else:
                self.setImplementationDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagRaidType)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                if not RaidArrayRaidType.isValidValue(newValue.asEnum()):
                    self._log("trx-element-update-candidate-illegal-enum-raidtype-failed")\
                        .error("illegal enum value %s for raidType", newValue.asEnum())
                    return ReturnCodes.kGeneralError
                self.myCandidateData.raidType = RaidArrayRaidType.getByValue(newValue.asEnum())
                self.myCandidateData.setHasRaidType()
                
            else:
                self.setRaidTypeDefaultValue(True)
        ...
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagAutoInit)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyraidarray').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.disk.system_defaults.raid_array.blinky_raid_array_gen import BlinkyRaidArray', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagOsDevice)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagImplementation)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagRaidType)):
                keyPathRegistered = True
                found = True
        ...
        logger('is-key-path-registered-blinkyraidarray-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.disk.system_defaults.raid_array.blinky_raid_array_gen import BlinkyRaidArray', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = RaidArrayData()
        
        res = self.setOsDeviceDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-osdevice-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setOsDeviceDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setImplementationDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-implementation-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setImplementationDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setRaidTypeDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-raidtype-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setRaidTypeDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
    ...
    def setOsDeviceDefaultValue (self, setHas):
        for logFunc in self._log("set-osdevice-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.osDevice = ""
    ...
    def setImplementationDefaultValue (self, setHas):
        for logFunc in self._log("set-implementation-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.implementation = RaidArrayImplementationType.kNone
    ...
    def setRaidTypeDefaultValue (self, setHas):
        for logFunc in self._log("set-raidtype-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.raidType = RaidArrayRaidType.kNone
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = RaidArrayData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyraidarray-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyraidarray-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_storage_disk.tech.storage.disk.system_defaults.raid_array.raid_array_data_gen import RaidArrayData", 
        "moduleYangNamespacePrefix": "qt-strg-dsk", 
        "validationPoint": null, 
        "yangName": "raid-array", 
        "namespace": "raid_array", 
        "logGroupName": "blinky-raid-array", 
        "className": "BlinkyRaidArray", 
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-disk-system-defaults-raid-array-blinky-raid-array-gen", 

...
from a.storage.disk.tech_storage.tech.storage.disk.blinky_disk_gen import BlinkyDisk

class BlinkyDiskList(BlinkyList):
    ...
    def __str__ (self):
        ret="{BlinkyDiskList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkydisklist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.storage.disk.tech_storage.tech.storage.disk.blinky_disk_gen import BlinkyDisk', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyDisk.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkydisklist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.storage.disk.tech_storage.tech.storage.disk.blinky_disk_gen import BlinkyDisk', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-disk-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-disk-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-disk-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-disk").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyDiskList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-disk").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyDisk. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyDisk(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-disk-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyDisk not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-disk").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyDisk. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyDisk(self, key):
        for logFunc in self._log("get-blinky-blinky-disk").debug1Func(): logFunc("getBlinkyDisk(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyDisk(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkyDisk(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-disk-moved-failed").errorFunc(): logFunc("getBlinkyDisk(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkyDisk(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-disk-after-failed").errorFunc(): logFunc("getBlinkyDisk(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-disk-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyDisk-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyDisk(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyDisk", 
        "containerLogGroupName": "blinky-disk", 
        "namespace": "disk", 
        "containerImportStatement": "from a.storage.disk.tech_storage.tech.storage.disk.blinky_disk_gen import BlinkyDisk", 
        "logGroupName": "blinky-disk-list", 
        "className": "BlinkyDiskList", 
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-disk-blinky-disk-list-gen", 
        "keyLeaf": {
            "varName": "disk", 

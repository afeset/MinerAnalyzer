...
from a.api.yang.modules.tech.common.qwilt_tech_storage_disk.tech.storage.disk.alarms.alarms_oper_data_gen import AlarmsOperData
...
class BlinkyOperAlarms(BlinkyOperNode):
    ...
    _kCallpointName = "tech-storage-disk-alarms-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-storage-disk-alarms-callpoint"
    ...
    def getOperRelativePath (self, operRelativePath):
        ...
        val = Value()
        val.setXmlTag(("alarms", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "qt-strg-dsk"))
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "content-disk-failure-reason"):
                    for logFunc in self._log("set-oper-data-requested-fields-contentdiskfailurereason-requested").debug3Func(): logFunc(
                        "content-disk-failure-reason requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setContentDiskFailureReasonRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "content-disk-failure-alarm"):
                    for logFunc in self._log("set-oper-data-requested-fields-contentdiskfailurealarm-requested").debug3Func(): logFunc(
                        "content-disk-failure-alarm requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setContentDiskFailureAlarmRequested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isContentDiskFailureReasonRequested() and operData.hasContentDiskFailureReason():
            val = Value()
            val.setEnum(operData.contentDiskFailureReason.getValue())
            tagValueList.push(("content-disk-failure-reason", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk"), val)
        
        if operData.isContentDiskFailureAlarmRequested() and operData.hasContentDiskFailureAlarm():
            val = Value()
            val.setBool(operData.contentDiskFailureAlarm)
            tagValueList.push(("content-disk-failure-alarm", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "content-disk-failure-reason"):
            if operData.isContentDiskFailureReasonRequested():
                 if operData.hasContentDiskFailureReason():
                     value.setEnum(operData.contentDiskFailureReason.getValue())
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-disk", "content-disk-failure-alarm"):
            if operData.isContentDiskFailureAlarmRequested():
                 if operData.hasContentDiskFailureAlarm():
                     value.setBool(operData.contentDiskFailureAlarm)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_storage_disk.tech.storage.disk.alarms.alarms_oper_data_gen import AlarmsOperData", 
        ...
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-disk-alarms-blinky-alarms-oper-gen", 
        "importStatement": "from a.storage.disk.tech_storage.tech.storage.disk.alarms.blinky_alarms_oper_gen import BlinkyOperAlarms", 
        "callpointName": "tech-storage-disk-alarms-callpoint", 

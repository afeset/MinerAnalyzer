...
from a.storage.disk.tech_storage.tech.storage.module.blinky_module_gen import BlinkyModule

class BlinkyModuleList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyModuleList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyModuleList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkymodulelist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.storage.disk.tech_storage.tech.storage.module.blinky_module_gen import BlinkyModule', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyModule.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkymodulelist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.storage.disk.tech_storage.tech.storage.module.blinky_module_gen import BlinkyModule', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-module-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-module-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-module").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyModuleList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-module").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyModule. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyModule(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-module-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyModule not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-module").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyModule. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyModule(self, key):
        for logFunc in self._log("get-blinky-blinky-module").debug1Func(): logFunc("getBlinkyModule(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyModule(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-module-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyModule-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyModule(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyModule", 
        "containerLogGroupName": "blinky-module", 
        "namespace": "module", 
        "containerImportStatement": "from a.storage.disk.tech_storage.tech.storage.module.blinky_module_gen import BlinkyModule", 
        "logGroupName": "blinky-module-list", 
        "className": "BlinkyModuleList", 
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-module-blinky-module-list-gen", 
        "keyLeaf": {
            "varName": "module", 

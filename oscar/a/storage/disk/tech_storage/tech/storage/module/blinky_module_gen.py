...
from a.api.yang.modules.tech.common.qwilt_tech_storage_module.tech.storage.module.module_data_gen import ModuleData


from a.storage.disk.tech_storage.tech.storage.module.system_defaults.blinky_system_defaults_gen import BlinkySystemDefaults
...
class BlinkyModule(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkymodule').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyModule._validationPointId, BlinkyModule._actionPointId)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkymodule').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.module.blinky_module_gen import BlinkyModule', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkymodule-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.storage.disk.tech_storage.tech.storage.module.blinky_module_gen import BlinkyModule', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = ModuleData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkymodule-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkymodule-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_storage_module.tech.storage.module.module_data_gen import ModuleData", 
        "moduleYangNamespacePrefix": "qt-strg-module", 
        "validationPoint": null, 
        "yangName": "module", 
        "namespace": "module", 
        "logGroupName": "blinky-module", 
        "className": "BlinkyModule", 
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-module-blinky-module-gen", 
        "importStatement": "from a.storage.disk.tech_storage.tech.storage.module.blinky_module_gen import BlinkyModule", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-module", 
        "dataClassName": "ModuleData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

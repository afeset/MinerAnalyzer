...
from a.api.yang.modules.tech.common.qwilt_tech_storage_module.tech.storage.module.actual.actual_oper_data_gen import ActualOperData
...
class BlinkyOperActual(BlinkyOperNode):

    _kCallpointName = "tech-storage-module-actual-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-storage-module-actual-callpoint"
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-module", "enabled"):
                    for logFunc in self._log("set-oper-data-requested-fields-enabled-requested").debug3Func(): logFunc(
                        "enabled requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setEnabledRequested()
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-module", "enabled"):
            if operData.isEnabledRequested():
                 if operData.hasEnabled():
                     value.setBool(operData.enabled)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_storage_module.tech.storage.module.actual.actual_oper_data_gen import ActualOperData", 
        ...
        "yangName": "actual", 
        "namespace": "actual", 
        "logGroupName": "blinky-actual-oper", 
        "className": "BlinkyOperActual", 
        "logModuleName": "a-storage-disk-tech-storage-tech-storage-module-actual-blinky-actual-oper-gen", 
        "importStatement": "from a.storage.disk.tech_storage.tech.storage.module.actual.blinky_actual_oper_gen import BlinkyOperActual", 
        "callpointName": "tech-storage-module-actual-callpoint", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-storage-module", 
        "dataClassName": "ActualOperData", 
        "getObjArgsNum": 2, 
        "actionPoint": null
    }, 
    "ancestors": [

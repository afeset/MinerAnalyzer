...
class BlinkyExport(BlinkyContainer):
    ...
    ourXmlTagUrlTranslation="url-translation"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyexport').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyExport._validationPointId, BlinkyExport._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagAnalytics)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.analytics = newValue.asBool()
                self.myCandidateData.setHasAnalytics()
                
            else:
                self.setAnalyticsDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagUrlTranslation)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.urlTranslation = newValue.asBool()
                self.myCandidateData.setHasUrlTranslation()
                
            else:
                self.setUrlTranslationDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagType_)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                if not ReportingExportTypesType.isValidValue(newValue.asEnum()):
                ...
                self.myCandidateData.type_ = ReportingExportTypesType.getByValue(newValue.asEnum())
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyexport').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.content.reporting.tech_content_reporting.tech.content.reporting.export_.blinky_export__gen import BlinkyExport', keyPath, keyDepth)
        ...
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagUrlTranslation)):
                keyPathRegistered = True
                found = True
        ...
        logger('is-key-path-registered-blinkyexport-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.content.reporting.tech_content_reporting.tech.content.reporting.export_.blinky_export__gen import BlinkyExport', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = ExportData()
        ...
        res = self.setAnalyticsDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-analytics-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setAnalyticsDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setUrlTranslationDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-urltranslation-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setUrlTranslationDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
    ...
    def setAnalyticsDefaultValue (self, setHas):
        for logFunc in self._log("set-analytics-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.analytics = False
    ...
    def setUrlTranslationDefaultValue (self, setHas):
        for logFunc in self._log("set-urltranslation-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.urlTranslation = False
    ...
    def setType_DefaultValue (self, setHas):
        ...
        self.myCandidateData.type_ = ReportingExportTypesType.kNone
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyexport-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyexport-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.content.qwilt_tech_content_reporting.tech.content.reporting.export_.export__data_gen import ExportData", 
        "moduleYangNamespacePrefix": "qtc-report", 
        "validationPoint": null, 
        "yangName": "export", 
        "namespace": "export_", 
        "logGroupName": "blinky-export", 
        "className": "BlinkyExport", 
        "logModuleName": "a-content-reporting-tech-content-reporting-tech-content-reporting-export--blinky-export--gen", 
        "importStatement": "from a.content.reporting.tech_content_reporting.tech.content.reporting.export_.blinky_export__gen import BlinkyExport", 

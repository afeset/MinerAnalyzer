...
class BlinkyReporting(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-reporting"
    #leaves
    

    #descendants
    
    ourXmlTagExportList="export"
    ...
    EXPORTLIST_CREATE_FUNCTOR = 'EXPORTLIST_CREATE_FUNCTOR'
    EXPORTLIST_DELETE_FUNCTOR = 'EXPORTLIST_DELETE_FUNCTOR'
    ...
    def __init__ (self, logger):
        ...
        self.myCreateExportListFunctor=None
        self.myDeleteExportListFunctor=None
        self.myExportList=None
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyreporting').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyReporting._validationPointId, BlinkyReporting._actionPointId)
    ...
    def setCreateExportListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-create-export-functor-active").errorFunc():
                logFunc("setCreateExportListFunctor() is illegal when blinky node is active")
        self.myCreateExportListFunctor = functor

    def setDeleteExportListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-delete-export-functor-active").errorFunc():
                logFunc("setDeleteExportListFunctor() is illegal when blinky node is active")
        self.myDeleteExportListFunctor = functor
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if (keyPath.isTagEqual(keyDepth_PBR.value(), self.ourNamespace, self.ourXmlTagExportList)):
            return self.myExportList
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyreporting').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.content.reporting.tech_content_reporting.tech.content.reporting.blinky_reporting_gen import BlinkyReporting', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        if not found:
            if keyPath.getLen() > keyDepth+1:
                if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagExportList)):
                    keyPathRegistered = BlinkyExportList.isKeyPathRegistered(logger, keyPath, keyDepth+1)
                    found = True
            else:
        ...
        logger('is-key-path-registered-blinkyreporting-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.content.reporting.tech_content_reporting.tech.content.reporting.blinky_reporting_gen import BlinkyReporting', keyPathRegistered, keyPath, keyDepth)
    ...
    def prepareMyBlinkyNode(self, keyPath, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagExportList)):
            if (self.myExportList):
                for logFunc in self._log("prepare-my-blinky-node-exportlist-already-exists").errorFunc():
                    logFunc("prepareMyBlinkyNode(): already exists, keyPath=%s, keyDepth=%s", keyPath, keyDepth)
                return None
            self.myExportList = BlinkyExportList(self._log)
            self.myExportList.setParent(self)
            self.myExportList.setKeyPath(keyPath)
            self.myExportList.setDomain(self.myDomain)
    ...
    def handleTrxElementCreate(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagExportList)):
                if (self.myCreateExportListFunctor):
                    if (not self.myExportList):
                        for logFunc in self._log("handle-trx-element-create-exportlist-not-exist").debug3Func():
                            logFunc("handleTrxElementCreate(): exportlist not exists, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    
                    timeoutGuard = TimeoutGuard(self._log, '%s-create-exportlist-functor-%s' % (self.myKeyPath, phase), 
                                                self.getFunctorTimeoutForPhase(self.EXPORTLIST_CREATE_FUNCTOR, phase),
                                                self.getFunctorMildTimeoutForPhase(self.EXPORTLIST_CREATE_FUNCTOR, phase))
                    try:
                        res = self.myCreateExportListFunctor(phase, self.myExportList)
                    except:
                        for logFunc in self._log("handle-trx-element-create-exportlist-functor-exception").exceptionFunc():
                            logFunc("ExportList's create functor raised an exception. trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        raise
                    timeoutGuard.checkAndLog("application problem: functor=%s" % self.myCreateExportListFunctor.__name__)
                    if (res != ReturnCodes.kOk):
                        if (phase.getConfdPhase() == TrxPhase.kPrepare):
                            for logFunc in self._log("handle-trx-element-create-exportlist-functor-failed-prepare").noticeFunc():
                                logFunc("handleTrxElementCreate(): exportlist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                            self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        else:
                            for logFunc in self._log("handle-trx-element-create-exportlist-functor-failed").errorFunc():
                                logFunc("handleTrxElementCreate(): exportlist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    ...
    def notifyDescendantsPrepareBlinkyDelete(self):
        ...
        if (self.myExportList):
            self.myExportList.notifyDescendantsPrepareBlinkyDelete()
    ...
    def handleTrxElementDelete(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagExportList)):
                res = self.activateDeleteExportListFunctor(phase)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("handle-trx-element-delete-exportlist-functor-failed-prepare").noticeFunc():
                            logFunc("handleTrxElementDelete(): exportlist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("handle-trx-element-delete-exportlist-functor-failed").errorFunc():
                            logFunc("handleTrxElementDelete(): exportlist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                    return ReturnCodes.kGeneralError
    ...
    def removeExportList(self):
        self.myExportList = None

    def activateDeleteExportListFunctor(self, phase):
        for logFunc in self._log("activate-delete-exportlist-functor").debug3Func(): logFunc("activateDeleteExportListFunctor(): called, phase=%s", phase)
        if (self.myIsActive):
            if (self.myDeleteExportListFunctor):
                timeoutGuard = TimeoutGuard(self._log, '%s-delete-exportlist-functor-%s' % (self.myKeyPath, phase), 
                                            self.getFunctorTimeoutForPhase(self.EXPORTLIST_DELETE_FUNCTOR, phase), 
                                            self.getFunctorMildTimeoutForPhase(self.EXPORTLIST_DELETE_FUNCTOR, phase))
                try:
                    res = self.myDeleteExportListFunctor(phase)
                except:
                    for logFunc in self._log("activate-delete-exportlist-functor-exception").exceptionFunc():
                        logFunc("ExportList's delete functor raised an exception. phase=%s", phase)
                    raise
                timeoutGuard.checkAndLog("application problem: functor=%s" % self.myDeleteExportListFunctor.__name__)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("activate-delete-exportlist-functor-functor-failed-prepare").noticeFunc():
                            logFunc("activateDeleteExportListFunctor(): functor failed, phase=%s", phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("activate-delete-exportlist-functor-functor-failed").errorFunc():
                            logFunc("activateDeleteExportListFunctor(): functor failed, phase=%s", phase)
                    return ReturnCodes.kGeneralError
        return ReturnCodes.kOk
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyreporting-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyreporting-failed").errorFunc():
    ...
    def handleInternalDestroy(self, phase):
        ...
        if (phase.getConfdPhase() == TrxPhase.kPrepare):
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myExportList):
                    res = self.myExportList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-exportlist-recursion-failed-prepare-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): delete-exportlist-recursion-failed-prepare-blinky, phase=%s",
                                  phase)
                        return ReturnCodes.kGeneralError
                ...
                if (self.myExportList):
                    res = self.activateDeleteExportListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-exportlist-functor-failed-prepare").noticeFunc():
                            logFunc("handleInternalDestroy(): activateDeleteExportListFunctor() failed, res=%s, phase=%s",
                                   res, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        return ReturnCodes.kGeneralError
                if (self.myExportList):
                    res = self.myExportList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-exportlist-recursion-failed-prepare-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myExportList.handleInternalDestroy() failed, res=%s, phase=%s",
                                   res, phase)
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myExportList):
                    res = self.myExportList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-exportlist-recursion-failed-commit-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myExportList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("handle internal destroy failed in commit phase (blinky)")
                        return ReturnCodes.kGeneralError
                    self.removeExportList()
                ...
                if (self.myExportList):
                    res = self.myExportList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-exportlist-recursion-failed-commit-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myExportList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                    ...
                    res = self.activateDeleteExportListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-exportlist-functor-failed-commit").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteExportListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("activateDeleteExportListFunctor failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myExportList):
                    res = self.myExportList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-exportlist-recursion-failed-abort-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myExportList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myExportList):
                    res = self.myExportList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-exportlist-recursion-failed-abort-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myExportList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myExportList):
                    res = self.activateDeleteExportListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-exportlist-functor-failed-abort").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteExportListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.content.qwilt_tech_content_reporting.tech.content.reporting.reporting_data_gen import ReportingData", 
        "moduleYangNamespacePrefix": "qtc-report", 
        "validationPoint": null, 
        "yangName": "reporting", 
        "namespace": "reporting", 
        "logGroupName": "blinky-reporting", 
        "className": "BlinkyReporting", 
        "logModuleName": "a-content-reporting-tech-content-reporting-tech-content-reporting-blinky-reporting-gen", 
        "importStatement": "from a.content.reporting.tech_content_reporting.tech.content.reporting.blinky_reporting_gen import BlinkyReporting", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-reporting", 
        "dataClassName": "ReportingData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {
    ...
    "descendants": [
        {
            "className": "BlinkyExportList", 
            "memberName": "ExportList", 
            "yangName": "export", 
            "importStatement": "from a.content.reporting.tech_content_reporting.tech.content.reporting.export_.blinky_export__list_gen import BlinkyExportList"
        }
    ], 

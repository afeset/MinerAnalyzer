...
class BlinkyExportList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyExportList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyExportList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyexportlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.content.reporting.tech_content_reporting.tech.content.reporting.export_.blinky_export__gen import BlinkyExport', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyExport.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyexportlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.content.reporting.tech_content_reporting.tech.content.reporting.export_.blinky_export__gen import BlinkyExport', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-export-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-export-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-export").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyExportList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-export").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyExport. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyExport(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-export-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyExport not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-export").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyExport. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyExport(self, key):
        for logFunc in self._log("get-blinky-blinky-export").debug1Func(): logFunc("getBlinkyExport(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyExport(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-export-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyExport-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyExport(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyExport", 
        "containerLogGroupName": "blinky-export", 
        "namespace": "export_", 
        "containerImportStatement": "from a.content.reporting.tech_content_reporting.tech.content.reporting.export_.blinky_export__gen import BlinkyExport", 
        "logGroupName": "blinky-export-list", 
        "className": "BlinkyExportList", 
        "logModuleName": "a-content-reporting-tech-content-reporting-tech-content-reporting-export--blinky-export--list-gen", 
        "keyLeaf": {
            "varName": "export_", 

...
class BlinkyOperStatus(BlinkyOperNode):
    ...
    _kCallpointName = "tech-content-reporting-export-status-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-content-reporting-export-status-callpoint"
    ...
    def getOperRelativePath (self, operRelativePath):
        ...
        val = Value()
        val.setXmlTag(("status", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-reporting", "qtc-report"))
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-reporting", "queue-utilization-percent"):
                    for logFunc in self._log("set-oper-data-requested-fields-queueutilizationpercent-requested").debug3Func(): logFunc(
                        "queue-utilization-percent requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setQueueUtilizationPercentRequested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isQueueUtilizationPercentRequested() and operData.hasQueueUtilizationPercent():
            val = Value()
            val.setUint64(operData.queueUtilizationPercent)
            tagValueList.push(("queue-utilization-percent", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-reporting"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-reporting", "queue-utilization-percent"):
            if operData.isQueueUtilizationPercentRequested():
                 if operData.hasQueueUtilizationPercent():
                     value.setUint64(operData.queueUtilizationPercent)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.content.qwilt_tech_content_reporting.tech.content.reporting.export_.status.status_oper_data_gen import StatusOperData", 
        ...
        "logModuleName": "a-content-reporting-tech-content-reporting-tech-content-reporting-export--status-blinky-status-oper-gen", 
        "importStatement": "from a.content.reporting.tech_content_reporting.tech.content.reporting.export_.status.blinky_status_oper_gen import BlinkyOperStatus", 
        "callpointName": "tech-content-reporting-export-status-callpoint", 

...
from a.api.yang.modules.tech.content.qwilt_tech_content_line.tech.content.line.analyzer.house_keeper.house_keeper_maapi_gen import BlinkyHouseKeeperMaapi
from a.api.yang.modules.tech.content.qwilt_tech_content_line.tech.content.line.analyzer.prediction.prediction_maapi_gen import BlinkyPredictionMaapi
from a.api.yang.modules.tech.content.qwilt_tech_content_line.tech.content.line.analyzer.delivery.delivery_maapi_gen import BlinkyDeliveryMaapi
from a.api.yang.modules.tech.content.qwilt_tech_content_line.tech.content.line.analyzer.potential.potential_maapi_gen import BlinkyPotentialMaapi
from a.api.yang.modules.tech.content.qwilt_tech_content_line.tech.content.line.analyzer.unit.unit_maapi_list_gen import BlinkyUnitMaapiList
from a.api.yang.modules.tech.content.qwilt_tech_content_line.tech.content.line.analyzer.policy.policy_maapi_gen import BlinkyPolicyMaapi
from a.api.yang.modules.tech.content.qwilt_tech_content_line.tech.content.line.analyzer.acquisition.acquisition_maapi_gen import BlinkyAcquisitionMaapi
...
class BlinkyAnalyzerMaapi(AnalyzerMaapiBase):
    ...
    def requestConfigAndOper (self):
        ...
        if not self.potentialObj:
            self.potentialObj = self.newPotential()
            self.potentialObj.requestConfigAndOper()
    ...
    def requestConfig (self):
        ...
        if not self.potentialObj:
            self.potentialObj = self.newPotential()
            self.potentialObj.requestConfig()
    ...
    def clearAllRequested (self):
        ...
        if not self.potentialObj:
            self.potentialObj = self.newPotential()
            self.potentialObj.clearAllRequested()
    ...
    def clearAllSet (self):
        ...
        if self.potentialObj:
            self.potentialObj.clearAllSet()
    ...
    def newPotential (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('new-potential').debug3Func(): logFunc('called.')
        potential = BlinkyPotentialMaapi(self._log)
        potential.init(self.domain)
        return potential

    def setPotentialObj (self, obj):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('set-potential').debug3Func(): logFunc('called. obj=%s', obj)
        self.potentialObj = obj

    def getPotentialObj (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('get-potential').debug3Func(): logFunc('called. self.potentialObj=%s', self.potentialObj)
        return self.potentialObj

    def hasPotential (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('has-potential').debug3Func(): logFunc('called. self.potentialObj=%s', self.potentialObj)
        if self.potentialObj:
            return True
        return False
    ...
    def _clearAllReadData (self):
        ...
        if self.potentialObj:
            self.potentialObj._clearAllReadData()
    ...
    def _collectItemsToDelete (self,
        ...
        if self.potentialObj:
            res = self.potentialObj._collectItemsToDelete(line, 
                                                                          
                                                                          itemsToDelete)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('collect-items-to-delete-potential-failed').errorFunc(): logFunc('potentialObj._collectItemsToDelete() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _fillWriteTagValues (self, tagValueList):
        ...
        if self.potentialObj:
            valBegin = Value()
            (tag, ns, prefix) = ("potential" , "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-line", "qtc-line")
            valBegin.setXmlBegin((tag, ns, prefix))
            tagValueList.push((tag, ns), valBegin)

            tagValueListLen = tagValueList.getLen()

            res = self.potentialObj._fillWriteTagValues(tagValueList)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('fill-write-tag-values-potential-failed').errorFunc(): logFunc('potentialObj._fillWriteTagValues() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _fillReadTagValues (self, tagValueList):
        ...
        if self.potentialObj:
            ...
            res = self.potentialObj._fillReadTagValues(tagValueList)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('fill-read-tag-values-potential-failed').errorFunc(): logFunc('potentialObj._fillReadTagValues() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _readTagValues (self, tagValueList, readAllOrFail):
        ...
        if self.potentialObj:
            ...
            if (tag != "potential") or \
                ...
                (valBegin.getType() != Value.kXmlBegin):
                for logFunc in self._log('reag-tag-values-unexpected-tag-begin').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                        "potential", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-line", Value.kXmlBegin,
                                                                        tag, ns, valBegin.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
            
            res = self.potentialObj._readTagValues(tagValueList, readAllOrFail)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('read-tag-values-potential-failed').errorFunc(): logFunc('potentialObj._readTagValues() failed. tagValueList=%s', tagValueList)
                if readAllOrFail:
                    self._clearAllReadData()
                return ReturnCodes.kGeneralError
            ...
            if (tag != "potential") or \
                ...
                (valEnd.getType() != Value.kXmlEnd):
                for logFunc in self._log('reag-tag-values-unexpected-tag-end').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                      "potential", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-line", Value.kXmlEnd,
                                                                        tag, ns, valEnd.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError

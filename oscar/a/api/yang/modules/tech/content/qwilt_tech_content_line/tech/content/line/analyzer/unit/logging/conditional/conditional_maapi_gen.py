...
from conditional_maapi_base_gen import ConditionalMaapiBase




class BlinkyConditionalMaapi(ConditionalMaapiBase):
    def __init__ (self, logger):
        self.myInitGuard = InitGuard()
        self._log=logger.createLogger("sys-blinky-oper-example","blinky-maapi-conditional")
        self.domain = None
    ...
    def requestCid (self, requested):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('request-cid').debug3Func(): logFunc('called. requested=%s', requested)
        self.cidRequested = requested
        self.cidSet = False

    def isCidRequested (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('is-cid-requested').debug3Func(): logFunc('called. requested=%s', self.cidRequested)
        return self.cidRequested

    def getCid (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('get-cid').debug3Func(): logFunc('called. self.cidSet=%s, self.cid=%s', self.cidSet, self.cid)
        if self.cidSet:
            return self.cid
        return None
    ...
    def hasCid (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('has-cid').debug3Func(): logFunc('called. self.cidSet=%s, self.cid=%s', self.cidSet, self.cid)
        if self.cidSet:
            return True
        return False
    ...
    def setCid (self, cid):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('set-cid').debug3Func(): logFunc('called. cid=%s, old=%s', cid, self.cid)
        self.cidSet = True
        self.cid = cid
    ...
    def _getSelfKeyPath (self, line
        ...
        xmlVal = Value()
        xmlVal.setXmlTag(("conditional", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-line", "qtc-line"))
    ...
    def _readTagValues (self, tagValueList, readAllOrFail):
        ...
        if self.isCidRequested():
            ...
            if (tag != "cid") or \
                (ns != "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-line"):
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-cid').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "cid", "cid", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-line", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
            ...
            if res != ReturnCodes.kOk or tempVar is None:
                for logFunc in self._log('read-tag-values-cid-bad-value').infoFunc(): logFunc('cid not read')
                if readAllOrFail:
                    self._clearAllReadData()
                    return ReturnCodes.kGeneralError
            if tempVar is not None:
                self.setCid(tempVar)
            for logFunc in self._log('read-tag-values-cid').debug3Func(): logFunc('read cid. cid=%s, tempValue=%s', self.cid, tempValue.getType())
...
Extracted from the below data: 
{
    "node": {
        ...
        "importStatement": "from a.api.yang.modules.tech.content.qwilt_tech_content_line.tech.content.line.analyzer.unit.logging.conditional.conditional_maapi_gen import ConditionalMaapi", 

...
class BlinkySiteMaapiList(SiteMaapiListBase):
    ...
    def _collectItemsToDelete (self,
        ...
        for key in self.sites.keys():
            if self.sites[key]:
                res = self.sites[key]._collectItemsToDelete(rule, 
    ...
    def _fillWriteTagValues (self, tagValueList):
        ...
        for key in self.sites.keys():
            if self.sites[key]:
                ...
                valKey = Value()
                valKey.setString(key)
                tagValueList.push(("site", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content"), valKey)
            ...
            if self.sites[key]:
                ...
                if (tag != "site") or \
                    ...
                    for logFunc in self._log('reag-tag-values-unexpected-tag-key').errorFunc(): logFunc('got unexpected tag-value for key. expected: (%s, %s), got: (%s, %s)',
                                                                          "site", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content", tag, ns)
                    self._clearAllReadData()
                    return ReturnCodes.kGeneralError

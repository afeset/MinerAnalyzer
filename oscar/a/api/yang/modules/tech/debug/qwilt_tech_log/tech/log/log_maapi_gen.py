...
from log_maapi_base_gen import LogMaapiBase

from a.api.yang.modules.tech.debug.qwilt_tech_log.tech.log.housekeeper.housekeeper_maapi_gen import BlinkyHousekeeperMaapi
from a.api.yang.modules.tech.debug.qwilt_tech_log.tech.log.system_defaults.system_defaults_maapi_gen import BlinkySystemDefaultsMaapi



class BlinkyLogMaapi(LogMaapiBase):
    def __init__ (self, logger):
        self.myInitGuard = InitGuard()
        self._log=logger.createLogger("sys-blinky-oper-example","blinky-maapi-log")
        self.domain = None
    ...
    def _collectItemsToDelete (self,
        ...
        if self.housekeeperObj:
            res = self.housekeeperObj._collectItemsToDelete(
                                                                          itemsToDelete)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('collect-items-to-delete-housekeeper-failed').errorFunc(): logFunc('housekeeperObj._collectItemsToDelete() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _fillWriteTagValues (self, tagValueList):
        ...
        if self.housekeeperObj:
            valBegin = Value()
            (tag, ns, prefix) = ("housekeeper" , "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", "qt-log")
            ...
            if res != ReturnCodes.kOk:
                for logFunc in self._log('fill-write-tag-values-housekeeper-failed').errorFunc(): logFunc('housekeeperObj._fillWriteTagValues() failed. PARAMS')
                return ReturnCodes.kGeneralError
        ...
        if self.systemDefaultsObj:
            valBegin = Value()
            (tag, ns, prefix) = ("system-defaults" , "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", "qt-log")
            valBegin.setXmlBegin((tag, ns, prefix))
            tagValueList.push((tag, ns), valBegin)
    ...
    def _fillReadTagValues (self, tagValueList):
        ...
        if self.housekeeperObj:
            ...
            if res != ReturnCodes.kOk:
                for logFunc in self._log('fill-read-tag-values-housekeeper-failed').errorFunc(): logFunc('housekeeperObj._fillReadTagValues() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _readTagValues (self, tagValueList, readAllOrFail):
        ...
        if self.housekeeperObj:
            ...
            if (tag != "housekeeper") or \
                (ns != "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log") or \
                (valBegin.getType() != Value.kXmlBegin):
                for logFunc in self._log('reag-tag-values-unexpected-tag-begin').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                        "housekeeper", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", Value.kXmlBegin,
            ...
            if res != ReturnCodes.kOk:
                for logFunc in self._log('read-tag-values-housekeeper-failed').errorFunc(): logFunc('housekeeperObj._readTagValues() failed. tagValueList=%s', tagValueList)
                if readAllOrFail:
                    self._clearAllReadData()
                return ReturnCodes.kGeneralError
            ...
            if (tag != "housekeeper") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-end').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                      "housekeeper", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", Value.kXmlEnd,
        ...
        if self.systemDefaultsObj:
            ...
            if (tag != "system-defaults") or \
                ...
                (valBegin.getType() != Value.kXmlBegin):
                for logFunc in self._log('reag-tag-values-unexpected-tag-begin').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                        "system-defaults", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", Value.kXmlBegin,
                                                                        tag, ns, valBegin.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
            ...
            if (tag != "system-defaults") or \
                ...
                (valEnd.getType() != Value.kXmlEnd):
                for logFunc in self._log('reag-tag-values-unexpected-tag-end').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                      "system-defaults", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", Value.kXmlEnd,
                                                                        tag, ns, valEnd.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError

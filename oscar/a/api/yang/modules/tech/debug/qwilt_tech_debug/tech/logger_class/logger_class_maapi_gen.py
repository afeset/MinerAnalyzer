...
from logger_class_maapi_base_gen import LoggerClassMaapiBase

from a.api.yang.modules.tech.debug.qwilt_tech_debug.tech.logger_class.instance.instance_maapi_list_gen import BlinkyInstanceMaapiList



class BlinkyLoggerClassMaapi(LoggerClassMaapiBase):
    ...
    def requestConfigAndOper (self):
        ...
        if not self.instanceListObj:
            self.instanceListObj = self.newInstanceList()
            self.instanceListObj.requestConfigAndOper()
    ...
    def requestConfig (self):
        ...
        if not self.instanceListObj:
            self.instanceListObj = self.newInstanceList()
            self.instanceListObj.requestConfig()
    ...
    def requestOper (self):
        ...
        if not self.instanceListObj:
            self.instanceListObj = self.newInstanceList()
            self.instanceListObj.requestOper()
    ...
    def clearAllRequested (self):
        ...
        if not self.instanceListObj:
            self.instanceListObj = self.newInstanceList()
            self.instanceListObj.clearAllRequested()
    ...
    def clearAllSet (self):
        ...
        if self.instanceListObj:
            self.instanceListObj.clearAllSet()
    ...
    def write (self
        ...
        return self._internalWrite(loggerClass, trxContext)
    ...
    def newInstanceList (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('new-instancelist').debug3Func(): logFunc('called.')
        instanceList = BlinkyInstanceMaapiList(self._log)
        instanceList.init(self.domain)
        return instanceList

    def setInstanceListObj (self, obj):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('set-instancelist').debug3Func(): logFunc('called. obj=%s', obj)
        self.instanceListObj = obj

    def getInstanceListObj (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('get-instancelist').debug3Func(): logFunc('called. self.instanceListObj=%s', self.instanceListObj)
        return self.instanceListObj

    def hasInstanceList (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('has-instancelist').debug3Func(): logFunc('called. self.instanceListObj=%s', self.instanceListObj)
        if self.instanceListObj:
            return True
        return False
    ...
    def _clearAllReadData (self):
        ...
        if self.instanceListObj:
            self.instanceListObj._clearAllReadData()
    ...
    def _collectItemsToDelete (self,
        ...
        if self.instanceListObj:
            res = self.instanceListObj._collectItemsToDelete(loggerClass, 
                                                                          
                                                                          itemsToDelete)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('collect-items-to-delete-instance-failed').errorFunc(): logFunc('instanceListObj._collectItemsToDelete() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _fillWriteTagValues (self, tagValueList):
        ...
        if self.instanceListObj:
            ...
            res = self.instanceListObj._fillWriteTagValues(tagValueList)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('fill-write-tag-values-instance-failed').errorFunc(): logFunc('instanceListObj._fillWriteTagValues() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _fillReadTagValues (self, tagValueList):
        ...
        if self.instanceListObj:
            ...
            res = self.instanceListObj._fillReadTagValues(tagValueList)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('fill-read-tag-values-instance-failed').errorFunc(): logFunc('instanceListObj._fillReadTagValues() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _readTagValues (self, tagValueList, readAllOrFail):
        ...
        if self.instanceListObj:
            ...
            res = self.instanceListObj._readTagValues(tagValueList, readAllOrFail)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('read-tag-values-instance-failed').errorFunc(): logFunc('instanceListObj._readTagValues() failed. tagValueList=%s', tagValueList)
                if readAllOrFail:
                    self._clearAllReadData()
                return ReturnCodes.kGeneralError

...
from fan_maapi_list_base_gen import FanMaapiListBase
from fan_maapi_gen import BlinkyFanMaapi

class BlinkyFanMaapiList(FanMaapiListBase):
    ...
    def newFan (self):
        ...
        for logFunc in self._log('new-fan').debug3Func(): logFunc('called.')
        fan = BlinkyFanMaapi(self._log)
        fan.init(self.domain)
        return fan

    def setFanObj (self, key, fanObj):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('set-fan-obj').debug3Func(): logFunc('called. key=%s, fanObj=%s', key, fanObj)
        if key not in self.fans:
            self.fanKeys.append(key)
        self.fans[str(key)] = fanObj

    def getFanObj (self, key):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('get-fan-obj').debug3Func(): logFunc('called. key=%s', key)
        if str(key) in self.fans.keys():
            for logFunc in self._log('get-fan-obj-done').debug3Func(): logFunc('Done. found key=%s, obj=%s', key, self.fans[str(key)])
            return self.fans[str(key)]
        for logFunc in self._log('get-fan-obj-missing').errorFunc(): logFunc('fan %s not in fans. existing items: %s', key, self.fans.keys())
        return None

    def deleteFan (self, key):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('delete-fan').debug3Func(): logFunc('called. key=%s', key)
        if str(key) not in self.fanKeys:
            for logFunc in self._log('delete-fan-not-found').warningFunc(): logFunc('key=%s is missing from the fanKeys list', key)
            if str(key) in self.fans.keys():
                # internal problem - list & dictionary are not synced
                for logFunc in self._log('delete-fan-not-found-but-in-dict').errorFunc(): logFunc('fans dictionary & fanKeys list are out-of-sync. key %s exists in dict but not in list', key)
            return ReturnCodes.kGeneralError
        if str(key) not in self.fans.keys():
            # internal problem - list & dictionary are not synced
            for logFunc in self._log('delete-fan-not-found-but-in-list').errorFunc(): logFunc('fans dictionary & fanKeys list are out-of-sync. key %s exists in list but not in dict', key)
            return ReturnCodes.kGeneralError

        self.fanKeys.remove(str(key))
        del self.fans[str(key)]

    def hasFanObj (self, key):
        self.myInitGuard.isInitOrCrash()
        has = False
        if str(key) in self.fans.keys():
            if self.fans[str(key)]:
                has = True
        for logFunc in self._log('has-fan-done').debug3Func(): logFunc('done. key=%s exists=%s', key, has)
        return has

    def getListKeys (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('get-list-keys').debug3Func(): logFunc('called. keys=%s', [str(x) for x in self.fanKeys])
        return self.fanKeys
    ...
    def _clearAllReadData (self):
        ...
        for fan in self.fans.values():
            if fan:
                fan._clearAllReadData()

    def clearAllSet (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('clear-all-set').debug3Func(): logFunc('called, PARAMS')
        for key in self.fans.keys():
            if self.fans[key]:
                self.fans[key].clearAllSet()
            else:
                self.fanKeys.remove(str(key))
                del self.fans[str(key)]
    ...
    def readListKeys (self
        ...
        for key in keys:
            self.fanKeys.append(key.getCannonicalStr())
            self.fans[key.getCannonicalStr()] = None
    ...
    def _collectItemsToDelete (self,
        ...
        for key in self.fans.keys():
            if self.fans[key]:
                res = self.fans[key]._collectItemsToDelete(
                                                                     key,
                                                                     itemsToDelete)
                if res != ReturnCodes.kOk:
                    for logFunc in self._log('collect-items-to-delete-fan-failed').errorFunc(): logFunc('fanObj._collectItemsToDelete() failed. key=%s. PARAMS', key)
                    return ReturnCodes.kGeneralError
    ...
    def _fillWriteTagValues (self, tagValueList):
        ...
        for key in self.fans.keys():
            if self.fans[key]:
                ...
                valKey = Value()
                valKey.setString(key)
                tagValueList.push(("name", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans"), valKey)

                tagValueListLen = tagValueList.getLen()

                res = self.fans[key]._fillWriteTagValues(tagValueList)
                if res != ReturnCodes.kOk:
                    for logFunc in self._log('fill-write-tag-values-fan-failed').errorFunc(): logFunc('fan._fillWriteTagValues() failed. key=%s', key)
                    return ReturnCodes.kGeneralError
    ...
    def _fillReadTagValues (self, tagValueList):
        ...
        for key in self.fans.keys():
            if self.fans[key]:
                ...
                res = self.fans[key]._fillReadTagValues(tagValueList)
                if res != ReturnCodes.kOk:
                    for logFunc in self._log('fill-read-tag-values-fan-failed').errorFunc(): logFunc('fan._fillReadTagValues() failed. key=%s', key)
                    return ReturnCodes.kGeneralError
            ...
            if self.fans[key]:
                ...
                if (tag != "name") or \
                    ...
                    for logFunc in self._log('reag-tag-values-unexpected-tag-key').errorFunc(): logFunc('got unexpected tag-value for key. expected: (%s, %s), got: (%s, %s)',
                                                                          "name", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", tag, ns)
                    self._clearAllReadData()
                    return ReturnCodes.kGeneralError
                ...
                res = self.fans[key]._readTagValues(tagValueList, readAllOrFail)
                if res != ReturnCodes.kOk:
                    for logFunc in self._log('read-tag-values-fan-failed').errorFunc(): logFunc('fan._readTagValues() failed. key=%s', key)
                    if readAllOrFail:
                        self._clearAllReadData()
                    return ReturnCodes.kGeneralError

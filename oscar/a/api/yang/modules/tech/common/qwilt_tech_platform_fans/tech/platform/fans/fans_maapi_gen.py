...
from fans_maapi_base_gen import FansMaapiBase

from a.api.yang.modules.tech.common.qwilt_tech_platform_fans.tech.platform.fans.status.status_maapi_gen import BlinkyStatusMaapi
from a.api.yang.modules.tech.common.qwilt_tech_platform_fans.tech.platform.fans.alarm.alarm_maapi_gen import BlinkyAlarmMaapi
from a.api.yang.modules.tech.common.qwilt_tech_platform_fans.tech.platform.fans.simulation.simulation_maapi_gen import BlinkySimulationMaapi
...
from a.api.yang.modules.tech.common.qwilt_tech_platform_fans.tech.platform.fans.system_defaults.system_defaults_maapi_gen import BlinkySystemDefaultsMaapi



class BlinkyFansMaapi(FansMaapiBase):
    def __init__ (self, logger):
        self.myInitGuard = InitGuard()
        self._log=logger.createLogger("sys-blinky-oper-example","blinky-maapi-fans")
        self.domain = None
    ...
    def requestConfigAndOper (self):
        ...
        if not self.fanListObj:
            self.fanListObj = self.newFanList()
            self.fanListObj.requestConfigAndOper()
    ...
    def clearAllRequested (self):
        ...
        if not self.fanListObj:
            self.fanListObj = self.newFanList()
            self.fanListObj.clearAllRequested()
    ...
    def newFanList (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('new-fanlist').debug3Func(): logFunc('called.')
        fanList = BlinkyFanMaapiList(self._log)
        fanList.init(self.domain)
        return fanList

    def setFanListObj (self, obj):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('set-fanlist').debug3Func(): logFunc('called. obj=%s', obj)
        self.fanListObj = obj

    def getFanListObj (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('get-fanlist').debug3Func(): logFunc('called. self.fanListObj=%s', self.fanListObj)
        return self.fanListObj

    def hasFanList (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('has-fanlist').debug3Func(): logFunc('called. self.fanListObj=%s', self.fanListObj)
        if self.fanListObj:
            return True
        return False
    ...
    def _clearAllReadData (self):
        ...
        if self.fanListObj:
            self.fanListObj._clearAllReadData()
    ...
    def _collectItemsToDelete (self,
        ...
        if self.fanListObj:
            res = self.fanListObj._collectItemsToDelete(
                                                                          itemsToDelete)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('collect-items-to-delete-fan-failed').errorFunc(): logFunc('fanListObj._collectItemsToDelete() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _fillWriteTagValues (self, tagValueList):
        ...
        if self.hasEnabled():
            ...
            if self.enabled is not None:
                ...
                valEnabled.setEmpty()
            tagValueList.push(("enabled", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans"), valEnabled)
        ...
        if self.alarmObj:
            valBegin = Value()
            (tag, ns, prefix) = ("alarm" , "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "qt-pltf-fans")
            valBegin.setXmlBegin((tag, ns, prefix))
            tagValueList.push((tag, ns), valBegin)
        ...
        if self.fanListObj:
            valBegin = Value()
            (tag, ns, prefix) = ("fan" , "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "qt-pltf-fans")
            valBegin.setXmlBegin((tag, ns, prefix))
            tagValueList.push((tag, ns), valBegin)

            tagValueListLen = tagValueList.getLen()

            res = self.fanListObj._fillWriteTagValues(tagValueList)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('fill-write-tag-values-fan-failed').errorFunc(): logFunc('fanListObj._fillWriteTagValues() failed. PARAMS')
                return ReturnCodes.kGeneralError
        ...
        if self.systemDefaultsObj:
            valBegin = Value()
            (tag, ns, prefix) = ("system-defaults" , "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "qt-pltf-fans")
            valBegin.setXmlBegin((tag, ns, prefix))
            tagValueList.push((tag, ns), valBegin)
    ...
    def _fillReadTagValues (self, tagValueList):
        ...
        if self.fanListObj:
            ...
            res = self.fanListObj._fillReadTagValues(tagValueList)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('fill-read-tag-values-fan-failed').errorFunc(): logFunc('fanListObj._fillReadTagValues() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _readTagValues (self, tagValueList, readAllOrFail):
        ...
        if self.isEnabledRequested():
            ...
            if (tag != "enabled") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-enabled').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "enabled", "enabled", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.alarmObj:
            ...
            if (tag != "alarm") or \
                ...
                (valBegin.getType() != Value.kXmlBegin):
                for logFunc in self._log('reag-tag-values-unexpected-tag-begin').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                        "alarm", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", Value.kXmlBegin,
                                                                        tag, ns, valBegin.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
            ...
            if (tag != "alarm") or \
                ...
                (valEnd.getType() != Value.kXmlEnd):
                for logFunc in self._log('reag-tag-values-unexpected-tag-end').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                      "alarm", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", Value.kXmlEnd,
                                                                        tag, ns, valEnd.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.fanListObj:
            ...
            if (tag != "fan") or \
                ...
                (valBegin.getType() != Value.kXmlBegin):
                for logFunc in self._log('reag-tag-values-unexpected-tag-begin').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                        "fan", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", Value.kXmlBegin,
                                                                        tag, ns, valBegin.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
            
            res = self.fanListObj._readTagValues(tagValueList, readAllOrFail)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('read-tag-values-fan-failed').errorFunc(): logFunc('fanListObj._readTagValues() failed. tagValueList=%s', tagValueList)
                if readAllOrFail:
                    self._clearAllReadData()
                return ReturnCodes.kGeneralError
            ...
            if (tag != "fan") or \
                ...
                (valEnd.getType() != Value.kXmlEnd):
                for logFunc in self._log('reag-tag-values-unexpected-tag-end').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                      "fan", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", Value.kXmlEnd,
                                                                        tag, ns, valEnd.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.systemDefaultsObj:
            ...
            if (tag != "system-defaults") or \
                ...
                (valBegin.getType() != Value.kXmlBegin):
                for logFunc in self._log('reag-tag-values-unexpected-tag-begin').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                        "system-defaults", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", Value.kXmlBegin,
                                                                        tag, ns, valBegin.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
            ...
            if (tag != "system-defaults") or \
                ...
                (valEnd.getType() != Value.kXmlEnd):
                for logFunc in self._log('reag-tag-values-unexpected-tag-end').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                      "system-defaults", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", Value.kXmlEnd,
                                                                        tag, ns, valEnd.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError

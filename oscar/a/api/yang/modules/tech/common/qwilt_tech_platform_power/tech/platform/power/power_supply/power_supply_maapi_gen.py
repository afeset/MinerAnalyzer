...
from power_supply_maapi_base_gen import PowerSupplyMaapiBase

from a.api.yang.modules.tech.common.qwilt_tech_platform_power.tech.platform.power.power_supply.status.status_maapi_gen import BlinkyStatusMaapi
from a.api.yang.modules.tech.common.qwilt_tech_platform_power.tech.platform.power.power_supply.alarm.alarm_maapi_gen import BlinkyAlarmMaapi
from a.api.yang.modules.tech.common.qwilt_tech_platform_power.tech.platform.power.power_supply.simulation.simulation_maapi_gen import BlinkySimulationMaapi
from a.api.yang.modules.tech.common.qwilt_tech_platform_power.tech.platform.power.power_supply.system_defaults.system_defaults_maapi_gen import BlinkySystemDefaultsMaapi
from a.api.yang.modules.tech.common.qwilt_tech_platform_power.tech.platform.power.power_supply.device.device_maapi_gen import BlinkyDeviceMaapi



class BlinkyPowerSupplyMaapi(PowerSupplyMaapiBase):
    def __init__ (self, logger):
        self.myInitGuard = InitGuard()
        self._log=logger.createLogger("sys-blinky-oper-example","blinky-maapi-powerSupply")
        self.domain = None
    ...
    def _collectItemsToDelete (self,
        ...
        if self.alarmObj:
            res = self.alarmObj._collectItemsToDelete(powerSupply, 
        ...
        if self.simulationObj:
            res = self.simulationObj._collectItemsToDelete(powerSupply, 
        ...
        if self.systemDefaultsObj:
            res = self.systemDefaultsObj._collectItemsToDelete(powerSupply, 
        ...
        if self.deviceObj:
            res = self.deviceObj._collectItemsToDelete(powerSupply, 
    ...
    def _fillWriteTagValues (self, tagValueList):
        ...
        if self.hasName():
            ...
            if self.name is not None:
                ...
                valName.setEmpty()
            tagValueList.push(("name", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-power"), valName)
        ...
        if self.deviceObj:
            valBegin = Value()
            (tag, ns, prefix) = ("device" , "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-power", "qt-pltf-pwr")
            valBegin.setXmlBegin((tag, ns, prefix))
            tagValueList.push((tag, ns), valBegin)
    ...
    def _readTagValues (self, tagValueList, readAllOrFail):
        ...
        if self.isNameRequested():
            ...
            if (tag != "name") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-name').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "name", "name", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-power", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.deviceObj:
            ...
            if (tag != "device") or \
                ...
                (valBegin.getType() != Value.kXmlBegin):
                for logFunc in self._log('reag-tag-values-unexpected-tag-begin').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                        "device", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-power", Value.kXmlBegin,
                                                                        tag, ns, valBegin.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
            ...
            if (tag != "device") or \
                ...
                (valEnd.getType() != Value.kXmlEnd):
                for logFunc in self._log('reag-tag-values-unexpected-tag-end').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                      "device", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-power", Value.kXmlEnd,
                                                                        tag, ns, valEnd.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError

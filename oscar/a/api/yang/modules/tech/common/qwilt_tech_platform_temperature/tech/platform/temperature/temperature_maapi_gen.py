...
from temperature_maapi_base_gen import TemperatureMaapiBase

from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.status.status_maapi_gen import BlinkyStatusMaapi
from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.alarm.alarm_maapi_gen import BlinkyAlarmMaapi
from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.simulation.simulation_maapi_gen import BlinkySimulationMaapi
from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.system_defaults.system_defaults_maapi_gen import BlinkySystemDefaultsMaapi
...
class BlinkyTemperatureMaapi(TemperatureMaapiBase):
    def __init__ (self, logger):
        self.myInitGuard = InitGuard()
        self._log=logger.createLogger("sys-blinky-oper-example","blinky-maapi-temperature")
        self.domain = None
    ...
    def requestConfigAndOper (self):
        ...
        if not self.sensorListObj:
            self.sensorListObj = self.newSensorList()
            self.sensorListObj.requestConfigAndOper()
    ...
    def requestConfig (self):
        ...
        if not self.sensorListObj:
            self.sensorListObj = self.newSensorList()
            self.sensorListObj.requestConfig()
    ...
    def requestOper (self):
        ...
        if not self.sensorListObj:
            self.sensorListObj = self.newSensorList()
            self.sensorListObj.requestOper()
    ...
    def clearAllRequested (self):
        ...
        if not self.sensorListObj:
            self.sensorListObj = self.newSensorList()
            self.sensorListObj.clearAllRequested()
    ...
    def clearAllSet (self):
        ...
        if self.sensorListObj:
            self.sensorListObj.clearAllSet()
    ...
    def newSensorList (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('new-sensorlist').debug3Func(): logFunc('called.')
        sensorList = BlinkySensorMaapiList(self._log)
        sensorList.init(self.domain)
        return sensorList

    def setSensorListObj (self, obj):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('set-sensorlist').debug3Func(): logFunc('called. obj=%s', obj)
        self.sensorListObj = obj

    def getSensorListObj (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('get-sensorlist').debug3Func(): logFunc('called. self.sensorListObj=%s', self.sensorListObj)
        return self.sensorListObj

    def hasSensorList (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('has-sensorlist').debug3Func(): logFunc('called. self.sensorListObj=%s', self.sensorListObj)
        if self.sensorListObj:
            return True
        return False
    ...
    def _clearAllReadData (self):
        ...
        if self.sensorListObj:
            self.sensorListObj._clearAllReadData()
    ...
    def _collectItemsToDelete (self,
        ...
        if self.sensorListObj:
            res = self.sensorListObj._collectItemsToDelete(
                                                                          itemsToDelete)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('collect-items-to-delete-sensor-failed').errorFunc(): logFunc('sensorListObj._collectItemsToDelete() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _fillWriteTagValues (self, tagValueList):
        ...
        if self.sensorListObj:
            ...
            res = self.sensorListObj._fillWriteTagValues(tagValueList)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('fill-write-tag-values-sensor-failed').errorFunc(): logFunc('sensorListObj._fillWriteTagValues() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _fillReadTagValues (self, tagValueList):
        ...
        if self.sensorListObj:
            ...
            res = self.sensorListObj._fillReadTagValues(tagValueList)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('fill-read-tag-values-sensor-failed').errorFunc(): logFunc('sensorListObj._fillReadTagValues() failed. PARAMS')
                return ReturnCodes.kGeneralError
    ...
    def _readTagValues (self, tagValueList, readAllOrFail):
        ...
        if self.isEnabledRequested():
            ...
            if (tag != "enabled") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-enabled').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "enabled", "enabled", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.sensorListObj:
            ...
            res = self.sensorListObj._readTagValues(tagValueList, readAllOrFail)
            if res != ReturnCodes.kOk:
                for logFunc in self._log('read-tag-values-sensor-failed').errorFunc(): logFunc('sensorListObj._readTagValues() failed. tagValueList=%s', tagValueList)
                if readAllOrFail:
                    self._clearAllReadData()
                return ReturnCodes.kGeneralError

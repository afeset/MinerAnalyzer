...
class BlinkySimulationMaapi(SimulationMaapiBase):
    ...
    def _fillWriteTagValues (self, tagValueList):
        ...
        if self.hasForceOperationalStatus():
            ...
            tagValueList.push(("force-operational-status", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), valForceOperationalStatus)
        ...
        if self.isForceOperationalStatusRequested():
            ...
            if (tag != "force-operational-status") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-forceoperationalstatus').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "forceOperationalStatus", "force-operational-status", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
...
Extracted from the below data: 
{
    "node": {
        ...
        "importStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.sensor.system_defaults.simulation.simulation_maapi_gen import SimulationMaapi", 

...
from analytics_maapi_base_gen import AnalyticsMaapiBase




class BlinkyAnalyticsMaapi(AnalyticsMaapiBase):
    def __init__ (self, logger):
        self.myInitGuard = InitGuard()
        self._log=logger.createLogger("sys-blinky-oper-example","blinky-maapi-analytics")
        self.domain = None
    ...
    def _fillWriteTagValues (self, tagValueList):
        ...
        if self.hasEnabled():
            ...
            if self.enabled is not None:
                ...
                valEnabled.setEmpty()
            tagValueList.push(("enabled", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces"), valEnabled)
        ...
        if self.isEnabledRequested():
            ...
            if (tag != "enabled") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-enabled').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "enabled", "enabled", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
...
Extracted from the below data: 
{
    "node": {
        "name": "analytics", 
        "namespace": "analytics", 
        "className": "AnalyticsMaapi", 
        "importStatement": "from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.system_defaults.content.analytics.analytics_maapi_gen import AnalyticsMaapi", 
        "baseClassName": "AnalyticsMaapiBase", 
        "baseModule": "analytics_maapi_base_gen"
    }, 
    "ancestors": [
        {

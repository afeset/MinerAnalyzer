...
from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.status.status_maapi_gen import BlinkyStatusMaapi
...
class BlinkyInterfacesMaapi(InterfacesMaapiBase):
    ...
    def _fillWriteTagValues (self, tagValueList):
        ...
        if self.interfaceListObj:
            valBegin = Value()
            (tag, ns, prefix) = ("interface" , "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "qt-if")
            valBegin.setXmlBegin((tag, ns, prefix))
            tagValueList.push((tag, ns), valBegin)
    ...
    def _readTagValues (self, tagValueList, readAllOrFail):
        ...
        if self.interfaceListObj:
            ...
            if (tag != "interface") or \
                ...
                (valBegin.getType() != Value.kXmlBegin):
                for logFunc in self._log('reag-tag-values-unexpected-tag-begin').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                        "interface", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", Value.kXmlBegin,
                                                                        tag, ns, valBegin.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
            ...
            if (tag != "interface") or \
                ...
                (valEnd.getType() != Value.kXmlEnd):
                for logFunc in self._log('reag-tag-values-unexpected-tag-end').errorFunc(): logFunc('got unexpected tag-value. expected: (%s, %s, type=%s), got: (%s, %s, type=%s)',
                                                                      "interface", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", Value.kXmlEnd,
                                                                        tag, ns, valEnd.getType())
                self._clearAllReadData()
                return ReturnCodes.kGeneralError

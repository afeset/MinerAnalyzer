...
class BlinkyStatusMaapi(StatusMaapiBase):
    ...
    def requestActualMethod (self, requested):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('request-actualmethod').debug3Func(): logFunc('called. requested=%s', requested)
        self.actualMethodRequested = requested
        self.actualMethodSet = False

    def isActualMethodRequested (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('is-actualmethod-requested').debug3Func(): logFunc('called. requested=%s', self.actualMethodRequested)
        return self.actualMethodRequested

    def getActualMethod (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('get-actualmethod').debug3Func(): logFunc('called. self.actualMethodSet=%s, self.actualMethod=%s', self.actualMethodSet, self.actualMethod)
    ...
    def hasActualMethod (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('has-actualmethod').debug3Func(): logFunc('called. self.actualMethodSet=%s, self.actualMethod=%s', self.actualMethodSet, self.actualMethod)
        if self.actualMethodSet:
            return True
        return False
    ...
    def setActualMethod (self, actualMethod):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('set-actualmethod').debug3Func(): logFunc('called. actualMethod=%s, old=%s', actualMethod, self.actualMethod)
    ...
    def _fillReadTagValues (self, tagValueList):
        ...
        if self.isActualMethodRequested():
            ...
            tagValueList.push(("actual-method", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces"), valActualMethod)
        ...
        if self.isOperationalStatusReasonRequested():
            ...
            tagValueList.push(("operational-status-reason", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces"), valOperationalStatusReason)
    ...
    def _readTagValues (self, tagValueList, readAllOrFail):
        ...
        if self.isActualMethodRequested():
            ...
            if (tag != "actual-method") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-actualmethod').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "actualMethod", "actual-method", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
            ...
            if res != ReturnCodes.kOk or tempVar is None:
                for logFunc in self._log('read-tag-values-actual-method-bad-value').infoFunc(): logFunc('actualMethod not read')
                if readAllOrFail:
                    self._clearAllReadData()
                    return ReturnCodes.kGeneralError
            if tempVar is not None:
                self.setActualMethod(tempVar)
            for logFunc in self._log('read-tag-values-actual-method').debug3Func(): logFunc('read actualMethod. actualMethod=%s, tempValue=%s', self.actualMethod, tempValue.getType())
        ...
        if self.isOperationalStatusReasonRequested():
            ...
            if (tag != "operational-status-reason") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-operationalstatusreason').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "operationalStatusReason", "operational-status-reason", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
...
Extracted from the below data: 
{
    "node": {
        ...
        "importStatement": "from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.connectivity_check.ipv6.status.status_maapi_gen import StatusMaapi", 

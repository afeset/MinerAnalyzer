...
from list_maapi_list_base_gen import ListMaapiListBase
from list_maapi_gen import BlinkyListMaapi

class BlinkyListMaapiList(ListMaapiListBase):
    ...
    def newList (self):
        ...
        for logFunc in self._log('new-list').debug3Func(): logFunc('called.')
        list = BlinkyListMaapi(self._log)
        list.init(self.domain)
        return list

    def setListObj (self, key, listObj):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('set-list-obj').debug3Func(): logFunc('called. key=%s, listObj=%s', key, listObj)
        if key not in self.lists:
            self.listKeys.append(key)
        self.lists[str(key)] = listObj

    def getListObj (self, key):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('get-list-obj').debug3Func(): logFunc('called. key=%s', key)
        if str(key) in self.lists.keys():
            for logFunc in self._log('get-list-obj-done').debug3Func(): logFunc('Done. found key=%s, obj=%s', key, self.lists[str(key)])
            return self.lists[str(key)]
        for logFunc in self._log('get-list-obj-missing').errorFunc(): logFunc('list %s not in lists. existing items: %s', key, self.lists.keys())
        return None

    def deleteList (self, key):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('delete-list').debug3Func(): logFunc('called. key=%s', key)
        if str(key) not in self.listKeys:
            for logFunc in self._log('delete-list-not-found').warningFunc(): logFunc('key=%s is missing from the listKeys list', key)
            if str(key) in self.lists.keys():
                # internal problem - list & dictionary are not synced
                for logFunc in self._log('delete-list-not-found-but-in-dict').errorFunc(): logFunc('lists dictionary & listKeys list are out-of-sync. key %s exists in dict but not in list', key)
            return ReturnCodes.kGeneralError
        if str(key) not in self.lists.keys():
            # internal problem - list & dictionary are not synced
            for logFunc in self._log('delete-list-not-found-but-in-list').errorFunc(): logFunc('lists dictionary & listKeys list are out-of-sync. key %s exists in list but not in dict', key)
            return ReturnCodes.kGeneralError

        self.listKeys.remove(str(key))
        del self.lists[str(key)]

    def hasListObj (self, key):
        self.myInitGuard.isInitOrCrash()
        has = False
        if str(key) in self.lists.keys():
            if self.lists[str(key)]:
                has = True
        for logFunc in self._log('has-list-done').debug3Func(): logFunc('done. key=%s exists=%s', key, has)
        return has

    def getListKeys (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('get-list-keys').debug3Func(): logFunc('called. keys=%s', [str(x) for x in self.listKeys])
    ...
    def readListKeys (self
        ...
        for key in keys:
            self.listKeys.append(key.getCannonicalStr())
            self.lists[key.getCannonicalStr()] = None
    ...
    def _fillWriteTagValues (self, tagValueList):
        ...
        for key in self.lists.keys():
            if self.lists[key]:
                ...
                valKey = Value()
                valKey.setInt64(key)
                tagValueList.push(("number", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valKey)

                tagValueListLen = tagValueList.getLen()

                res = self.lists[key]._fillWriteTagValues(tagValueList)
                if res != ReturnCodes.kOk:
                    for logFunc in self._log('fill-write-tag-values-list-failed').errorFunc(): logFunc('list._fillWriteTagValues() failed. key=%s', key)
                    return ReturnCodes.kGeneralError
    ...
    def _fillReadTagValues (self, tagValueList):
        ...
        for key in self.lists.keys():
            if self.lists[key]:
                ...
                res = self.lists[key]._fillReadTagValues(tagValueList)
                if res != ReturnCodes.kOk:
                    for logFunc in self._log('fill-read-tag-values-list-failed').errorFunc(): logFunc('list._fillReadTagValues() failed. key=%s', key)
                    return ReturnCodes.kGeneralError
            ...
            if self.lists[key]:
                ...
                if (tag != "number") or \
                    ...
                    for logFunc in self._log('reag-tag-values-unexpected-tag-key').errorFunc(): logFunc('got unexpected tag-value for key. expected: (%s, %s), got: (%s, %s)',
                                                                          "number", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                    self._clearAllReadData()
                    return ReturnCodes.kGeneralError
                ...
                res = self.lists[key]._readTagValues(tagValueList, readAllOrFail)
                if res != ReturnCodes.kOk:
                    for logFunc in self._log('read-tag-values-list-failed').errorFunc(): logFunc('list._readTagValues() failed. key=%s', key)
                    if readAllOrFail:
                        self._clearAllReadData()
                    return ReturnCodes.kGeneralError
...
{
    "node": {
        "containerClassName": "BlinkyListMaapi", 
        "name": "list", 
        "keyLeaf": {
            "varName": "list", 
            "yangName": "number", 
        ...
        "className": "ListMaapiList", 
        "importStatement": "from a.api.yang.modules.tech.common.qwilt_tech_system_alarms.tech.system.alarms.list.list_maapi_list_gen import ListMaapiList", 
        "baseClassName": "ListMaapiListBase", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", 
        "containerModule": "list_maapi_gen", 
        "baseModule": "list_maapi_list_base_gen"
    }, 
    "ancestors": [
        {

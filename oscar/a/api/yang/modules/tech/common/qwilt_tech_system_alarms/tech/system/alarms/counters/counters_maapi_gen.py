...
class BlinkyCountersMaapi(CountersMaapiBase):
    def __init__ (self, logger):
        ...
        self.alarmsCreatedRequested = False
        self.alarmsCreated = None
        self.alarmsCreatedSet = False
        ...
        self.pollErrorsRequested = False
        self.pollErrors = None
        self.pollErrorsSet = False
    ...
    def requestAlarmsCreated (self, requested):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('request-alarmscreated').debug3Func(): logFunc('called. requested=%s', requested)
        self.alarmsCreatedRequested = requested
        self.alarmsCreatedSet = False

    def isAlarmsCreatedRequested (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('is-alarmscreated-requested').debug3Func(): logFunc('called. requested=%s', self.alarmsCreatedRequested)
        return self.alarmsCreatedRequested

    def getAlarmsCreated (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('get-alarmscreated').debug3Func(): logFunc('called. self.alarmsCreatedSet=%s, self.alarmsCreated=%s', self.alarmsCreatedSet, self.alarmsCreated)
        if self.alarmsCreatedSet:
            return self.alarmsCreated
        return None

    def hasAlarmsCreated (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('has-alarmscreated').debug3Func(): logFunc('called. self.alarmsCreatedSet=%s, self.alarmsCreated=%s', self.alarmsCreatedSet, self.alarmsCreated)
        if self.alarmsCreatedSet:
            return True
        return False
    ...
    def setAlarmsCreated (self, alarmsCreated):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('set-alarmscreated').debug3Func(): logFunc('called. alarmsCreated=%s, old=%s', alarmsCreated, self.alarmsCreated)
        self.alarmsCreatedSet = True
    ...
    def requestPollErrors (self, requested):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('request-pollerrors').debug3Func(): logFunc('called. requested=%s', requested)
        self.pollErrorsRequested = requested
        self.pollErrorsSet = False

    def isPollErrorsRequested (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('is-pollerrors-requested').debug3Func(): logFunc('called. requested=%s', self.pollErrorsRequested)
        return self.pollErrorsRequested

    def getPollErrors (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('get-pollerrors').debug3Func(): logFunc('called. self.pollErrorsSet=%s, self.pollErrors=%s', self.pollErrorsSet, self.pollErrors)
        if self.pollErrorsSet:
            return self.pollErrors
        return None

    def hasPollErrors (self):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('has-pollerrors').debug3Func(): logFunc('called. self.pollErrorsSet=%s, self.pollErrors=%s', self.pollErrorsSet, self.pollErrors)
        if self.pollErrorsSet:
            return True
        return False
    ...
    def setPollErrors (self, pollErrors):
        self.myInitGuard.isInitOrCrash()
        for logFunc in self._log('set-pollerrors').debug3Func(): logFunc('called. pollErrors=%s, old=%s', pollErrors, self.pollErrors)
        self.pollErrorsSet = True
    ...
    def _getSelfKeyPath (self
        ...
        xmlVal = Value()
        xmlVal.setXmlTag(("counters", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", "qt-sys-alarms"))
    ...
    def _fillReadTagValues (self, tagValueList):
        ...
        if self.isActiveSecondsRequested():
            ...
            tagValueList.push(("active-seconds", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valActiveSeconds)
        
        if self.isPollLatencyErrorRequested():
            valPollLatencyError = Value()
            valPollLatencyError.setEmpty()
            tagValueList.push(("poll-latency-error", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valPollLatencyError)
        
        if self.isOverallPollDurationWarningRequested():
            valOverallPollDurationWarning = Value()
            valOverallPollDurationWarning.setEmpty()
            tagValueList.push(("overall-poll-duration-warning", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valOverallPollDurationWarning)
        
        if self.isTotalSecondsRequested():
            valTotalSeconds = Value()
            valTotalSeconds.setEmpty()
            tagValueList.push(("total-seconds", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valTotalSeconds)
        
        if self.isPollsRequested():
            valPolls = Value()
            valPolls.setEmpty()
            tagValueList.push(("polls", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valPolls)
        
        if self.isMissedPollsRequested():
            valMissedPolls = Value()
            valMissedPolls.setEmpty()
            tagValueList.push(("missed-polls", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valMissedPolls)
        
        if self.isSinglePollDurationWarningRequested():
            valSinglePollDurationWarning = Value()
            valSinglePollDurationWarning.setEmpty()
            tagValueList.push(("single-poll-duration-warning", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valSinglePollDurationWarning)
        
        if self.isSinglePollDurationErrorRequested():
            valSinglePollDurationError = Value()
            valSinglePollDurationError.setEmpty()
            tagValueList.push(("single-poll-duration-error", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valSinglePollDurationError)
        
        if self.isAlarmsCreatedRequested():
            valAlarmsCreated = Value()
            valAlarmsCreated.setEmpty()
            tagValueList.push(("alarms-created", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valAlarmsCreated)
        
        if self.isPollLatencyWarningRequested():
            valPollLatencyWarning = Value()
            valPollLatencyWarning.setEmpty()
            tagValueList.push(("poll-latency-warning", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valPollLatencyWarning)
        
        if self.isOverallPollDurationErrorRequested():
            valOverallPollDurationError = Value()
            valOverallPollDurationError.setEmpty()
            tagValueList.push(("overall-poll-duration-error", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valOverallPollDurationError)
        
        if self.isPollErrorsRequested():
            valPollErrors = Value()
            valPollErrors.setEmpty()
            tagValueList.push(("poll-errors", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"), valPollErrors)
    ...
    def _readTagValues (self, tagValueList, readAllOrFail):
        ...
        if self.isActiveSecondsRequested():
            ...
            if (tag != "active-seconds") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-activeseconds').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "activeSeconds", "active-seconds", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.isPollLatencyErrorRequested():
            ...
            if (tag != "poll-latency-error") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-polllatencyerror').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "pollLatencyError", "poll-latency-error", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.isOverallPollDurationWarningRequested():
            ...
            if (tag != "overall-poll-duration-warning") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-overallpolldurationwarning').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "overallPollDurationWarning", "overall-poll-duration-warning", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.isTotalSecondsRequested():
            ...
            if (tag != "total-seconds") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-totalseconds').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "totalSeconds", "total-seconds", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.isPollsRequested():
            ...
            if (tag != "polls") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-polls').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "polls", "polls", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.isMissedPollsRequested():
            ...
            if (tag != "missed-polls") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-missedpolls').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "missedPolls", "missed-polls", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.isSinglePollDurationWarningRequested():
            ...
            if (tag != "single-poll-duration-warning") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-singlepolldurationwarning').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "singlePollDurationWarning", "single-poll-duration-warning", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.isSinglePollDurationErrorRequested():
            ...
            if (tag != "single-poll-duration-error") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-singlepolldurationerror').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "singlePollDurationError", "single-poll-duration-error", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.isAlarmsCreatedRequested():
            ((tag, ns), tempValue) = tagValueList.popFront()
            if (tag != "alarms-created") or \
                (ns != "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"):
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-alarmscreated').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "alarmsCreated", "alarms-created", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
            ...
            tempVar = tempValue.asInt64()
            if res != ReturnCodes.kOk or tempVar is None:
                for logFunc in self._log('read-tag-values-alarms-created-bad-value').infoFunc(): logFunc('alarmsCreated not read')
                if readAllOrFail:
                    self._clearAllReadData()
                    return ReturnCodes.kGeneralError
            if tempVar is not None:
                self.setAlarmsCreated(tempVar)
            for logFunc in self._log('read-tag-values-alarms-created').debug3Func(): logFunc('read alarmsCreated. alarmsCreated=%s, tempValue=%s', self.alarmsCreated, tempValue.getType())
        ...
        if self.isPollLatencyWarningRequested():
            ...
            if (tag != "poll-latency-warning") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-polllatencywarning').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "pollLatencyWarning", "poll-latency-warning", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.isOverallPollDurationErrorRequested():
            ...
            if (tag != "overall-poll-duration-error") or \
                ...
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-overallpolldurationerror').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "overallPollDurationError", "overall-poll-duration-error", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
        ...
        if self.isPollErrorsRequested():
            ((tag, ns), tempValue) = tagValueList.popFront()
            if (tag != "poll-errors") or \
                (ns != "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms"):
                for logFunc in self._log('reag-tag-values-unexpected-tag-leaf-pollerrors').errorFunc(): logFunc('got unexpected tag-value for leaf: %s. expected: (%s, %s), got: (%s, %s)',
                                                                                                 "pollErrors", "poll-errors", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", tag, ns)
                self._clearAllReadData()
                return ReturnCodes.kGeneralError
            ...
            tempVar = tempValue.asInt64()
            if res != ReturnCodes.kOk or tempVar is None:
                for logFunc in self._log('read-tag-values-poll-errors-bad-value').infoFunc(): logFunc('pollErrors not read')
                if readAllOrFail:
                    self._clearAllReadData()
                    return ReturnCodes.kGeneralError
            if tempVar is not None:
                self.setPollErrors(tempVar)
            for logFunc in self._log('read-tag-values-poll-errors').debug3Func(): logFunc('read pollErrors. pollErrors=%s, tempValue=%s', self.pollErrors, tempValue.getType())

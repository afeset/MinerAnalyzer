...
from a.sys.log.blinky.manager.tech.logger_class.instance.rule.blinky_rule_gen import BlinkyRule

class BlinkyRuleList(BlinkyList):
    ...
    def __str__ (self):
        ret="{BlinkyRuleList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyrulelist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.log.blinky.manager.tech.logger_class.instance.rule.blinky_rule_gen import BlinkyRule', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyRule.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyrulelist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.log.blinky.manager.tech.logger_class.instance.rule.blinky_rule_gen import BlinkyRule', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-rule-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-rule-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-rule-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-rule").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyRuleList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-rule").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyRule. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyRule(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-rule-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyRule not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-rule").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyRule. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyRule(self, key):
        for logFunc in self._log("get-blinky-blinky-rule").debug1Func(): logFunc("getBlinkyRule(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyRule(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkyRule(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-rule-moved-failed").errorFunc(): logFunc("getBlinkyRule(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkyRule(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-rule-after-failed").errorFunc(): logFunc("getBlinkyRule(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-rule-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyRule-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyRule(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyRule", 
        "containerLogGroupName": "blinky-rule", 
        "namespace": "rule", 
        "containerImportStatement": "from a.sys.log.blinky.manager.tech.logger_class.instance.rule.blinky_rule_gen import BlinkyRule", 
        "logGroupName": "blinky-rule-list", 
        "className": "BlinkyRuleList", 
        "logModuleName": "a-sys-log-blinky-manager-tech-logger-class-instance-rule-blinky-rule-list-gen", 
        "keyLeaf": {
            "varName": "rule", 

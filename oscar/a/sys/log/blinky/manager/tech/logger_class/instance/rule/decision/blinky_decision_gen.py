...
from a.api.yang.modules.tech.debug.qwilt_tech_debug.tech.logger_class.instance.rule.decision.decision_data_gen import DecisionData
...
class BlinkyDecision(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-debug"
    #leaves
    
    ourXmlTagLogBacktrace="log-backtrace"
    
    ourXmlTagIgnoreCondition="ignore-condition"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkydecision').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyDecision._validationPointId, BlinkyDecision._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagLogBacktrace)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                if not EnableDecision.isValidValue(newValue.asEnum()):
                    self._log("trx-element-update-candidate-illegal-enum-logbacktrace-failed")\
                        .error("illegal enum value %s for logBacktrace", newValue.asEnum())
                    return ReturnCodes.kGeneralError
                self.myCandidateData.logBacktrace = EnableDecision.getByValue(newValue.asEnum())
                self.myCandidateData.setHasLogBacktrace()
                
            else:
                self.setLogBacktraceDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagIgnoreCondition)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                if not EnableDecision.isValidValue(newValue.asEnum()):
                    self._log("trx-element-update-candidate-illegal-enum-ignorecondition-failed")\
                        .error("illegal enum value %s for ignoreCondition", newValue.asEnum())
                    return ReturnCodes.kGeneralError
                self.myCandidateData.ignoreCondition = EnableDecision.getByValue(newValue.asEnum())
                self.myCandidateData.setHasIgnoreCondition()
                
            else:
                self.setIgnoreConditionDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagLog)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                if not EnableDecision.isValidValue(newValue.asEnum()):
                    self._log("trx-element-update-candidate-illegal-enum-log-failed")\
                        .error("illegal enum value %s for log", newValue.asEnum())
                    return ReturnCodes.kGeneralError
                self.myCandidateData.log = EnableDecision.getByValue(newValue.asEnum())
                self.myCandidateData.setHasLog()
                
            else:
                self.setLogDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkydecision').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.log.blinky.manager.tech.logger_class.instance.rule.decision.blinky_decision_gen import BlinkyDecision', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagLogBacktrace)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagIgnoreCondition)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagLog)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkydecision-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.log.blinky.manager.tech.logger_class.instance.rule.decision.blinky_decision_gen import BlinkyDecision', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = DecisionData()
        
        res = self.setLogBacktraceDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-logbacktrace-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setLogBacktraceDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setIgnoreConditionDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-ignorecondition-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setIgnoreConditionDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setLogDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-log-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setLogDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setLogBacktraceDefaultValue (self, setHas):
        for logFunc in self._log("set-logbacktrace-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.logBacktrace = EnableDecision.kPassThrough
    ...
    def setIgnoreConditionDefaultValue (self, setHas):
        for logFunc in self._log("set-ignorecondition-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.ignoreCondition = EnableDecision.kPassThrough
    ...
    def setLogDefaultValue (self, setHas):
        for logFunc in self._log("set-log-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.log = EnableDecision.kPassThrough
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = DecisionData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkydecision-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkydecision-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.debug.qwilt_tech_debug.tech.logger_class.instance.rule.decision.decision_data_gen import DecisionData", 
        "moduleYangNamespacePrefix": "qt-debug", 
        "validationPoint": null, 
        "yangName": "decision", 
        "namespace": "decision", 
        "logGroupName": "blinky-decision", 
        "className": "BlinkyDecision", 
        "logModuleName": "a-sys-log-blinky-manager-tech-logger-class-instance-rule-decision-blinky-decision-gen", 

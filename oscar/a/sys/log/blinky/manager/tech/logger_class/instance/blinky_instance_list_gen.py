...
from a.sys.log.blinky.manager.tech.logger_class.instance.blinky_instance_gen import BlinkyInstance

class BlinkyInstanceList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyInstanceList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyInstanceList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyinstancelist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.log.blinky.manager.tech.logger_class.instance.blinky_instance_gen import BlinkyInstance', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyInstance.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyinstancelist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.log.blinky.manager.tech.logger_class.instance.blinky_instance_gen import BlinkyInstance', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-instance-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-instance-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-instance").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyInstanceList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-instance").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyInstance. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyInstance(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-instance-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyInstance not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-instance").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyInstance. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyInstance(self, key):
        for logFunc in self._log("get-blinky-blinky-instance").debug1Func(): logFunc("getBlinkyInstance(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyInstance(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-instance-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyInstance-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyInstance(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyInstance", 
        "containerLogGroupName": "blinky-instance", 
        "namespace": "instance", 
        "containerImportStatement": "from a.sys.log.blinky.manager.tech.logger_class.instance.blinky_instance_gen import BlinkyInstance", 
        "logGroupName": "blinky-instance-list", 
        "className": "BlinkyInstanceList", 
        "logModuleName": "a-sys-log-blinky-manager-tech-logger-class-instance-blinky-instance-list-gen", 
        "keyLeaf": {
            "varName": "instance", 

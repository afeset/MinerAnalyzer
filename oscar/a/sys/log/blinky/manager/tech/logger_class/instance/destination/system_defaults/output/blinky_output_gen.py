...
from a.api.yang.modules.tech.debug.qwilt_tech_debug.tech.logger_class.instance.destination.system_defaults.output.output_data_gen import OutputData
...
class BlinkyOutput(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-debug"
    #leaves
    
    ourXmlTagFileBaseName="file-base-name"
    
    ourXmlTagMaxFileSizePercent="max-file-size-percent"
    
    ourXmlTagFileDirectory="file-directory"
    
    ourXmlTagArchiveMode="archive-mode"
    
    ourXmlTagMaxSizeMb="max-size-mb"
    
    ourXmlTagWriteMode="write-mode"
    
    ourXmlTagFileRotationIntervalMinutes="file-rotation-interval-minutes"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyoutput').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyOutput._validationPointId, BlinkyOutput._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagFileBaseName)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.fileBaseName = newValue.asString()
                self.myCandidateData.setHasFileBaseName()
                
            else:
                self.setFileBaseNameDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagMaxFileSizePercent)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.maxFileSizePercent = newValue.asInt64()
                self.myCandidateData.setHasMaxFileSizePercent()
                
            else:
                self.setMaxFileSizePercentDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagFileDirectory)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.fileDirectory = newValue.asString()
                self.myCandidateData.setHasFileDirectory()
                
            else:
                self.setFileDirectoryDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagArchiveMode)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                if not LogArchiveModeType.isValidValue(newValue.asEnum()):
                    self._log("trx-element-update-candidate-illegal-enum-archivemode-failed")\
                        .error("illegal enum value %s for archiveMode", newValue.asEnum())
                    return ReturnCodes.kGeneralError
                self.myCandidateData.archiveMode = LogArchiveModeType.getByValue(newValue.asEnum())
                self.myCandidateData.setHasArchiveMode()
                
            else:
                self.setArchiveModeDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagMaxSizeMb)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.maxSizeMb = newValue.asInt64()
                self.myCandidateData.setHasMaxSizeMb()
                
            else:
                self.setMaxSizeMbDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagWriteMode)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                if not LogWriteModeType.isValidValue(newValue.asEnum()):
                    self._log("trx-element-update-candidate-illegal-enum-writemode-failed")\
                        .error("illegal enum value %s for writeMode", newValue.asEnum())
                    return ReturnCodes.kGeneralError
                self.myCandidateData.writeMode = LogWriteModeType.getByValue(newValue.asEnum())
                self.myCandidateData.setHasWriteMode()
                
            else:
                self.setWriteModeDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagFileRotationIntervalMinutes)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.fileRotationIntervalMinutes = newValue.asInt64()
                self.myCandidateData.setHasFileRotationIntervalMinutes()
                
            else:
                self.setFileRotationIntervalMinutesDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyoutput').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.log.blinky.manager.tech.logger_class.instance.destination.system_defaults.output.blinky_output_gen import BlinkyOutput', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagFileBaseName)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagMaxFileSizePercent)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagFileDirectory)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagArchiveMode)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagMaxSizeMb)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagWriteMode)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagFileRotationIntervalMinutes)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkyoutput-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.log.blinky.manager.tech.logger_class.instance.destination.system_defaults.output.blinky_output_gen import BlinkyOutput', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = OutputData()
        
        res = self.setFileBaseNameDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-filebasename-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setFileBaseNameDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setMaxFileSizePercentDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-maxfilesizepercent-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setMaxFileSizePercentDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setFileDirectoryDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-filedirectory-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setFileDirectoryDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setArchiveModeDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-archivemode-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setArchiveModeDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setMaxSizeMbDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-maxsizemb-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setMaxSizeMbDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setWriteModeDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-writemode-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setWriteModeDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setFileRotationIntervalMinutesDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-filerotationintervalminutes-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setFileRotationIntervalMinutesDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setFileBaseNameDefaultValue (self, setHas):
        for logFunc in self._log("set-filebasename-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.fileBaseName = ""
    ...
    def setMaxFileSizePercentDefaultValue (self, setHas):
        for logFunc in self._log("set-maxfilesizepercent-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.maxFileSizePercent = 0
    ...
    def setFileDirectoryDefaultValue (self, setHas):
        for logFunc in self._log("set-filedirectory-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.fileDirectory = ""
    ...
    def setArchiveModeDefaultValue (self, setHas):
        for logFunc in self._log("set-archivemode-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.archiveMode = LogArchiveModeType.kNone
    ...
    def setMaxSizeMbDefaultValue (self, setHas):
        for logFunc in self._log("set-maxsizemb-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.maxSizeMb = 0
    ...
    def setWriteModeDefaultValue (self, setHas):
        for logFunc in self._log("set-writemode-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.writeMode = LogWriteModeType.kSafe
    ...
    def setFileRotationIntervalMinutesDefaultValue (self, setHas):
        for logFunc in self._log("set-filerotationintervalminutes-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.fileRotationIntervalMinutes = 0
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyoutput-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyoutput-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.debug.qwilt_tech_debug.tech.logger_class.instance.destination.system_defaults.output.output_data_gen import OutputData", 
        "moduleYangNamespacePrefix": "qt-debug", 
        "validationPoint": null, 
        "yangName": "output", 
        "namespace": "output", 
        "logGroupName": "blinky-output", 
        "className": "BlinkyOutput", 
        "logModuleName": "a-sys-log-blinky-manager-tech-logger-class-instance-destination-system-defaults-output-blinky-output-gen", 

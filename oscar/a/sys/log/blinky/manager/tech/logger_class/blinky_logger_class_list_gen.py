...
from a.sys.log.blinky.manager.tech.logger_class.blinky_logger_class_gen import BlinkyLoggerClass

class BlinkyLoggerClassList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyLoggerClassList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyLoggerClassList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyloggerclasslist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.log.blinky.manager.tech.logger_class.blinky_logger_class_gen import BlinkyLoggerClass', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyLoggerClass.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyloggerclasslist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.log.blinky.manager.tech.logger_class.blinky_logger_class_gen import BlinkyLoggerClass', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-logger-class-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-logger-class-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-logger-class").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyLoggerClassList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-logger-class").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyLoggerClass. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyLoggerClass(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-logger-class-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyLoggerClass not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-logger-class").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyLoggerClass. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyLoggerClass(self, key):
        for logFunc in self._log("get-blinky-blinky-logger-class").debug1Func(): logFunc("getBlinkyLoggerClass(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyLoggerClass(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-logger-class-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyLoggerClass-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyLoggerClass(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyLoggerClass", 
        "containerLogGroupName": "blinky-logger-class", 
        "namespace": "logger_class", 
        "containerImportStatement": "from a.sys.log.blinky.manager.tech.logger_class.blinky_logger_class_gen import BlinkyLoggerClass", 
        "logGroupName": "blinky-logger-class-list", 
        "className": "BlinkyLoggerClassList", 
        "logModuleName": "a-sys-log-blinky-manager-tech-logger-class-blinky-logger-class-list-gen", 
        "keyLeaf": {
            "varName": "loggerClass", 

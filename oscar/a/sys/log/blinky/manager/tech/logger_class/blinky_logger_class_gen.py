...
from a.api.yang.modules.tech.debug.qwilt_tech_debug.tech.logger_class.logger_class_data_gen import LoggerClassData


from a.sys.log.blinky.manager.tech.logger_class.instance.blinky_instance_list_gen import BlinkyInstanceList




class BlinkyLoggerClass(BlinkyContainer):
    ...
    ourXmlTagInstanceList="instance"
    ...
    INSTANCELIST_CREATE_FUNCTOR = 'INSTANCELIST_CREATE_FUNCTOR'
    INSTANCELIST_DELETE_FUNCTOR = 'INSTANCELIST_DELETE_FUNCTOR'
    ...
    def __init__ (self, logger):
        ...
        self.myCreateInstanceListFunctor=None
        self.myDeleteInstanceListFunctor=None
        self.myInstanceList=None
    ...
    def setCreateInstanceListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-create-instance-functor-active").errorFunc():
                logFunc("setCreateInstanceListFunctor() is illegal when blinky node is active")
        self.myCreateInstanceListFunctor = functor

    def setDeleteInstanceListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-delete-instance-functor-active").errorFunc():
                logFunc("setDeleteInstanceListFunctor() is illegal when blinky node is active")
        self.myDeleteInstanceListFunctor = functor
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if (keyPath.isTagEqual(keyDepth_PBR.value(), self.ourNamespace, self.ourXmlTagInstanceList)):
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyloggerclass').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.log.blinky.manager.tech.logger_class.blinky_logger_class_gen import BlinkyLoggerClass', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        if not found:
            if keyPath.getLen() > keyDepth+1:
                if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagInstanceList)):
                    keyPathRegistered = BlinkyInstanceList.isKeyPathRegistered(logger, keyPath, keyDepth+1)
                    found = True
            else:
        ...
        logger('is-key-path-registered-blinkyloggerclass-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.log.blinky.manager.tech.logger_class.blinky_logger_class_gen import BlinkyLoggerClass', keyPathRegistered, keyPath, keyDepth)
    ...
    def prepareMyBlinkyNode(self, keyPath, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagInstanceList)):
            if (self.myInstanceList):
                for logFunc in self._log("prepare-my-blinky-node-instancelist-already-exists").errorFunc():
                    logFunc("prepareMyBlinkyNode(): already exists, keyPath=%s, keyDepth=%s", keyPath, keyDepth)
                return None
            self.myInstanceList = BlinkyInstanceList(self._log)
            self.myInstanceList.setParent(self)
            self.myInstanceList.setKeyPath(keyPath)
            self.myInstanceList.setDomain(self.myDomain)
    ...
    def handleTrxElementCreate(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagInstanceList)):
                if (self.myCreateInstanceListFunctor):
                    if (not self.myInstanceList):
                        for logFunc in self._log("handle-trx-element-create-instancelist-not-exist").debug3Func():
                            logFunc("handleTrxElementCreate(): instancelist not exists, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    
                    timeoutGuard = TimeoutGuard(self._log, '%s-create-instancelist-functor-%s' % (self.myKeyPath, phase), 
                                                self.getFunctorTimeoutForPhase(self.INSTANCELIST_CREATE_FUNCTOR, phase),
                                                self.getFunctorMildTimeoutForPhase(self.INSTANCELIST_CREATE_FUNCTOR, phase))
                    try:
                        res = self.myCreateInstanceListFunctor(phase, self.myInstanceList)
                    except:
                        for logFunc in self._log("handle-trx-element-create-instancelist-functor-exception").exceptionFunc():
                            logFunc("InstanceList's create functor raised an exception. trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        raise
                    timeoutGuard.checkAndLog("application problem: functor=%s" % self.myCreateInstanceListFunctor.__name__)
                    if (res != ReturnCodes.kOk):
                        if (phase.getConfdPhase() == TrxPhase.kPrepare):
                            for logFunc in self._log("handle-trx-element-create-instancelist-functor-failed-prepare").noticeFunc():
                                logFunc("handleTrxElementCreate(): instancelist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                            self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        else:
                            for logFunc in self._log("handle-trx-element-create-instancelist-functor-failed").errorFunc():
                                logFunc("handleTrxElementCreate(): instancelist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    ...
    def notifyDescendantsPrepareBlinkyDelete(self):
        ...
        if (self.myInstanceList):
            self.myInstanceList.notifyDescendantsPrepareBlinkyDelete()
    ...
    def handleTrxElementDelete(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagInstanceList)):
                res = self.activateDeleteInstanceListFunctor(phase)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("handle-trx-element-delete-instancelist-functor-failed-prepare").noticeFunc():
                            logFunc("handleTrxElementDelete(): instancelist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("handle-trx-element-delete-instancelist-functor-failed").errorFunc():
                            logFunc("handleTrxElementDelete(): instancelist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                    return ReturnCodes.kGeneralError
    ...
    def removeInstanceList(self):
        self.myInstanceList = None

    def activateDeleteInstanceListFunctor(self, phase):
        for logFunc in self._log("activate-delete-instancelist-functor").debug3Func(): logFunc("activateDeleteInstanceListFunctor(): called, phase=%s", phase)
        if (self.myIsActive):
            if (self.myDeleteInstanceListFunctor):
                timeoutGuard = TimeoutGuard(self._log, '%s-delete-instancelist-functor-%s' % (self.myKeyPath, phase), 
                                            self.getFunctorTimeoutForPhase(self.INSTANCELIST_DELETE_FUNCTOR, phase), 
                                            self.getFunctorMildTimeoutForPhase(self.INSTANCELIST_DELETE_FUNCTOR, phase))
                try:
                    res = self.myDeleteInstanceListFunctor(phase)
                except:
                    for logFunc in self._log("activate-delete-instancelist-functor-exception").exceptionFunc():
                        logFunc("InstanceList's delete functor raised an exception. phase=%s", phase)
                    raise
                timeoutGuard.checkAndLog("application problem: functor=%s" % self.myDeleteInstanceListFunctor.__name__)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("activate-delete-instancelist-functor-functor-failed-prepare").noticeFunc():
                            logFunc("activateDeleteInstanceListFunctor(): functor failed, phase=%s", phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("activate-delete-instancelist-functor-functor-failed").errorFunc():
                            logFunc("activateDeleteInstanceListFunctor(): functor failed, phase=%s", phase)
                    return ReturnCodes.kGeneralError
        return ReturnCodes.kOk
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = LoggerClassData()
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = LoggerClassData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyloggerclass-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyloggerclass-failed").errorFunc():
    ...
    def handleInternalDestroy(self, phase):
        ...
        if (phase.getConfdPhase() == TrxPhase.kPrepare):
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myInstanceList):
                    res = self.myInstanceList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-instancelist-recursion-failed-prepare-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): delete-instancelist-recursion-failed-prepare-blinky, phase=%s",
                                  phase)
                        return ReturnCodes.kGeneralError
                ...
                if (self.myInstanceList):
                    res = self.activateDeleteInstanceListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-instancelist-functor-failed-prepare").noticeFunc():
                            logFunc("handleInternalDestroy(): activateDeleteInstanceListFunctor() failed, res=%s, phase=%s",
                                   res, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        return ReturnCodes.kGeneralError
                if (self.myInstanceList):
                    res = self.myInstanceList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-instancelist-recursion-failed-prepare-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myInstanceList.handleInternalDestroy() failed, res=%s, phase=%s",
                                   res, phase)
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myInstanceList):
                    res = self.myInstanceList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-instancelist-recursion-failed-commit-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myInstanceList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myInstanceList):
                    res = self.myInstanceList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-instancelist-recursion-failed-commit-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myInstanceList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                    ...
                    res = self.activateDeleteInstanceListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-instancelist-functor-failed-commit").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteInstanceListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("activateDeleteInstanceListFunctor failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myInstanceList):
                    res = self.myInstanceList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-instancelist-recursion-failed-abort-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myInstanceList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myInstanceList):
                    res = self.myInstanceList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-instancelist-recursion-failed-abort-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myInstanceList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myInstanceList):
                    res = self.activateDeleteInstanceListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-instancelist-functor-failed-abort").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteInstanceListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.debug.qwilt_tech_debug.tech.logger_class.logger_class_data_gen import LoggerClassData", 
        "moduleYangNamespacePrefix": "qt-debug", 
        "validationPoint": null, 
        "yangName": "logger-class", 
        "namespace": "logger_class", 
        "logGroupName": "blinky-logger-class", 
        "className": "BlinkyLoggerClass", 
        "logModuleName": "a-sys-log-blinky-manager-tech-logger-class-blinky-logger-class-gen", 
        "importStatement": "from a.sys.log.blinky.manager.tech.logger_class.blinky_logger_class_gen import BlinkyLoggerClass", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-debug", 
        "dataClassName": "LoggerClassData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {
    ...
    "descendants": [
        {
            "className": "BlinkyInstanceList", 
            "memberName": "InstanceList", 
            "yangName": "instance", 
            "importStatement": "from a.sys.log.blinky.manager.tech.logger_class.instance.blinky_instance_list_gen import BlinkyInstanceList"
        }
    ], 

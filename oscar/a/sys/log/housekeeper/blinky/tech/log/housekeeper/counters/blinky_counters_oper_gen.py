...
from a.api.yang.modules.tech.debug.qwilt_tech_log.tech.log.housekeeper.counters.counters_oper_data_gen import CountersOperData
...
class BlinkyOperCounters(BlinkyOperNode):
    ...
    _kCallpointName = "tech-log-housekeeper-counters-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-log-housekeeper-counters-callpoint"
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", "total-seconds"):
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", "polls"):
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", "polls-missed"):
                    for logFunc in self._log("set-oper-data-requested-fields-pollsmissed-requested").debug3Func(): logFunc(
                        "polls-missed requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setPollsMissedRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", "poll-latency-warnings"):
                    for logFunc in self._log("set-oper-data-requested-fields-polllatencywarnings-requested").debug3Func(): logFunc(
                        "poll-latency-warnings requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setPollLatencyWarningsRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", "poll-latency-errors"):
                    for logFunc in self._log("set-oper-data-requested-fields-polllatencyerrors-requested").debug3Func(): logFunc(
                        "poll-latency-errors requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setPollLatencyErrorsRequested()
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", "polls-missed"):
            if operData.isPollsMissedRequested():
                 if operData.hasPollsMissed():
                     value.setInt64(operData.pollsMissed)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", "poll-latency-warnings"):
            if operData.isPollLatencyWarningsRequested():
                 if operData.hasPollLatencyWarnings():
                     value.setInt64(operData.pollLatencyWarnings)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-log", "poll-latency-errors"):
            if operData.isPollLatencyErrorsRequested():
                 if operData.hasPollLatencyErrors():
                     value.setInt64(operData.pollLatencyErrors)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.debug.qwilt_tech_log.tech.log.housekeeper.counters.counters_oper_data_gen import CountersOperData", 
        ...
        "logModuleName": "a-sys-log-housekeeper-blinky-tech-log-housekeeper-counters-blinky-counters-oper-gen", 
        "importStatement": "from a.sys.log.housekeeper.blinky.tech.log.housekeeper.counters.blinky_counters_oper_gen import BlinkyOperCounters", 
        "callpointName": "tech-log-housekeeper-counters-callpoint", 

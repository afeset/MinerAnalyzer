...
from a.sys.linux.manager.variable_collection.blinky.tech_linux_variables.tech.linux_.variable_collection.blinky_variable_collection_gen import BlinkyVariableCollection

class BlinkyVariableCollectionList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyVariableCollectionList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyVariableCollectionList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyvariablecollectionlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.linux.manager.variable_collection.blinky.tech_linux_variables.tech.linux_.variable_collection.blinky_variable_collection_gen import BlinkyVariableCollection', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyVariableCollection.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyvariablecollectionlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.linux.manager.variable_collection.blinky.tech_linux_variables.tech.linux_.variable_collection.blinky_variable_collection_gen import BlinkyVariableCollection', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-variable-collection-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-variable-collection-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-variable-collection").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyVariableCollectionList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-variable-collection").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyVariableCollection. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyVariableCollection(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-variable-collection-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyVariableCollection not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-variable-collection").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyVariableCollection. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyVariableCollection(self, key):
        for logFunc in self._log("get-blinky-blinky-variable-collection").debug1Func(): logFunc("getBlinkyVariableCollection(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyVariableCollection(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-variable-collection-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyVariableCollection-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyVariableCollection(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyVariableCollection", 
        "containerLogGroupName": "blinky-variable-collection", 
        "namespace": "variable_collection", 
        "containerImportStatement": "from a.sys.linux.manager.variable_collection.blinky.tech_linux_variables.tech.linux_.variable_collection.blinky_variable_collection_gen import BlinkyVariableCollection", 
        "logGroupName": "blinky-variable-collection-list", 
        "className": "BlinkyVariableCollectionList", 
        "logModuleName": "a-sys-linux-manager-variable-collection-blinky-tech-linux-variables-tech-linux--variable-collection-blinky-variable-collection-list-gen", 
        "keyLeaf": {

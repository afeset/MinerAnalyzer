...
from a.sys.linux.manager.variable_collection.blinky.tech_linux_variables.tech.linux_.variable_collection.variable.blinky_variable_gen import BlinkyVariable

class BlinkyVariableList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyVariableList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyVariableList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyvariablelist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.linux.manager.variable_collection.blinky.tech_linux_variables.tech.linux_.variable_collection.variable.blinky_variable_gen import BlinkyVariable', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyVariable.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyvariablelist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.linux.manager.variable_collection.blinky.tech_linux_variables.tech.linux_.variable_collection.variable.blinky_variable_gen import BlinkyVariable', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-variable-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-variable-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-variable").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyVariableList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-variable").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyVariable. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyVariable(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-variable-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyVariable not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-variable").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyVariable. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyVariable(self, key):
        for logFunc in self._log("get-blinky-blinky-variable").debug1Func(): logFunc("getBlinkyVariable(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyVariable(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-variable-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyVariable-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyVariable(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyVariable", 
        "containerLogGroupName": "blinky-variable", 
        "namespace": "variable", 
        "containerImportStatement": "from a.sys.linux.manager.variable_collection.blinky.tech_linux_variables.tech.linux_.variable_collection.variable.blinky_variable_gen import BlinkyVariable", 
        "logGroupName": "blinky-variable-list", 
        "className": "BlinkyVariableList", 
        "logModuleName": "a-sys-linux-manager-variable-collection-blinky-tech-linux-variables-tech-linux--variable-collection-variable-blinky-variable-list-gen", 
        "keyLeaf": {
            "varName": "variable", 

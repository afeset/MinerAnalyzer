...
from a.api.yang.modules.tech.common.qwilt_tech_linux_variables.tech.linux_.variable_collection.variable.status.status_oper_data_gen import StatusOperData
...
class BlinkyOperStatus(BlinkyOperNode):
    ...
    _kCallpointName = "tech-linux-variable-collection-variable-status-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-linux-variable-collection-variable-status-callpoint"
    ...
    def getOperRelativePath (self, operRelativePath):
        ...
        val = Value()
        val.setXmlTag(("status", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-linux-variables", "qt-lnx-variables"))
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-linux-variables", "initial-value"):
                    for logFunc in self._log("set-oper-data-requested-fields-initialvalue-requested").debug3Func(): logFunc(
                        "initial-value requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setInitialValueRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-linux-variables", "actual-value"):
                    for logFunc in self._log("set-oper-data-requested-fields-actualvalue-requested").debug3Func(): logFunc(
                        "actual-value requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setActualValueRequested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isInitialValueRequested() and operData.hasInitialValue():
            val = Value()
            val.setString(operData.initialValue)
            tagValueList.push(("initial-value", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-linux-variables"), val)
        
        if operData.isActualValueRequested() and operData.hasActualValue():
            val = Value()
            val.setString(operData.actualValue)
            tagValueList.push(("actual-value", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-linux-variables"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-linux-variables", "initial-value"):
            if operData.isInitialValueRequested():
                 if operData.hasInitialValue():
                     value.setString(operData.initialValue)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-linux-variables", "actual-value"):
            if operData.isActualValueRequested():
                 if operData.hasActualValue():
                     value.setString(operData.actualValue)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_linux_variables.tech.linux_.variable_collection.variable.status.status_oper_data_gen import StatusOperData", 
        ...
        "logModuleName": "a-sys-linux-manager-variable-collection-blinky-tech-linux-variables-tech-linux--variable-collection-variable-status-blinky-status-oper-gen", 
        "importStatement": "from a.sys.linux.manager.variable_collection.blinky.tech_linux_variables.tech.linux_.variable_collection.variable.status.blinky_status_oper_gen import BlinkyOperStatus", 
        "callpointName": "tech-linux-variable-collection-variable-status-callpoint", 

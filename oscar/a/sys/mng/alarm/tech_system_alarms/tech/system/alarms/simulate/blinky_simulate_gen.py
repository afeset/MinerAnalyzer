...
class BlinkySimulate(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkysimulate').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkySimulate._validationPointId, BlinkySimulate._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagId)):
            ...
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                if not AlarmNameType.isValidValue(newValue.asEnum()):
                    self._log("trx-element-update-candidate-illegal-enum-name-failed")\
                        .error("illegal enum value %s for name", newValue.asEnum())
                    return ReturnCodes.kGeneralError
                self.myCandidateData.name = AlarmNameType.getByValue(newValue.asEnum())
                self.myCandidateData.setHasName()
                
            else:
                self.setNameDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagEntity)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.entity = newValue.asString()
                self.myCandidateData.setHasEntity()
                
            else:
                self.setEntityDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkysimulate').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.mng.alarm.tech_system_alarms.tech.system.alarms.simulate.blinky_simulate_gen import BlinkySimulate', keyPath, keyDepth)
        ...
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagEntity)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkysimulate-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.mng.alarm.tech_system_alarms.tech.system.alarms.simulate.blinky_simulate_gen import BlinkySimulate', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = SimulateData()
        ...
        res = self.setEntityDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-entity-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setEntityDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
    ...
    def setNameDefaultValue (self, setHas):
        ...
        self.myCandidateData.name = AlarmNameType.kTemperatureSensorWarning
    ...
    def setEntityDefaultValue (self, setHas):
        for logFunc in self._log("set-entity-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.entity = ""
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = SimulateData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkysimulate-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkysimulate-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_system_alarms.tech.system.alarms.simulate.simulate_data_gen import SimulateData", 
        "moduleYangNamespacePrefix": "qt-sys-alarms", 
        "validationPoint": null, 
        "yangName": "simulate", 
        "namespace": "simulate", 
        "logGroupName": "blinky-simulate", 
        "className": "BlinkySimulate", 
        "logModuleName": "a-sys-mng-alarm-tech-system-alarms-tech-system-alarms-simulate-blinky-simulate-gen", 
        "importStatement": "from a.sys.mng.alarm.tech_system_alarms.tech.system.alarms.simulate.blinky_simulate_gen import BlinkySimulate", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-alarms", 
        "dataClassName": "SimulateData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

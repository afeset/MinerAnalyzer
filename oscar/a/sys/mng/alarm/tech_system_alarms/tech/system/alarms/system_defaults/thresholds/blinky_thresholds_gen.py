...
from a.api.yang.modules.tech.common.qwilt_tech_system_alarms.tech.system.alarms.system_defaults.thresholds.thresholds_data_gen import ThresholdsData
...
class BlinkyThresholds(BlinkyContainer):
    ...
    ourXmlTagSinglePollDurationWarningMsec="single-poll-duration-warning-msec"
    
    ourXmlTagPollLatencyErrorSeconds="poll-latency-error-seconds"
    
    ourXmlTagSinglePollDurationErrorMsec="single-poll-duration-error-msec"
    
    ourXmlTagOverallPollDurationErrorMsec="overall-poll-duration-error-msec"
    
    ourXmlTagPollLatencyWarningSeconds="poll-latency-warning-seconds"
    
    ourXmlTagOverallPollDurationWarningMsec="overall-poll-duration-warning-msec"
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagSinglePollDurationWarningMsec)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.singlePollDurationWarningMsec = newValue.asInt64()
                self.myCandidateData.setHasSinglePollDurationWarningMsec()
                
            else:
                self.setSinglePollDurationWarningMsecDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagPollLatencyErrorSeconds)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.pollLatencyErrorSeconds = newValue.asInt64()
                self.myCandidateData.setHasPollLatencyErrorSeconds()
                
            else:
                self.setPollLatencyErrorSecondsDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagSinglePollDurationErrorMsec)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.singlePollDurationErrorMsec = newValue.asInt64()
                self.myCandidateData.setHasSinglePollDurationErrorMsec()
                
            else:
                self.setSinglePollDurationErrorMsecDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagOverallPollDurationErrorMsec)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.overallPollDurationErrorMsec = newValue.asInt64()
                self.myCandidateData.setHasOverallPollDurationErrorMsec()
                
            else:
                self.setOverallPollDurationErrorMsecDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagPollLatencyWarningSeconds)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.pollLatencyWarningSeconds = newValue.asInt64()
                self.myCandidateData.setHasPollLatencyWarningSeconds()
                
            else:
                self.setPollLatencyWarningSecondsDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagOverallPollDurationWarningMsec)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.overallPollDurationWarningMsec = newValue.asInt64()
                self.myCandidateData.setHasOverallPollDurationWarningMsec()
                
            else:
                self.setOverallPollDurationWarningMsecDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkythresholds').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.mng.alarm.tech_system_alarms.tech.system.alarms.system_defaults.thresholds.blinky_thresholds_gen import BlinkyThresholds', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagSinglePollDurationWarningMsec)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagPollLatencyErrorSeconds)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagSinglePollDurationErrorMsec)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagOverallPollDurationErrorMsec)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagPollLatencyWarningSeconds)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagOverallPollDurationWarningMsec)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkythresholds-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.mng.alarm.tech_system_alarms.tech.system.alarms.system_defaults.thresholds.blinky_thresholds_gen import BlinkyThresholds', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        ...
        res = self.setSinglePollDurationWarningMsecDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-singlepolldurationwarningmsec-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setSinglePollDurationWarningMsecDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setPollLatencyErrorSecondsDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-polllatencyerrorseconds-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setPollLatencyErrorSecondsDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setSinglePollDurationErrorMsecDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-singlepolldurationerrormsec-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setSinglePollDurationErrorMsecDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setOverallPollDurationErrorMsecDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-overallpolldurationerrormsec-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setOverallPollDurationErrorMsecDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setPollLatencyWarningSecondsDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-polllatencywarningseconds-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setPollLatencyWarningSecondsDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setOverallPollDurationWarningMsecDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-overallpolldurationwarningmsec-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setOverallPollDurationWarningMsecDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setSinglePollDurationWarningMsecDefaultValue (self, setHas):
        for logFunc in self._log("set-singlepolldurationwarningmsec-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.singlePollDurationWarningMsec = 0
    ...
    def setPollLatencyErrorSecondsDefaultValue (self, setHas):
        for logFunc in self._log("set-polllatencyerrorseconds-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.pollLatencyErrorSeconds = 0
    ...
    def setSinglePollDurationErrorMsecDefaultValue (self, setHas):
        for logFunc in self._log("set-singlepolldurationerrormsec-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.singlePollDurationErrorMsec = 0
    ...
    def setOverallPollDurationErrorMsecDefaultValue (self, setHas):
        for logFunc in self._log("set-overallpolldurationerrormsec-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.overallPollDurationErrorMsec = 0
    ...
    def setPollLatencyWarningSecondsDefaultValue (self, setHas):
        for logFunc in self._log("set-polllatencywarningseconds-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.pollLatencyWarningSeconds = 0
    ...
    def setOverallPollDurationWarningMsecDefaultValue (self, setHas):
        for logFunc in self._log("set-overallpolldurationwarningmsec-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.overallPollDurationWarningMsec = 0
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_system_alarms.tech.system.alarms.system_defaults.thresholds.thresholds_data_gen import ThresholdsData", 
        ...
        "logModuleName": "a-sys-mng-alarm-tech-system-alarms-tech-system-alarms-system-defaults-thresholds-blinky-thresholds-gen", 
        "importStatement": "from a.sys.mng.alarm.tech_system_alarms.tech.system.alarms.system_defaults.thresholds.blinky_thresholds_gen import BlinkyThresholds", 

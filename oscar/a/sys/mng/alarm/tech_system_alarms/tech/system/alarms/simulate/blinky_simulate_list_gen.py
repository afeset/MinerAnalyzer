...
class BlinkySimulateList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkySimulateList(logger)
    ...
    def __str__ (self):
        ret="{BlinkySimulateList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkysimulatelist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.mng.alarm.tech_system_alarms.tech.system.alarms.simulate.blinky_simulate_gen import BlinkySimulate', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkySimulate.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkysimulatelist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.mng.alarm.tech_system_alarms.tech.system.alarms.simulate.blinky_simulate_gen import BlinkySimulate', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-simulate-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-simulate-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-simulate").debug1Func(): logFunc("findBlinkyNode(): handling BlinkySimulateList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-simulate").debug1Func(): logFunc("findBlinkyNode(): getting BlinkySimulate. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkySimulate(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-simulate-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkySimulate not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-simulate").debug1Func(): logFunc("findBlinkyNode(): querying BlinkySimulate. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkySimulate(self, key):
        for logFunc in self._log("get-blinky-blinky-simulate").debug1Func(): logFunc("getBlinkySimulate(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkySimulate(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-simulate-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkySimulate-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkySimulate(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkySimulate", 
        "containerLogGroupName": "blinky-simulate", 
        "namespace": "simulate", 
        "containerImportStatement": "from a.sys.mng.alarm.tech_system_alarms.tech.system.alarms.simulate.blinky_simulate_gen import BlinkySimulate", 
        "logGroupName": "blinky-simulate-list", 
        "className": "BlinkySimulateList", 
        "logModuleName": "a-sys-mng-alarm-tech-system-alarms-tech-system-alarms-simulate-blinky-simulate-list-gen", 
        "keyLeaf": {
            "varName": "simulate", 

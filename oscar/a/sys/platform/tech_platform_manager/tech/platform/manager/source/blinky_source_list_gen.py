...
from a.sys.platform.tech_platform_manager.tech.platform.manager.source.blinky_source_gen import BlinkySource

class BlinkySourceList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkySourceList(logger)
    ...
    def __str__ (self):
        ret="{BlinkySourceList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkysourcelist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.platform.tech_platform_manager.tech.platform.manager.source.blinky_source_gen import BlinkySource', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkySource.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkysourcelist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.platform.tech_platform_manager.tech.platform.manager.source.blinky_source_gen import BlinkySource', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-source-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-source-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-source-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-source").debug1Func(): logFunc("findBlinkyNode(): handling BlinkySourceList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-source").debug1Func(): logFunc("findBlinkyNode(): getting BlinkySource. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkySource(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-source-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkySource not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-source").debug1Func(): logFunc("findBlinkyNode(): querying BlinkySource. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkySource(self, key):
        for logFunc in self._log("get-blinky-blinky-source").debug1Func(): logFunc("getBlinkySource(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkySource(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkySource(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-source-moved-failed").errorFunc(): logFunc("getBlinkySource(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkySource(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-source-after-failed").errorFunc(): logFunc("getBlinkySource(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-source-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkySource-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkySource(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkySource", 
        "containerLogGroupName": "blinky-source", 
        "namespace": "source", 
        "containerImportStatement": "from a.sys.platform.tech_platform_manager.tech.platform.manager.source.blinky_source_gen import BlinkySource", 
        "logGroupName": "blinky-source-list", 
        "className": "BlinkySourceList", 
        "logModuleName": "a-sys-platform-tech-platform-manager-tech-platform-manager-source-blinky-source-list-gen", 
        "keyLeaf": {
            "varName": "source", 

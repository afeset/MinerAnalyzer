...
from a.api.yang.modules.tech.common.qwilt_tech_platform_manager.tech.platform.manager.system_defaults.thresholds.thresholds_data_gen import ThresholdsData
...
class BlinkyThresholds(BlinkyContainer):
    ...
    ourXmlTagPollLatencyError="poll-latency-error"
    
    ourXmlTagOverallPollDurationWarning="overall-poll-duration-warning"
    
    ourXmlTagPollLatencyWarning="poll-latency-warning"
    
    ourXmlTagSinglePollDurationWarning="single-poll-duration-warning"
    
    ourXmlTagSinglePollDurationError="single-poll-duration-error"
    
    ourXmlTagOverallPollDurationError="overall-poll-duration-error"
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagPollLatencyError)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.pollLatencyError = newValue.asInt64()
                self.myCandidateData.setHasPollLatencyError()
                
            else:
                self.setPollLatencyErrorDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagOverallPollDurationWarning)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.overallPollDurationWarning = newValue.asInt64()
                self.myCandidateData.setHasOverallPollDurationWarning()
                
            else:
                self.setOverallPollDurationWarningDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagPollLatencyWarning)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.pollLatencyWarning = newValue.asInt64()
                self.myCandidateData.setHasPollLatencyWarning()
                
            else:
                self.setPollLatencyWarningDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagSinglePollDurationWarning)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.singlePollDurationWarning = newValue.asInt64()
                self.myCandidateData.setHasSinglePollDurationWarning()
                
            else:
                self.setSinglePollDurationWarningDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagSinglePollDurationError)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.singlePollDurationError = newValue.asInt64()
                self.myCandidateData.setHasSinglePollDurationError()
                
            else:
                self.setSinglePollDurationErrorDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagOverallPollDurationError)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.overallPollDurationError = newValue.asInt64()
                self.myCandidateData.setHasOverallPollDurationError()
                
            else:
                self.setOverallPollDurationErrorDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkythresholds').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.platform.tech_platform_manager.tech.platform.manager.system_defaults.thresholds.blinky_thresholds_gen import BlinkyThresholds', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagPollLatencyError)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagOverallPollDurationWarning)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagPollLatencyWarning)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagSinglePollDurationWarning)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagSinglePollDurationError)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagOverallPollDurationError)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkythresholds-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.platform.tech_platform_manager.tech.platform.manager.system_defaults.thresholds.blinky_thresholds_gen import BlinkyThresholds', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        ...
        res = self.setPollLatencyErrorDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-polllatencyerror-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setPollLatencyErrorDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setOverallPollDurationWarningDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-overallpolldurationwarning-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setOverallPollDurationWarningDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setPollLatencyWarningDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-polllatencywarning-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setPollLatencyWarningDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setSinglePollDurationWarningDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-singlepolldurationwarning-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setSinglePollDurationWarningDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setSinglePollDurationErrorDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-singlepolldurationerror-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setSinglePollDurationErrorDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setOverallPollDurationErrorDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-overallpolldurationerror-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setOverallPollDurationErrorDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setPollLatencyErrorDefaultValue (self, setHas):
        for logFunc in self._log("set-polllatencyerror-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.pollLatencyError = 0
    ...
    def setOverallPollDurationWarningDefaultValue (self, setHas):
        for logFunc in self._log("set-overallpolldurationwarning-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.overallPollDurationWarning = 0
    ...
    def setPollLatencyWarningDefaultValue (self, setHas):
        for logFunc in self._log("set-polllatencywarning-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.pollLatencyWarning = 0
    ...
    def setSinglePollDurationWarningDefaultValue (self, setHas):
        for logFunc in self._log("set-singlepolldurationwarning-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.singlePollDurationWarning = 0
    ...
    def setSinglePollDurationErrorDefaultValue (self, setHas):
        for logFunc in self._log("set-singlepolldurationerror-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.singlePollDurationError = 0
    ...
    def setOverallPollDurationErrorDefaultValue (self, setHas):
        for logFunc in self._log("set-overallpolldurationerror-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.overallPollDurationError = 0
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_manager.tech.platform.manager.system_defaults.thresholds.thresholds_data_gen import ThresholdsData", 
        ...
        "logModuleName": "a-sys-platform-tech-platform-manager-tech-platform-manager-system-defaults-thresholds-blinky-thresholds-gen", 
        "importStatement": "from a.sys.platform.tech_platform_manager.tech.platform.manager.system_defaults.thresholds.blinky_thresholds_gen import BlinkyThresholds", 

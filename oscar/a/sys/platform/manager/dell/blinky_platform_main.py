...
import a.sys.platform.manager.dell.temperature
...
import a.sys.platform.manager.dell.temperature_source
...
import a.sys.platform.manager.blinky.temperature_blinky_adaptor

# Bypass for PyChecker
if  __package__ is None:
    G_MODULE_NAME_PLATFORM_MAIN            = "unknown"
    G_GROUP_NAME_BLINKY_PLATFORM_MAIN         = "unknown"
else:
    from . import G_MODULE_NAME_PLATFORM_MAIN                  
    from . import G_GROUP_NAME_BLINKY_PLATFORM_MAIN        



class PlatformMainError(Exception):
    """ Error thrown by the platform main code
    """
...
class BlinkyPlatformMain:
    """ Creates the platform manager (including all sources and sub-systems) for Dell platforms, and attaches it all to Blinky
    """
    def __init__ (self, logger):
        self._log = logger.createLogger(G_MODULE_NAME_PLATFORM_MAIN, G_GROUP_NAME_BLINKY_PLATFORM_MAIN)
...
#        self._platformSpecificInitializer = None
    ...
    def initPlatformManagedComponenets (self):        
        ...
        self._temperatureReporter = a.sys.platform.manager.dell.temperature_source.TemperatureSource(self._log)
        ...
        self._platformManager.addReporter(self._temperatureReporter)
        ...
        self._platformTemperature = a.sys.platform.manager.dell.temperature.TemperatureSubsystem(self._log)
        self._platformTemperature.initTemperatureReporter(self._temperatureReporter)       

        self._platformManager.addManagedElement(self._platformFans)
        self._platformManager.addManagedElement(self._platformPower)
        self._platformManager.addManagedElement(self._platformTemperature)
    ...
    def initBlinkyAgent (self, blinkyAgent):
        ...
        self._platformTemperatureBlinkyAdaptor = a.sys.platform.manager.blinky.temperature_blinky_adaptor.TemperatureBlinkyAdaptor(self._log, self, self._configDomain, self._operDomain, self._maapiDomain)
        ...
        self._logAndRaiseErrorForBadReturnCode(self._platformManagerBlinkyAdaptor.createAndAttachBlinkyPlatform(self._platformManager))
        self._logAndRaiseErrorForBadReturnCode(self._platformFansBlinkyAdaptor.createAndAttachBlinkyFans(self._platformFans))
        self._logAndRaiseErrorForBadReturnCode(self._platformPowerBlinkyAdaptor.createAndAttachBlinkyPower(self._platformPower))
        self._logAndRaiseErrorForBadReturnCode(self._platformTemperatureBlinkyAdaptor.createAndAttachBlinkyTemperature(self._platformTemperature))

        self._logAndRaiseErrorForBadReturnCode(self._platformSourceBlinkyAdaptor.createAndAttachBlinkySourceList())
        self._logAndRaiseErrorForBadReturnCode(self._platformSourceBlinkyAdaptor.createAndAttachBlinkySource(self._powerReporter      , "power"))
        self._logAndRaiseErrorForBadReturnCode(self._platformSourceBlinkyAdaptor.createAndAttachBlinkySource(self._fansReporter       , "fans"))
        self._logAndRaiseErrorForBadReturnCode(self._platformSourceBlinkyAdaptor.createAndAttachBlinkySource(self._fruReporter        , "fru"))
        self._logAndRaiseErrorForBadReturnCode(self._platformSourceBlinkyAdaptor.createAndAttachBlinkySource(self._temperatureReporter, "temperature"))
    ...
    def _registerDone (self):
        ...
        self._logAndRaiseErrorForBadReturnCode(self._configDomain.triggerSubscriptions())


    def _logAndRaiseErrorForBadReturnCode (self, rc):
        if rc != ReturnCodes.kOk:
            self._log("log-and-raise-error-for-bad-return-code").error("bad returned code %s", rc)
            raise PlatformMainError("Failed, received bad return code")
...
#################################################################
    ...
    def getTemperatureSubsystem (self):
        return self._platformTemperature

...
from a.sys.platform.tech_platform_power.tech.platform.power.power_supply.blinky_power_supply_gen import BlinkyPowerSupply

class BlinkyPowerSupplyList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyPowerSupplyList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyPowerSupplyList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkypowersupplylist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.platform.tech_platform_power.tech.platform.power.power_supply.blinky_power_supply_gen import BlinkyPowerSupply', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyPowerSupply.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkypowersupplylist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.platform.tech_platform_power.tech.platform.power.power_supply.blinky_power_supply_gen import BlinkyPowerSupply', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-power-supply-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-power-supply-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-power-supply-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-power-supply").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyPowerSupplyList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-power-supply").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyPowerSupply. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyPowerSupply(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-power-supply-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyPowerSupply not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-power-supply").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyPowerSupply. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyPowerSupply(self, key):
        for logFunc in self._log("get-blinky-blinky-power-supply").debug1Func(): logFunc("getBlinkyPowerSupply(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyPowerSupply(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkyPowerSupply(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-power-supply-moved-failed").errorFunc(): logFunc("getBlinkyPowerSupply(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkyPowerSupply(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-power-supply-after-failed").errorFunc(): logFunc("getBlinkyPowerSupply(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-power-supply-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyPowerSupply-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyPowerSupply(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyPowerSupply", 
        "containerLogGroupName": "blinky-power-supply", 
        "namespace": "power_supply", 
        "containerImportStatement": "from a.sys.platform.tech_platform_power.tech.platform.power.power_supply.blinky_power_supply_gen import BlinkyPowerSupply", 
        "logGroupName": "blinky-power-supply-list", 
        "className": "BlinkyPowerSupplyList", 
        "logModuleName": "a-sys-platform-tech-platform-power-tech-platform-power-power-supply-blinky-power-supply-list-gen", 
        "keyLeaf": {
            "varName": "powerSupply", 

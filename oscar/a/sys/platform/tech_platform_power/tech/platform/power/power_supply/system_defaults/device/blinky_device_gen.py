...
from a.api.yang.modules.tech.common.qwilt_tech_platform_power.tech.platform.power.power_supply.system_defaults.device.device_data_gen import DeviceData
...
class BlinkyDevice(BlinkyContainer):
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagFruId)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.fruId = newValue.asString()
                self.myCandidateData.setHasFruId()
                
            else:
                self.setFruIdDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkydevice').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.platform.tech_platform_power.tech.platform.power.power_supply.system_defaults.device.blinky_device_gen import BlinkyDevice', keyPath, keyDepth)
        ...
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagFruId)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkydevice-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.platform.tech_platform_power.tech.platform.power.power_supply.system_defaults.device.blinky_device_gen import BlinkyDevice', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        ...
        res = self.setFruIdDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-fruid-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setFruIdDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
    ...
    def setFruIdDefaultValue (self, setHas):
        for logFunc in self._log("set-fruid-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.fruId = ""
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_power.tech.platform.power.power_supply.system_defaults.device.device_data_gen import DeviceData", 
        ...
        "logModuleName": "a-sys-platform-tech-platform-power-tech-platform-power-power-supply-system-defaults-device-blinky-device-gen", 
        "importStatement": "from a.sys.platform.tech_platform_power.tech.platform.power.power_supply.system_defaults.device.blinky_device_gen import BlinkyDevice", 

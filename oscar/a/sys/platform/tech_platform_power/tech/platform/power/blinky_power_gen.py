...
from a.api.yang.modules.tech.common.qwilt_tech_platform_power.tech.platform.power.power_data_gen import PowerData


from a.sys.platform.tech_platform_power.tech.platform.power.power_supply.blinky_power_supply_list_gen import BlinkyPowerSupplyList

from a.sys.platform.tech_platform_power.tech.platform.power.simulation.blinky_simulation_gen import BlinkySimulation

from a.sys.platform.tech_platform_power.tech.platform.power.system_defaults.blinky_system_defaults_gen import BlinkySystemDefaults




class BlinkyPower(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-power"
    #leaves
    ...
    ourXmlTagPowerSupplyList="power-supply"
    ...
    POWERSUPPLYLIST_CREATE_FUNCTOR = 'POWERSUPPLYLIST_CREATE_FUNCTOR'
    POWERSUPPLYLIST_DELETE_FUNCTOR = 'POWERSUPPLYLIST_DELETE_FUNCTOR'
    ...
    def __init__ (self, logger):
        ...
        self.myCreatePowerSupplyListFunctor=None
        self.myDeletePowerSupplyListFunctor=None
        self.myPowerSupplyList=None
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkypower').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyPower._validationPointId, BlinkyPower._actionPointId)
    ...
    def setCreatePowerSupplyListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-create-power-supply-functor-active").errorFunc():
                logFunc("setCreatePowerSupplyListFunctor() is illegal when blinky node is active")
        self.myCreatePowerSupplyListFunctor = functor

    def setDeletePowerSupplyListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-delete-power-supply-functor-active").errorFunc():
                logFunc("setDeletePowerSupplyListFunctor() is illegal when blinky node is active")
        self.myDeletePowerSupplyListFunctor = functor
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if (keyPath.isTagEqual(keyDepth_PBR.value(), self.ourNamespace, self.ourXmlTagPowerSupplyList)):
            return self.myPowerSupplyList
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkypower').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.platform.tech_platform_power.tech.platform.power.blinky_power_gen import BlinkyPower', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        if not found:
            if keyPath.getLen() > keyDepth+1:
                if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagPowerSupplyList)):
                    keyPathRegistered = BlinkyPowerSupplyList.isKeyPathRegistered(logger, keyPath, keyDepth+1)
                    found = True
            else:
        ...
        logger('is-key-path-registered-blinkypower-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.platform.tech_platform_power.tech.platform.power.blinky_power_gen import BlinkyPower', keyPathRegistered, keyPath, keyDepth)
    ...
    def prepareMyBlinkyNode(self, keyPath, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagPowerSupplyList)):
            if (self.myPowerSupplyList):
                for logFunc in self._log("prepare-my-blinky-node-powersupplylist-already-exists").errorFunc():
                    logFunc("prepareMyBlinkyNode(): already exists, keyPath=%s, keyDepth=%s", keyPath, keyDepth)
                return None
            self.myPowerSupplyList = BlinkyPowerSupplyList(self._log)
            self.myPowerSupplyList.setParent(self)
            self.myPowerSupplyList.setKeyPath(keyPath)
            self.myPowerSupplyList.setDomain(self.myDomain)
    ...
    def handleTrxElementCreate(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagPowerSupplyList)):
                if (self.myCreatePowerSupplyListFunctor):
                    if (not self.myPowerSupplyList):
                        for logFunc in self._log("handle-trx-element-create-powersupplylist-not-exist").debug3Func():
                            logFunc("handleTrxElementCreate(): powersupplylist not exists, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    
                    timeoutGuard = TimeoutGuard(self._log, '%s-create-powersupplylist-functor-%s' % (self.myKeyPath, phase), 
                                                self.getFunctorTimeoutForPhase(self.POWERSUPPLYLIST_CREATE_FUNCTOR, phase),
                                                self.getFunctorMildTimeoutForPhase(self.POWERSUPPLYLIST_CREATE_FUNCTOR, phase))
                    try:
                        res = self.myCreatePowerSupplyListFunctor(phase, self.myPowerSupplyList)
                    except:
                        for logFunc in self._log("handle-trx-element-create-powersupplylist-functor-exception").exceptionFunc():
                            logFunc("PowerSupplyList's create functor raised an exception. trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        raise
                    timeoutGuard.checkAndLog("application problem: functor=%s" % self.myCreatePowerSupplyListFunctor.__name__)
                    if (res != ReturnCodes.kOk):
                        if (phase.getConfdPhase() == TrxPhase.kPrepare):
                            for logFunc in self._log("handle-trx-element-create-powersupplylist-functor-failed-prepare").noticeFunc():
                                logFunc("handleTrxElementCreate(): powersupplylist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                            self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        else:
                            for logFunc in self._log("handle-trx-element-create-powersupplylist-functor-failed").errorFunc():
                                logFunc("handleTrxElementCreate(): powersupplylist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    ...
    def notifyDescendantsPrepareBlinkyDelete(self):
        ...
        if (self.myPowerSupplyList):
            self.myPowerSupplyList.notifyDescendantsPrepareBlinkyDelete()
    ...
    def handleTrxElementDelete(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagPowerSupplyList)):
                res = self.activateDeletePowerSupplyListFunctor(phase)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("handle-trx-element-delete-powersupplylist-functor-failed-prepare").noticeFunc():
                            logFunc("handleTrxElementDelete(): powersupplylist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("handle-trx-element-delete-powersupplylist-functor-failed").errorFunc():
                            logFunc("handleTrxElementDelete(): powersupplylist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                    return ReturnCodes.kGeneralError
    ...
    def removePowerSupplyList(self):
        self.myPowerSupplyList = None

    def activateDeletePowerSupplyListFunctor(self, phase):
        for logFunc in self._log("activate-delete-powersupplylist-functor").debug3Func(): logFunc("activateDeletePowerSupplyListFunctor(): called, phase=%s", phase)
        if (self.myIsActive):
            if (self.myDeletePowerSupplyListFunctor):
                timeoutGuard = TimeoutGuard(self._log, '%s-delete-powersupplylist-functor-%s' % (self.myKeyPath, phase), 
                                            self.getFunctorTimeoutForPhase(self.POWERSUPPLYLIST_DELETE_FUNCTOR, phase), 
                                            self.getFunctorMildTimeoutForPhase(self.POWERSUPPLYLIST_DELETE_FUNCTOR, phase))
                try:
                    res = self.myDeletePowerSupplyListFunctor(phase)
                except:
                    for logFunc in self._log("activate-delete-powersupplylist-functor-exception").exceptionFunc():
                        logFunc("PowerSupplyList's delete functor raised an exception. phase=%s", phase)
                    raise
                timeoutGuard.checkAndLog("application problem: functor=%s" % self.myDeletePowerSupplyListFunctor.__name__)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("activate-delete-powersupplylist-functor-functor-failed-prepare").noticeFunc():
                            logFunc("activateDeletePowerSupplyListFunctor(): functor failed, phase=%s", phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("activate-delete-powersupplylist-functor-functor-failed").errorFunc():
                            logFunc("activateDeletePowerSupplyListFunctor(): functor failed, phase=%s", phase)
                    return ReturnCodes.kGeneralError
        return ReturnCodes.kOk
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = PowerData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkypower-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkypower-failed").errorFunc():
    ...
    def handleInternalDestroy(self, phase):
        ...
        if (phase.getConfdPhase() == TrxPhase.kPrepare):
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myPowerSupplyList):
                    res = self.myPowerSupplyList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-powersupplylist-recursion-failed-prepare-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): delete-powersupplylist-recursion-failed-prepare-blinky, phase=%s",
                                  phase)
                        return ReturnCodes.kGeneralError
                ...
                if (self.myPowerSupplyList):
                    res = self.activateDeletePowerSupplyListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-powersupplylist-functor-failed-prepare").noticeFunc():
                            logFunc("handleInternalDestroy(): activateDeletePowerSupplyListFunctor() failed, res=%s, phase=%s",
                                   res, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        return ReturnCodes.kGeneralError
                if (self.myPowerSupplyList):
                    res = self.myPowerSupplyList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-powersupplylist-recursion-failed-prepare-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myPowerSupplyList.handleInternalDestroy() failed, res=%s, phase=%s",
                                   res, phase)
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myPowerSupplyList):
                    res = self.myPowerSupplyList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-powersupplylist-recursion-failed-commit-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myPowerSupplyList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("handle internal destroy failed in commit phase (blinky)")
                        return ReturnCodes.kGeneralError
                    self.removePowerSupplyList()
                ...
                if (self.myPowerSupplyList):
                    res = self.myPowerSupplyList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-powersupplylist-recursion-failed-commit-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myPowerSupplyList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                    ...
                    res = self.activateDeletePowerSupplyListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-powersupplylist-functor-failed-commit").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeletePowerSupplyListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("activateDeletePowerSupplyListFunctor failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myPowerSupplyList):
                    res = self.myPowerSupplyList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-powersupplylist-recursion-failed-abort-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myPowerSupplyList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myPowerSupplyList):
                    res = self.myPowerSupplyList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-powersupplylist-recursion-failed-abort-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myPowerSupplyList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myPowerSupplyList):
                    res = self.activateDeletePowerSupplyListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-powersupplylist-functor-failed-abort").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeletePowerSupplyListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_power.tech.platform.power.power_data_gen import PowerData", 
        "moduleYangNamespacePrefix": "qt-pltf-pwr", 
        "validationPoint": null, 
        "yangName": "power", 
        "namespace": "power", 
        "logGroupName": "blinky-power", 
        "className": "BlinkyPower", 
        "logModuleName": "a-sys-platform-tech-platform-power-tech-platform-power-blinky-power-gen", 
        "importStatement": "from a.sys.platform.tech_platform_power.tech.platform.power.blinky_power_gen import BlinkyPower", 
    ...
    "descendants": [
        {
            "className": "BlinkyPowerSupplyList", 
            "memberName": "PowerSupplyList", 
            "yangName": "power-supply", 
            "importStatement": "from a.sys.platform.tech_platform_power.tech.platform.power.power_supply.blinky_power_supply_list_gen import BlinkyPowerSupplyList"
        }, 
        {
            "className": "BlinkySimulation", 
            "memberName": "Simulation", 
            "yangName": "simulation", 
            "importStatement": "from a.sys.platform.tech_platform_power.tech.platform.power.simulation.blinky_simulation_gen import BlinkySimulation"
        }, 
        {
            "className": "BlinkySystemDefaults", 
            "memberName": "SystemDefaults", 
            "yangName": "system-defaults", 
            "importStatement": "from a.sys.platform.tech_platform_power.tech.platform.power.system_defaults.blinky_system_defaults_gen import BlinkySystemDefaults"
        }
    ], 

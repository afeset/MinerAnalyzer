...
from a.api.yang.modules.tech.common.qwilt_tech_platform_power.tech.platform.power.power_supply.alarm.alarm_oper_data_gen import AlarmOperData
...
class BlinkyOperAlarm(BlinkyOperNode):
    ...
    _kCallpointName = "tech-platform-power-power-supply-alarm-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-platform-power-power-supply-alarm-callpoint"
    ...
    def getOperRelativePath (self, operRelativePath):
        ...
        val = Value()
        val.setXmlTag(("alarm", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-power", "qt-pltf-pwr"))
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-power", "power-supply-failure"):
                    for logFunc in self._log("set-oper-data-requested-fields-powersupplyfailure-requested").debug3Func(): logFunc(
                        "power-supply-failure requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setPowerSupplyFailureRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-power", "power-supply-failure-reason"):
                    for logFunc in self._log("set-oper-data-requested-fields-powersupplyfailurereason-requested").debug3Func(): logFunc(
                        "power-supply-failure-reason requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setPowerSupplyFailureReasonRequested()
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-power", "power-supply-failure"):
            if operData.isPowerSupplyFailureRequested():
                 if operData.hasPowerSupplyFailure():
                     value.setBool(operData.powerSupplyFailure)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-power", "power-supply-failure-reason"):
            if operData.isPowerSupplyFailureReasonRequested():
                 if operData.hasPowerSupplyFailureReason():
                     value.setEnum(operData.powerSupplyFailureReason.getValue())
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_power.tech.platform.power.power_supply.alarm.alarm_oper_data_gen import AlarmOperData", 
        ...
        "logModuleName": "a-sys-platform-tech-platform-power-tech-platform-power-power-supply-alarm-blinky-alarm-oper-gen", 
        "importStatement": "from a.sys.platform.tech_platform_power.tech.platform.power.power_supply.alarm.blinky_alarm_oper_gen import BlinkyOperAlarm", 
        "callpointName": "tech-platform-power-power-supply-alarm-callpoint", 

...
from a.api.yang.modules.tech.common.qwilt_tech_platform_fans.tech.platform.fans.status.status_oper_data_gen import StatusOperData
...
class BlinkyOperStatus(BlinkyOperNode):
    ...
    _kCallpointName = "tech-platform-fans-status-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-platform-fans-status-callpoint"
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "redundancy-status-raw"):
                    for logFunc in self._log("set-oper-data-requested-fields-redundancystatusraw-requested").debug3Func(): logFunc(
                        "redundancy-status-raw requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setRedundancyStatusRawRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "operational-status"):
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "operational-status-reason"):
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "redundancy-status"):
                    for logFunc in self._log("set-oper-data-requested-fields-redundancystatus-requested").debug3Func(): logFunc(
                        "redundancy-status requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setRedundancyStatusRequested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isRedundancyStatusRawRequested() and operData.hasRedundancyStatusRaw():
            val = Value()
            val.setString(operData.redundancyStatusRaw)
            tagValueList.push(("redundancy-status-raw", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans"), val)
        ...
        if operData.isOperationalStatusRequested() and operData.hasOperationalStatus():
            ...
            tagValueList.push(("operational-status", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans"), val)
        ...
        if operData.isOperationalStatusReasonRequested() and operData.hasOperationalStatusReason():
            ...
            tagValueList.push(("operational-status-reason", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans"), val)
        
        if operData.isRedundancyStatusRequested() and operData.hasRedundancyStatus():
            val = Value()
            val.setEnum(operData.redundancyStatus.getValue())
            tagValueList.push(("redundancy-status", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "redundancy-status-raw"):
            if operData.isRedundancyStatusRawRequested():
                 if operData.hasRedundancyStatusRaw():
                     value.setString(operData.redundancyStatusRaw)
                 else:
                     value.setEmpty()
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "redundancy-status"):
            if operData.isRedundancyStatusRequested():
                 if operData.hasRedundancyStatus():
                     value.setEnum(operData.redundancyStatus.getValue())
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_fans.tech.platform.fans.status.status_oper_data_gen import StatusOperData", 
        ...
        "logModuleName": "a-sys-platform-tech-platform-fans-tech-platform-fans-status-blinky-status-oper-gen", 
        "importStatement": "from a.sys.platform.tech_platform_fans.tech.platform.fans.status.blinky_status_oper_gen import BlinkyOperStatus", 
        "callpointName": "tech-platform-fans-status-callpoint", 

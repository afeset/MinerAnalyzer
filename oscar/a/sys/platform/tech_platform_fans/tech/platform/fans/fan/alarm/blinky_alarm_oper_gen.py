...
from a.api.yang.modules.tech.common.qwilt_tech_platform_fans.tech.platform.fans.fan.alarm.alarm_oper_data_gen import AlarmOperData
...
class BlinkyOperAlarm(BlinkyOperNode):
    ...
    _kCallpointName = "tech-platform-fans-fan-alarm-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-platform-fans-fan-alarm-callpoint"
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "fan-failure-reason"):
                    for logFunc in self._log("set-oper-data-requested-fields-fanfailurereason-requested").debug3Func(): logFunc(
                        "fan-failure-reason requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setFanFailureReasonRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "fan-failure"):
                    for logFunc in self._log("set-oper-data-requested-fields-fanfailure-requested").debug3Func(): logFunc(
                        "fan-failure requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setFanFailureRequested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isFanFailureReasonRequested() and operData.hasFanFailureReason():
            val = Value()
            val.setEnum(operData.fanFailureReason.getValue())
            tagValueList.push(("fan-failure-reason", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans"), val)
        
        if operData.isFanFailureRequested() and operData.hasFanFailure():
            val = Value()
            val.setBool(operData.fanFailure)
            tagValueList.push(("fan-failure", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "fan-failure-reason"):
            if operData.isFanFailureReasonRequested():
                 if operData.hasFanFailureReason():
                     value.setEnum(operData.fanFailureReason.getValue())
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "fan-failure"):
            if operData.isFanFailureRequested():
                 if operData.hasFanFailure():
                     value.setBool(operData.fanFailure)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_fans.tech.platform.fans.fan.alarm.alarm_oper_data_gen import AlarmOperData", 
        ...
        "logModuleName": "a-sys-platform-tech-platform-fans-tech-platform-fans-fan-alarm-blinky-alarm-oper-gen", 
        "importStatement": "from a.sys.platform.tech_platform_fans.tech.platform.fans.fan.alarm.blinky_alarm_oper_gen import BlinkyOperAlarm", 
        "callpointName": "tech-platform-fans-fan-alarm-callpoint", 

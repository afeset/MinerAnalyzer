...
from a.sys.platform.tech_platform_fans.tech.platform.fans.fan.blinky_fan_gen import BlinkyFan

class BlinkyFanList(BlinkyList):
    ...
    def __str__ (self):
        ret="{BlinkyFanList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyfanlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.platform.tech_platform_fans.tech.platform.fans.fan.blinky_fan_gen import BlinkyFan', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyFan.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyfanlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.platform.tech_platform_fans.tech.platform.fans.fan.blinky_fan_gen import BlinkyFan', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-fan-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-fan-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-fan-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-fan").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyFanList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-fan").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyFan. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyFan(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-fan-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyFan not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-fan").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyFan. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyFan(self, key):
        for logFunc in self._log("get-blinky-blinky-fan").debug1Func(): logFunc("getBlinkyFan(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyFan(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkyFan(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-fan-moved-failed").errorFunc(): logFunc("getBlinkyFan(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkyFan(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-fan-after-failed").errorFunc(): logFunc("getBlinkyFan(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-fan-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyFan-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyFan(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyFan", 
        "containerLogGroupName": "blinky-fan", 
        "namespace": "fan", 
        "containerImportStatement": "from a.sys.platform.tech_platform_fans.tech.platform.fans.fan.blinky_fan_gen import BlinkyFan", 
        "logGroupName": "blinky-fan-list", 
        "className": "BlinkyFanList", 
        "logModuleName": "a-sys-platform-tech-platform-fans-tech-platform-fans-fan-blinky-fan-list-gen", 
        "keyLeaf": {
            "varName": "fan", 

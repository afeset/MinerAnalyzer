...
from a.api.yang.modules.tech.common.qwilt_tech_platform_fans.tech.platform.fans.alarm.alarm_oper_data_gen import AlarmOperData
...
class BlinkyOperAlarm(BlinkyOperNode):
    ...
    _kCallpointName = "tech-platform-fans-alarm-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-platform-fans-alarm-callpoint"
    ...
    def getOperRelativePath (self, operRelativePath):
        ...
        val = Value()
        val.setXmlTag(("alarm", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "qt-pltf-fans"))
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "no-redundancy-reason"):
                    for logFunc in self._log("set-oper-data-requested-fields-noredundancyreason-requested").debug3Func(): logFunc(
                        "no-redundancy-reason requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setNoRedundancyReasonRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "no-redundancy"):
                    for logFunc in self._log("set-oper-data-requested-fields-noredundancy-requested").debug3Func(): logFunc(
                        "no-redundancy requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setNoRedundancyRequested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isNoRedundancyReasonRequested() and operData.hasNoRedundancyReason():
            val = Value()
            val.setEnum(operData.noRedundancyReason.getValue())
            tagValueList.push(("no-redundancy-reason", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans"), val)
        
        if operData.isNoRedundancyRequested() and operData.hasNoRedundancy():
            val = Value()
            val.setBool(operData.noRedundancy)
            tagValueList.push(("no-redundancy", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "no-redundancy-reason"):
            if operData.isNoRedundancyReasonRequested():
                 if operData.hasNoRedundancyReason():
                     value.setEnum(operData.noRedundancyReason.getValue())
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-fans", "no-redundancy"):
            if operData.isNoRedundancyRequested():
                 if operData.hasNoRedundancy():
                     value.setBool(operData.noRedundancy)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_fans.tech.platform.fans.alarm.alarm_oper_data_gen import AlarmOperData", 
        ...
        "logModuleName": "a-sys-platform-tech-platform-fans-tech-platform-fans-alarm-blinky-alarm-oper-gen", 
        "importStatement": "from a.sys.platform.tech_platform_fans.tech.platform.fans.alarm.blinky_alarm_oper_gen import BlinkyOperAlarm", 
        "callpointName": "tech-platform-fans-alarm-callpoint", 

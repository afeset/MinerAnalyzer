...
from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.sensor.device.status.status_oper_data_gen import StatusOperData
...
class BlinkyOperStatus(BlinkyOperNode):
    ...
    _kCallpointName = "tech-platform-temperature-sensor-device-status-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-platform-temperature-sensor-device-status-callpoint"
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "status"):
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "index"):
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "temperature-raw"):
                    for logFunc in self._log("set-oper-data-requested-fields-temperatureraw-requested").debug3Func(): logFunc(
                        "temperature-raw requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setTemperatureRawRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "maximum-critical-raw"):
                    for logFunc in self._log("set-oper-data-requested-fields-maximumcriticalraw-requested").debug3Func(): logFunc(
                        "maximum-critical-raw requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setMaximumCriticalRawRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "maximum-warning-raw"):
                    for logFunc in self._log("set-oper-data-requested-fields-maximumwarningraw-requested").debug3Func(): logFunc(
                        "maximum-warning-raw requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setMaximumWarningRawRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "minimum-warning-raw"):
                    for logFunc in self._log("set-oper-data-requested-fields-minimumwarningraw-requested").debug3Func(): logFunc(
                        "minimum-warning-raw requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setMinimumWarningRawRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "minimum-critical-raw"):
                    for logFunc in self._log("set-oper-data-requested-fields-minimumcriticalraw-requested").debug3Func(): logFunc(
                        "minimum-critical-raw requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setMinimumCriticalRawRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "probe-name"):
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "status-raw"):
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isStatusRequested() and operData.hasStatus():
            ...
            tagValueList.push(("status", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        
        if operData.isIndexRequested() and operData.hasIndex():
            val = Value()
            val.setString(operData.index)
            tagValueList.push(("index", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        
        if operData.isTemperatureRawRequested() and operData.hasTemperatureRaw():
            val = Value()
            val.setString(operData.temperatureRaw)
            tagValueList.push(("temperature-raw", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        
        if operData.isMaximumCriticalRawRequested() and operData.hasMaximumCriticalRaw():
            val = Value()
            val.setString(operData.maximumCriticalRaw)
            tagValueList.push(("maximum-critical-raw", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        
        if operData.isMaximumWarningRawRequested() and operData.hasMaximumWarningRaw():
            val = Value()
            val.setString(operData.maximumWarningRaw)
            tagValueList.push(("maximum-warning-raw", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        
        if operData.isMinimumWarningRawRequested() and operData.hasMinimumWarningRaw():
            val = Value()
            val.setString(operData.minimumWarningRaw)
            tagValueList.push(("minimum-warning-raw", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        
        if operData.isMinimumCriticalRawRequested() and operData.hasMinimumCriticalRaw():
            val = Value()
            val.setString(operData.minimumCriticalRaw)
            tagValueList.push(("minimum-critical-raw", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        
        if operData.isProbeNameRequested() and operData.hasProbeName():
            val = Value()
            val.setString(operData.probeName)
            tagValueList.push(("probe-name", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        
        if operData.isStatusRawRequested() and operData.hasStatusRaw():
            val = Value()
            val.setString(operData.statusRaw)
            tagValueList.push(("status-raw", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "temperature-raw"):
            if operData.isTemperatureRawRequested():
                 if operData.hasTemperatureRaw():
                     value.setString(operData.temperatureRaw)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "maximum-critical-raw"):
            if operData.isMaximumCriticalRawRequested():
                 if operData.hasMaximumCriticalRaw():
                     value.setString(operData.maximumCriticalRaw)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "maximum-warning-raw"):
            if operData.isMaximumWarningRawRequested():
                 if operData.hasMaximumWarningRaw():
                     value.setString(operData.maximumWarningRaw)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "minimum-warning-raw"):
            if operData.isMinimumWarningRawRequested():
                 if operData.hasMinimumWarningRaw():
                     value.setString(operData.minimumWarningRaw)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "minimum-critical-raw"):
            if operData.isMinimumCriticalRawRequested():
                 if operData.hasMinimumCriticalRaw():
                     value.setString(operData.minimumCriticalRaw)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.sensor.device.status.status_oper_data_gen import StatusOperData", 
        ...
        "logModuleName": "a-sys-platform-tech-platform-temperature-tech-platform-temperature-sensor-device-status-blinky-status-oper-gen", 
        "importStatement": "from a.sys.platform.tech_platform_temperature.tech.platform.temperature.sensor.device.status.blinky_status_oper_gen import BlinkyOperStatus", 
        "callpointName": "tech-platform-temperature-sensor-device-status-callpoint", 

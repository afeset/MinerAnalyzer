...
from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.sensor.alarm.alarm_oper_data_gen import AlarmOperData
...
class BlinkyOperAlarm(BlinkyOperNode):
    ...
    _kCallpointName = "tech-platform-temperature-sensor-alarm-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-platform-temperature-sensor-alarm-callpoint"
    ...
    def getOperRelativePath (self, operRelativePath):
        ...
        val = Value()
        val.setXmlTag(("alarm", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "qt-pltf-temperature"))
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "temperature-warning-reason"):
                    for logFunc in self._log("set-oper-data-requested-fields-temperaturewarningreason-requested").debug3Func(): logFunc(
                        "temperature-warning-reason requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setTemperatureWarningReasonRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "temperature-critical-reason"):
                    for logFunc in self._log("set-oper-data-requested-fields-temperaturecriticalreason-requested").debug3Func(): logFunc(
                        "temperature-critical-reason requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setTemperatureCriticalReasonRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "temperature-critical"):
                    for logFunc in self._log("set-oper-data-requested-fields-temperaturecritical-requested").debug3Func(): logFunc(
                        "temperature-critical requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setTemperatureCriticalRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "temperature-warning"):
                    for logFunc in self._log("set-oper-data-requested-fields-temperaturewarning-requested").debug3Func(): logFunc(
                        "temperature-warning requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setTemperatureWarningRequested()
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "temperature-warning-reason"):
            if operData.isTemperatureWarningReasonRequested():
                 if operData.hasTemperatureWarningReason():
                     value.setEnum(operData.temperatureWarningReason.getValue())
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "temperature-critical-reason"):
            if operData.isTemperatureCriticalReasonRequested():
                 if operData.hasTemperatureCriticalReason():
                     value.setEnum(operData.temperatureCriticalReason.getValue())
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "temperature-critical"):
            if operData.isTemperatureCriticalRequested():
                 if operData.hasTemperatureCritical():
                     value.setBool(operData.temperatureCritical)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "temperature-warning"):
            if operData.isTemperatureWarningRequested():
                 if operData.hasTemperatureWarning():
                     value.setBool(operData.temperatureWarning)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.sensor.alarm.alarm_oper_data_gen import AlarmOperData", 
        ...
        "logModuleName": "a-sys-platform-tech-platform-temperature-tech-platform-temperature-sensor-alarm-blinky-alarm-oper-gen", 
        "importStatement": "from a.sys.platform.tech_platform_temperature.tech.platform.temperature.sensor.alarm.blinky_alarm_oper_gen import BlinkyOperAlarm", 
        "callpointName": "tech-platform-temperature-sensor-alarm-callpoint", 

...
from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.temperature_data_gen import TemperatureData


from a.sys.platform.tech_platform_temperature.tech.platform.temperature.simulation.blinky_simulation_gen import BlinkySimulation

from a.sys.platform.tech_platform_temperature.tech.platform.temperature.system_defaults.blinky_system_defaults_gen import BlinkySystemDefaults

from a.sys.platform.tech_platform_temperature.tech.platform.temperature.sensor.blinky_sensor_list_gen import BlinkySensorList




class BlinkyTemperature(BlinkyContainer):
    ...
    SENSORLIST_CREATE_FUNCTOR = 'SENSORLIST_CREATE_FUNCTOR'
    SENSORLIST_DELETE_FUNCTOR = 'SENSORLIST_DELETE_FUNCTOR'
    ...
    def __init__ (self, logger):
        ...
        self.myCreateSensorListFunctor=None
        self.myDeleteSensorListFunctor=None
        self.mySensorList=None
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkytemperature').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyTemperature._validationPointId, BlinkyTemperature._actionPointId)
        ...
        newNode=BlinkyTemperature(logger)
    ...
    def setCreateSensorListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-create-sensor-functor-active").errorFunc():
                logFunc("setCreateSensorListFunctor() is illegal when blinky node is active")
        self.myCreateSensorListFunctor = functor

    def setDeleteSensorListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-delete-sensor-functor-active").errorFunc():
                logFunc("setDeleteSensorListFunctor() is illegal when blinky node is active")
        self.myDeleteSensorListFunctor = functor
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if (keyPath.isTagEqual(keyDepth_PBR.value(), self.ourNamespace, self.ourXmlTagSensorList)):
            return self.mySensorList
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkytemperature').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.platform.tech_platform_temperature.tech.platform.temperature.blinky_temperature_gen import BlinkyTemperature', keyPath, keyDepth)
        ...
        if not found:
            if keyPath.getLen() > keyDepth+1:
                if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagSensorList)):
                    keyPathRegistered = BlinkySensorList.isKeyPathRegistered(logger, keyPath, keyDepth+1)
                    found = True
            else:
                keyPathRegistered = True
                found = True
        ...
        logger('is-key-path-registered-blinkytemperature-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.platform.tech_platform_temperature.tech.platform.temperature.blinky_temperature_gen import BlinkyTemperature', keyPathRegistered, keyPath, keyDepth)
    ...
    def prepareMyBlinkyNode(self, keyPath, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagSensorList)):
            if (self.mySensorList):
                for logFunc in self._log("prepare-my-blinky-node-sensorlist-already-exists").errorFunc():
                    logFunc("prepareMyBlinkyNode(): already exists, keyPath=%s, keyDepth=%s", keyPath, keyDepth)
                return None
            self.mySensorList = BlinkySensorList(self._log)
            self.mySensorList.setParent(self)
            self.mySensorList.setKeyPath(keyPath)
            self.mySensorList.setDomain(self.myDomain)
    ...
    def handleTrxElementCreate(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagSensorList)):
                if (self.myCreateSensorListFunctor):
                    if (not self.mySensorList):
                        for logFunc in self._log("handle-trx-element-create-sensorlist-not-exist").debug3Func():
                            logFunc("handleTrxElementCreate(): sensorlist not exists, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    
                    timeoutGuard = TimeoutGuard(self._log, '%s-create-sensorlist-functor-%s' % (self.myKeyPath, phase), 
                                                self.getFunctorTimeoutForPhase(self.SENSORLIST_CREATE_FUNCTOR, phase),
                                                self.getFunctorMildTimeoutForPhase(self.SENSORLIST_CREATE_FUNCTOR, phase))
                    try:
                        res = self.myCreateSensorListFunctor(phase, self.mySensorList)
                    except:
                        for logFunc in self._log("handle-trx-element-create-sensorlist-functor-exception").exceptionFunc():
                            logFunc("SensorList's create functor raised an exception. trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        raise
                    timeoutGuard.checkAndLog("application problem: functor=%s" % self.myCreateSensorListFunctor.__name__)
                    if (res != ReturnCodes.kOk):
                        if (phase.getConfdPhase() == TrxPhase.kPrepare):
                            for logFunc in self._log("handle-trx-element-create-sensorlist-functor-failed-prepare").noticeFunc():
                                logFunc("handleTrxElementCreate(): sensorlist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                            self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        else:
                            for logFunc in self._log("handle-trx-element-create-sensorlist-functor-failed").errorFunc():
                                logFunc("handleTrxElementCreate(): sensorlist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    ...
    def notifyDescendantsPrepareBlinkyDelete(self):
        ...
        if (self.mySensorList):
            self.mySensorList.notifyDescendantsPrepareBlinkyDelete()
    ...
    def handleTrxElementDelete(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagSensorList)):
                res = self.activateDeleteSensorListFunctor(phase)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("handle-trx-element-delete-sensorlist-functor-failed-prepare").noticeFunc():
                            logFunc("handleTrxElementDelete(): sensorlist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("handle-trx-element-delete-sensorlist-functor-failed").errorFunc():
                            logFunc("handleTrxElementDelete(): sensorlist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                    return ReturnCodes.kGeneralError
    ...
    def removeSensorList(self):
        self.mySensorList = None

    def activateDeleteSensorListFunctor(self, phase):
        for logFunc in self._log("activate-delete-sensorlist-functor").debug3Func(): logFunc("activateDeleteSensorListFunctor(): called, phase=%s", phase)
        if (self.myIsActive):
            if (self.myDeleteSensorListFunctor):
                timeoutGuard = TimeoutGuard(self._log, '%s-delete-sensorlist-functor-%s' % (self.myKeyPath, phase), 
                                            self.getFunctorTimeoutForPhase(self.SENSORLIST_DELETE_FUNCTOR, phase), 
                                            self.getFunctorMildTimeoutForPhase(self.SENSORLIST_DELETE_FUNCTOR, phase))
                try:
                    res = self.myDeleteSensorListFunctor(phase)
                except:
                    for logFunc in self._log("activate-delete-sensorlist-functor-exception").exceptionFunc():
                        logFunc("SensorList's delete functor raised an exception. phase=%s", phase)
                    raise
                timeoutGuard.checkAndLog("application problem: functor=%s" % self.myDeleteSensorListFunctor.__name__)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("activate-delete-sensorlist-functor-functor-failed-prepare").noticeFunc():
                            logFunc("activateDeleteSensorListFunctor(): functor failed, phase=%s", phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("activate-delete-sensorlist-functor-functor-failed").errorFunc():
                            logFunc("activateDeleteSensorListFunctor(): functor failed, phase=%s", phase)
                    return ReturnCodes.kGeneralError
        return ReturnCodes.kOk
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = TemperatureData()
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = TemperatureData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkytemperature-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkytemperature-failed").errorFunc():
    ...
    def handleInternalDestroy(self, phase):
        ...
        if (phase.getConfdPhase() == TrxPhase.kPrepare):
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.mySensorList):
                    res = self.mySensorList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-sensorlist-recursion-failed-prepare-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): delete-sensorlist-recursion-failed-prepare-blinky, phase=%s",
                                  phase)
                        return ReturnCodes.kGeneralError
                ...
                if (self.mySensorList):
                    res = self.activateDeleteSensorListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-sensorlist-functor-failed-prepare").noticeFunc():
                            logFunc("handleInternalDestroy(): activateDeleteSensorListFunctor() failed, res=%s, phase=%s",
                                   res, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        return ReturnCodes.kGeneralError
                if (self.mySensorList):
                    res = self.mySensorList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-sensorlist-recursion-failed-prepare-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): mySensorList.handleInternalDestroy() failed, res=%s, phase=%s",
                                   res, phase)
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.mySensorList):
                    res = self.mySensorList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-sensorlist-recursion-failed-commit-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): mySensorList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("handle internal destroy failed in commit phase (blinky)")
                        return ReturnCodes.kGeneralError
                    self.removeSensorList()
                ...
                if (self.mySensorList):
                    res = self.mySensorList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-sensorlist-recursion-failed-commit-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): mySensorList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("handle internal destroy failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
                    res = self.activateDeleteSensorListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-sensorlist-functor-failed-commit").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteSensorListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("activateDeleteSensorListFunctor failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.mySensorList):
                    res = self.mySensorList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-sensorlist-recursion-failed-abort-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): mySensorList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("internal destroy failed in abort (blinky)")
                        return ReturnCodes.kOk
                ...
                if (self.mySensorList):
                    res = self.mySensorList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-sensorlist-recursion-failed-abort-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): mySensorList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("internal destroy failed in abort (private/public)")
                        return ReturnCodes.kOk
                if (self.mySensorList):
                    res = self.activateDeleteSensorListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-sensorlist-functor-failed-abort").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteSensorListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("delete functor failed in abort (blinky)")
                        return ReturnCodes.kOk
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.temperature_data_gen import TemperatureData", 
        "moduleYangNamespacePrefix": "qt-pltf-temperature", 
        "validationPoint": null, 
        "yangName": "temperature", 
        "namespace": "temperature", 
        "logGroupName": "blinky-temperature", 
        "className": "BlinkyTemperature", 
        "logModuleName": "a-sys-platform-tech-platform-temperature-tech-platform-temperature-blinky-temperature-gen", 
        "importStatement": "from a.sys.platform.tech_platform_temperature.tech.platform.temperature.blinky_temperature_gen import BlinkyTemperature", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", 
        "dataClassName": "TemperatureData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {
    ...
    "descendants": [
        {
            ...
            "yangName": "simulation", 
            "importStatement": "from a.sys.platform.tech_platform_temperature.tech.platform.temperature.simulation.blinky_simulation_gen import BlinkySimulation"
        }, 
        {
        ...
        {
            "className": "BlinkySensorList", 
            "memberName": "SensorList", 
            "yangName": "sensor", 
            "importStatement": "from a.sys.platform.tech_platform_temperature.tech.platform.temperature.sensor.blinky_sensor_list_gen import BlinkySensorList"
        }
    ], 

...
from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.sensor.simulation.simulation_data_gen import SimulationData
...
class BlinkySimulation(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"
    #leaves
    
    ourXmlTagForceOperationalStatus="force-operational-status"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkysimulation').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkySimulation._validationPointId, BlinkySimulation._actionPointId)

        confd_key=KeyPath()
        
        
        xmlVal = Value()
        xmlVal.setXmlTag(("simulation", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "qt-pltf-temperature"))
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagForceOperationalStatus)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                if not SensorForceOperationalStatusType.isValidValue(newValue.asEnum()):
                    self._log("trx-element-update-candidate-illegal-enum-forceoperationalstatus-failed")\
                        .error("illegal enum value %s for forceOperationalStatus", newValue.asEnum())
                    return ReturnCodes.kGeneralError
                self.myCandidateData.forceOperationalStatus = SensorForceOperationalStatusType.getByValue(newValue.asEnum())
                self.myCandidateData.setHasForceOperationalStatus()
                
            else:
                self.setForceOperationalStatusDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkysimulation').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.platform.tech_platform_temperature.tech.platform.temperature.sensor.simulation.blinky_simulation_gen import BlinkySimulation', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagForceOperationalStatus)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkysimulation-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.platform.tech_platform_temperature.tech.platform.temperature.sensor.simulation.blinky_simulation_gen import BlinkySimulation', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = SimulationData()
        
        res = self.setForceOperationalStatusDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-forceoperationalstatus-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setForceOperationalStatusDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setForceOperationalStatusDefaultValue (self, setHas):
        for logFunc in self._log("set-forceoperationalstatus-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.forceOperationalStatus = SensorForceOperationalStatusType.kNone
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = SimulationData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkysimulation-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkysimulation-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.sensor.simulation.simulation_data_gen import SimulationData", 
        "moduleYangNamespacePrefix": "qt-pltf-temperature", 
        "validationPoint": null, 
        "yangName": "simulation", 
        "namespace": "simulation", 
        "logGroupName": "blinky-simulation", 
        "className": "BlinkySimulation", 
        "logModuleName": "a-sys-platform-tech-platform-temperature-tech-platform-temperature-sensor-simulation-blinky-simulation-gen", 
        "importStatement": "from a.sys.platform.tech_platform_temperature.tech.platform.temperature.sensor.simulation.blinky_simulation_gen import BlinkySimulation", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", 
        "dataClassName": "SimulationData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

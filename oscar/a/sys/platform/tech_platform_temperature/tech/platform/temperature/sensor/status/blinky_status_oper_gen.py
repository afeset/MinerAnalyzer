...
from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.sensor.status.status_oper_data_gen import StatusOperData
...
class BlinkyOperStatus(BlinkyOperNode):
    ...
    _kCallpointName = "tech-platform-temperature-sensor-status-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-platform-temperature-sensor-status-callpoint"
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "maximum-critical"):
                    for logFunc in self._log("set-oper-data-requested-fields-maximumcritical-requested").debug3Func(): logFunc(
                        "maximum-critical requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setMaximumCriticalRequested()
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "temperature"):
                    for logFunc in self._log("set-oper-data-requested-fields-temperature-requested").debug3Func(): logFunc(
                        "temperature requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setTemperatureRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "minimum-warning"):
                    for logFunc in self._log("set-oper-data-requested-fields-minimumwarning-requested").debug3Func(): logFunc(
                        "minimum-warning requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setMinimumWarningRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "minimum-critical"):
                    for logFunc in self._log("set-oper-data-requested-fields-minimumcritical-requested").debug3Func(): logFunc(
                        "minimum-critical requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setMinimumCriticalRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "location"):
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "maximum-warning"):
                    for logFunc in self._log("set-oper-data-requested-fields-maximumwarning-requested").debug3Func(): logFunc(
                        "maximum-warning requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setMaximumWarningRequested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isMaximumCriticalRequested() and operData.hasMaximumCritical():
            val = Value()
            val.setInt64(operData.maximumCritical)
            tagValueList.push(("maximum-critical", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        ...
        if operData.isTemperatureRequested() and operData.hasTemperature():
            val = Value()
            val.setInt64(operData.temperature)
            tagValueList.push(("temperature", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        
        if operData.isMinimumWarningRequested() and operData.hasMinimumWarning():
            val = Value()
            val.setInt64(operData.minimumWarning)
            tagValueList.push(("minimum-warning", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        
        if operData.isMinimumCriticalRequested() and operData.hasMinimumCritical():
            val = Value()
            val.setInt64(operData.minimumCritical)
            tagValueList.push(("minimum-critical", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        
        if operData.isLocationRequested() and operData.hasLocation():
            val = Value()
            val.setString(operData.location)
            tagValueList.push(("location", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
        
        if operData.isMaximumWarningRequested() and operData.hasMaximumWarning():
            val = Value()
            val.setInt64(operData.maximumWarning)
            tagValueList.push(("maximum-warning", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "maximum-critical"):
            if operData.isMaximumCriticalRequested():
                 if operData.hasMaximumCritical():
                     value.setInt64(operData.maximumCritical)
                 else:
                     value.setEmpty()
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "temperature"):
            if operData.isTemperatureRequested():
                 if operData.hasTemperature():
                     value.setInt64(operData.temperature)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "minimum-warning"):
            if operData.isMinimumWarningRequested():
                 if operData.hasMinimumWarning():
                     value.setInt64(operData.minimumWarning)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "minimum-critical"):
            if operData.isMinimumCriticalRequested():
                 if operData.hasMinimumCritical():
                     value.setInt64(operData.minimumCritical)
                 else:
                     value.setEmpty()
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-platform-temperature", "maximum-warning"):
            if operData.isMaximumWarningRequested():
                 if operData.hasMaximumWarning():
                     value.setInt64(operData.maximumWarning)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_platform_temperature.tech.platform.temperature.sensor.status.status_oper_data_gen import StatusOperData", 
        ...
        "logModuleName": "a-sys-platform-tech-platform-temperature-tech-platform-temperature-sensor-status-blinky-status-oper-gen", 
        "importStatement": "from a.sys.platform.tech_platform_temperature.tech.platform.temperature.sensor.status.blinky_status_oper_gen import BlinkyOperStatus", 
        "callpointName": "tech-platform-temperature-sensor-status-callpoint", 

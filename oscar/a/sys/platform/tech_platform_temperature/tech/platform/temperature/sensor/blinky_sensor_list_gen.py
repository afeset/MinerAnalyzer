...
from a.sys.platform.tech_platform_temperature.tech.platform.temperature.sensor.blinky_sensor_gen import BlinkySensor

class BlinkySensorList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkySensorList(logger)
    ...
    def __str__ (self):
        ret="{BlinkySensorList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkysensorlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.platform.tech_platform_temperature.tech.platform.temperature.sensor.blinky_sensor_gen import BlinkySensor', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkySensor.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkysensorlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.platform.tech_platform_temperature.tech.platform.temperature.sensor.blinky_sensor_gen import BlinkySensor', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-sensor-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-sensor-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-sensor-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-sensor").debug1Func(): logFunc("findBlinkyNode(): handling BlinkySensorList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-sensor").debug1Func(): logFunc("findBlinkyNode(): getting BlinkySensor. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkySensor(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-sensor-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkySensor not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-sensor").debug1Func(): logFunc("findBlinkyNode(): querying BlinkySensor. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkySensor(self, key):
        for logFunc in self._log("get-blinky-blinky-sensor").debug1Func(): logFunc("getBlinkySensor(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkySensor(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkySensor(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-sensor-moved-failed").errorFunc(): logFunc("getBlinkySensor(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkySensor(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-sensor-after-failed").errorFunc(): logFunc("getBlinkySensor(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-sensor-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkySensor-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkySensor(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkySensor", 
        "containerLogGroupName": "blinky-sensor", 
        "namespace": "sensor", 
        "containerImportStatement": "from a.sys.platform.tech_platform_temperature.tech.platform.temperature.sensor.blinky_sensor_gen import BlinkySensor", 
        "logGroupName": "blinky-sensor-list", 
        "className": "BlinkySensorList", 
        "logModuleName": "a-sys-platform-tech-platform-temperature-tech-platform-temperature-sensor-blinky-sensor-list-gen", 
        "keyLeaf": {
            "varName": "sensor", 

...
from a.sys.snmp.tech_snmp_manager.tech.snmp.notifications.destination.blinky_destination_gen import BlinkyDestination

class BlinkyDestinationList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyDestinationList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyDestinationList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkydestinationlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.snmp.tech_snmp_manager.tech.snmp.notifications.destination.blinky_destination_gen import BlinkyDestination', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyDestination.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkydestinationlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.snmp.tech_snmp_manager.tech.snmp.notifications.destination.blinky_destination_gen import BlinkyDestination', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-destination-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-destination-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-destination").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyDestinationList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-destination").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyDestination. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyDestination(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-destination-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyDestination not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-destination").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyDestination. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyDestination(self, key):
        for logFunc in self._log("get-blinky-blinky-destination").debug1Func(): logFunc("getBlinkyDestination(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyDestination(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-destination-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyDestination-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyDestination(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyDestination", 
        "containerLogGroupName": "blinky-destination", 
        "namespace": "destination", 
        "containerImportStatement": "from a.sys.snmp.tech_snmp_manager.tech.snmp.notifications.destination.blinky_destination_gen import BlinkyDestination", 
        "logGroupName": "blinky-destination-list", 
        "className": "BlinkyDestinationList", 
        "logModuleName": "a-sys-snmp-tech-snmp-manager-tech-snmp-notifications-destination-blinky-destination-list-gen", 
        "keyLeaf": {
            "varName": "destination", 

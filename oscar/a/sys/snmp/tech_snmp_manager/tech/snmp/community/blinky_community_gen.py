...
class BlinkyCommunity(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-snmp"
    #leaves
    
    ourXmlTagEntryName="entry-name"
    
    ourXmlTagAccessMode="access-mode"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkycommunity').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyCommunity._validationPointId, BlinkyCommunity._actionPointId)
        ...
        newNode=BlinkyCommunity(logger)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagEntryName)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.entryName = newValue.asString()
                self.myCandidateData.setHasEntryName()
                
            else:
                self.setEntryNameDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagAccessMode)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                if not Accessmodetype.isValidValue(newValue.asEnum()):
                    self._log("trx-element-update-candidate-illegal-enum-accessmode-failed")\
                        .error("illegal enum value %s for accessMode", newValue.asEnum())
                    return ReturnCodes.kGeneralError
                self.myCandidateData.accessMode = Accessmodetype.getByValue(newValue.asEnum())
                self.myCandidateData.setHasAccessMode()
                
            else:
                self.setAccessModeDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkycommunity').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.snmp.tech_snmp_manager.tech.snmp.community.blinky_community_gen import BlinkyCommunity', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagEntryName)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagAccessMode)):
                keyPathRegistered = True
                found = True
        ...
        logger('is-key-path-registered-blinkycommunity-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.snmp.tech_snmp_manager.tech.snmp.community.blinky_community_gen import BlinkyCommunity', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = CommunityData()
        
        res = self.setEntryNameDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-entryname-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setEntryNameDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setAccessModeDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-accessmode-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setAccessModeDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
    ...
    def setEntryNameDefaultValue (self, setHas):
        for logFunc in self._log("set-entryname-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.entryName = ""
    ...
    def setAccessModeDefaultValue (self, setHas):
        for logFunc in self._log("set-accessmode-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.accessMode = Accessmodetype.kReadOnly
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = CommunityData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkycommunity-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkycommunity-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_snmp.tech.snmp.community.community_data_gen import CommunityData", 
        "moduleYangNamespacePrefix": "qt-snmp", 
        "validationPoint": null, 
        "yangName": "community", 
        "namespace": "community", 
        "logGroupName": "blinky-community", 
        "className": "BlinkyCommunity", 
        "logModuleName": "a-sys-snmp-tech-snmp-manager-tech-snmp-community-blinky-community-gen", 
        "importStatement": "from a.sys.snmp.tech_snmp_manager.tech.snmp.community.blinky_community_gen import BlinkyCommunity", 

...
from a.sys.snmp.tech_snmp_manager.tech.snmp.community.blinky_community_gen import BlinkyCommunity

class BlinkyCommunityList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyCommunityList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyCommunityList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkycommunitylist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.snmp.tech_snmp_manager.tech.snmp.community.blinky_community_gen import BlinkyCommunity', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyCommunity.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkycommunitylist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.snmp.tech_snmp_manager.tech.snmp.community.blinky_community_gen import BlinkyCommunity', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-community-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-community-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-community").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyCommunityList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-community").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyCommunity. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyCommunity(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-community-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyCommunity not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-community").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyCommunity. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyCommunity(self, key):
        for logFunc in self._log("get-blinky-blinky-community").debug1Func(): logFunc("getBlinkyCommunity(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyCommunity(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-community-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyCommunity-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyCommunity(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyCommunity", 
        "containerLogGroupName": "blinky-community", 
        "namespace": "community", 
        "containerImportStatement": "from a.sys.snmp.tech_snmp_manager.tech.snmp.community.blinky_community_gen import BlinkyCommunity", 
        "logGroupName": "blinky-community-list", 
        "className": "BlinkyCommunityList", 
        "logModuleName": "a-sys-snmp-tech-snmp-manager-tech-snmp-community-blinky-community-list-gen", 
        "keyLeaf": {
            "varName": "community", 

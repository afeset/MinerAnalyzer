...
from a.api.yang.modules.tech.content.qwilt_tech_content.tech.content.delivery.delivery_data_gen import DeliveryData
...
class BlinkyDelivery(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-content"
    #leaves
    
    ourXmlTagDeliveryInterfaceFailover="delivery-interface-failover"
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagDeliveryInterfaceFailover)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.deliveryInterfaceFailover = newValue.asBool()
                self.myCandidateData.setHasDeliveryInterfaceFailover()
                
            else:
                self.setDeliveryInterfaceFailoverDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkydelivery').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_content_delivery.tech.content.delivery.blinky_delivery_gen import BlinkyDelivery', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagDeliveryInterfaceFailover)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkydelivery-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_content_delivery.tech.content.delivery.blinky_delivery_gen import BlinkyDelivery', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        ...
        res = self.setDeliveryInterfaceFailoverDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-deliveryinterfacefailover-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setDeliveryInterfaceFailoverDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setDeliveryInterfaceFailoverDefaultValue (self, setHas):
        for logFunc in self._log("set-deliveryinterfacefailover-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.deliveryInterfaceFailover = False
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.content.qwilt_tech_content.tech.content.delivery.delivery_data_gen import DeliveryData", 
        ...
        "logModuleName": "a-sys-net-tech-content-delivery-tech-content-delivery-blinky-delivery-gen", 
        "importStatement": "from a.sys.net.tech_content_delivery.tech.content.delivery.blinky_delivery_gen import BlinkyDelivery", 

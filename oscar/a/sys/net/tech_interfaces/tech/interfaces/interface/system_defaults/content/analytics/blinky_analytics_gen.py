...
from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.system_defaults.content.analytics.analytics_data_gen import AnalyticsData





class BlinkyAnalytics(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyanalytics').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyAnalytics._validationPointId, BlinkyAnalytics._actionPointId)

        confd_key=KeyPath()
        
        
        xmlVal = Value()
        xmlVal.setXmlTag(("analytics", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "qt-if"))
        ...
        newNode=BlinkyAnalytics(logger)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyanalytics').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.system_defaults.content.analytics.blinky_analytics_gen import BlinkyAnalytics', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkyanalytics-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.system_defaults.content.analytics.blinky_analytics_gen import BlinkyAnalytics', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = AnalyticsData()
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = AnalyticsData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyanalytics-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyanalytics-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.system_defaults.content.analytics.analytics_data_gen import AnalyticsData", 
        "moduleYangNamespacePrefix": "qt-if", 
        "validationPoint": null, 
        "yangName": "analytics", 
        "namespace": "analytics", 
        "logGroupName": "blinky-analytics", 
        "className": "BlinkyAnalytics", 
        "logModuleName": "a-sys-net-tech-interfaces-tech-interfaces-interface-system-defaults-content-analytics-blinky-analytics-gen", 
        "importStatement": "from a.sys.net.tech_interfaces.tech.interfaces.interface.system_defaults.content.analytics.blinky_analytics_gen import BlinkyAnalytics", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", 
        "dataClassName": "AnalyticsData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

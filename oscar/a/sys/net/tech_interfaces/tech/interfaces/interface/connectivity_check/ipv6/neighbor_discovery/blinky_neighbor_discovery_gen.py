...
from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.connectivity_check.ipv6.neighbor_discovery.neighbor_discovery_data_gen import NeighborDiscoveryData





class BlinkyNeighborDiscovery(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces"
    #leaves
    
    ourXmlTagTestTimeoutMsec="test-timeout-msec"
    
    ourXmlTagTestIntervalMsec="test-interval-msec"
    
    ourXmlTagUpPeriod="up-period"
    
    ourXmlTagDownPeriod="down-period"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyneighbordiscovery').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyNeighborDiscovery._validationPointId, BlinkyNeighborDiscovery._actionPointId)

        confd_key=KeyPath()
        
        
        xmlVal = Value()
        xmlVal.setXmlTag(("neighbor-discovery", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "qt-if"))
        ...
        newNode=BlinkyNeighborDiscovery(logger)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagTestTimeoutMsec)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.testTimeoutMsec = newValue.asInt64()
                self.myCandidateData.setHasTestTimeoutMsec()
                
            else:
                self.setTestTimeoutMsecDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagTestIntervalMsec)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.testIntervalMsec = newValue.asInt64()
                self.myCandidateData.setHasTestIntervalMsec()
                
            else:
                self.setTestIntervalMsecDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagUpPeriod)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.upPeriod = newValue.asInt64()
                self.myCandidateData.setHasUpPeriod()
                
            else:
                self.setUpPeriodDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagDownPeriod)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.downPeriod = newValue.asInt64()
                self.myCandidateData.setHasDownPeriod()
                
            else:
                self.setDownPeriodDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyneighbordiscovery').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.connectivity_check.ipv6.neighbor_discovery.blinky_neighbor_discovery_gen import BlinkyNeighborDiscovery', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagTestTimeoutMsec)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagTestIntervalMsec)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagUpPeriod)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagDownPeriod)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkyneighbordiscovery-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.connectivity_check.ipv6.neighbor_discovery.blinky_neighbor_discovery_gen import BlinkyNeighborDiscovery', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = NeighborDiscoveryData()
        
        res = self.setTestTimeoutMsecDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-testtimeoutmsec-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setTestTimeoutMsecDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setTestIntervalMsecDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-testintervalmsec-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setTestIntervalMsecDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setUpPeriodDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-upperiod-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setUpPeriodDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setDownPeriodDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-downperiod-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setDownPeriodDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setTestTimeoutMsecDefaultValue (self, setHas):
        for logFunc in self._log("set-testtimeoutmsec-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.testTimeoutMsec = 0
    ...
    def setTestIntervalMsecDefaultValue (self, setHas):
        for logFunc in self._log("set-testintervalmsec-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.testIntervalMsec = 0
    ...
    def setUpPeriodDefaultValue (self, setHas):
        for logFunc in self._log("set-upperiod-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.upPeriod = 0
    ...
    def setDownPeriodDefaultValue (self, setHas):
        for logFunc in self._log("set-downperiod-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.downPeriod = 0
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = NeighborDiscoveryData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyneighbordiscovery-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyneighbordiscovery-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.connectivity_check.ipv6.neighbor_discovery.neighbor_discovery_data_gen import NeighborDiscoveryData", 
        "moduleYangNamespacePrefix": "qt-if", 
        "validationPoint": null, 
        "yangName": "neighbor-discovery", 
        "namespace": "neighbor_discovery", 
        "logGroupName": "blinky-neighbor-discovery", 
        "className": "BlinkyNeighborDiscovery", 
        "logModuleName": "a-sys-net-tech-interfaces-tech-interfaces-interface-connectivity-check-ipv6-neighbor-discovery-blinky-neighbor-discovery-gen", 
        "importStatement": "from a.sys.net.tech_interfaces.tech.interfaces.interface.connectivity_check.ipv6.neighbor_discovery.blinky_neighbor_discovery_gen import BlinkyNeighborDiscovery", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", 
        "dataClassName": "NeighborDiscoveryData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

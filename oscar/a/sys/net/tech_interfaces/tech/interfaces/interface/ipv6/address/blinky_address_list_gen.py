...
from a.sys.net.tech_interfaces.tech.interfaces.interface.ipv6.address.blinky_address_gen import BlinkyAddress

class BlinkyAddressList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyAddressList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyAddressList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyaddresslist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.net.tech_interfaces.tech.interfaces.interface.ipv6.address.blinky_address_gen import BlinkyAddress', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyAddress.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyaddresslist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.net.tech_interfaces.tech.interfaces.interface.ipv6.address.blinky_address_gen import BlinkyAddress', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-address-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-address-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-address").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyAddressList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-address").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyAddress. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyAddress(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-address-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyAddress not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-address").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyAddress. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyAddress(self, key):
        for logFunc in self._log("get-blinky-blinky-address").debug1Func(): logFunc("getBlinkyAddress(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyAddress(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-address-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyAddress-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyAddress(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyAddress", 
        "containerLogGroupName": "blinky-address", 
        "namespace": "address", 
        "containerImportStatement": "from a.sys.net.tech_interfaces.tech.interfaces.interface.ipv6.address.blinky_address_gen import BlinkyAddress", 
        "logGroupName": "blinky-address-list", 
        "className": "BlinkyAddressList", 
        "logModuleName": "a-sys-net-tech-interfaces-tech-interfaces-interface-ipv6-address-blinky-address-list-gen", 
        "keyLeaf": {
            "varName": "address", 

...
from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.ethernet.ethernet_data_gen import EthernetData





class BlinkyEthernet(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyethernet').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyEthernet._validationPointId, BlinkyEthernet._actionPointId)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyethernet').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.ethernet.blinky_ethernet_gen import BlinkyEthernet', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkyethernet-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.ethernet.blinky_ethernet_gen import BlinkyEthernet', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = EthernetData()
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = EthernetData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyethernet-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyethernet-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.ethernet.ethernet_data_gen import EthernetData", 
        "moduleYangNamespacePrefix": "qt-if", 
        "validationPoint": null, 
        "yangName": "ethernet", 
        "namespace": "ethernet", 
        "logGroupName": "blinky-ethernet", 
        "className": "BlinkyEthernet", 
        "logModuleName": "a-sys-net-tech-interfaces-tech-interfaces-interface-ethernet-blinky-ethernet-gen", 

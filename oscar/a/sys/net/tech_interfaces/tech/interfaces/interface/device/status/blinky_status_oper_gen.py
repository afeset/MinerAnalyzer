...
class BlinkyOperStatus(BlinkyOperNode):
    ...
    _kCallpointName = "tech-interfaces-interface-device-status-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-interfaces-interface-device-status-callpoint"
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "driver-type"):
                    for logFunc in self._log("set-oper-data-requested-fields-drivertype-requested").debug3Func(): logFunc(
                        "driver-type requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setDriverTypeRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "pci-address"):
                    for logFunc in self._log("set-oper-data-requested-fields-pciaddress-requested").debug3Func(): logFunc(
                        "pci-address requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setPciAddressRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "route-table-id"):
                    for logFunc in self._log("set-oper-data-requested-fields-routetableid-requested").debug3Func(): logFunc(
                        "route-table-id requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setRouteTableIdRequested()
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "driver-type"):
            if operData.isDriverTypeRequested():
                 if operData.hasDriverType():
                     value.setEnum(operData.driverType.getValue())
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "pci-address"):
            if operData.isPciAddressRequested():
                 if operData.hasPciAddress():
                     value.setString(operData.pciAddress)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "route-table-id"):
            if operData.isRouteTableIdRequested():
                 if operData.hasRouteTableId():
                     value.setString(operData.routeTableId)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.device.status.status_oper_data_gen import StatusOperData", 
        ...
        "logModuleName": "a-sys-net-tech-interfaces-tech-interfaces-interface-device-status-blinky-status-oper-gen", 
        "importStatement": "from a.sys.net.tech_interfaces.tech.interfaces.interface.device.status.blinky_status_oper_gen import BlinkyOperStatus", 
        "callpointName": "tech-interfaces-interface-device-status-callpoint", 

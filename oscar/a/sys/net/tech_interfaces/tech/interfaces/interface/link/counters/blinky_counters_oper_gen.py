...
from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.link.counters.counters_oper_data_gen import CountersOperData
...
class BlinkyOperCounters(BlinkyOperNode):
    ...
    _kCallpointName = "tech-interfaces-interface-link-counters-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-interfaces-interface-link-counters-callpoint"
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "result-down"):
                    for logFunc in self._log("set-oper-data-requested-fields-resultdown-requested").debug3Func(): logFunc(
                        "result-down requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setResultDownRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "result-unknown"):
                    for logFunc in self._log("set-oper-data-requested-fields-resultunknown-requested").debug3Func(): logFunc(
                        "result-unknown requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setResultUnknownRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "tests"):
                    for logFunc in self._log("set-oper-data-requested-fields-tests-requested").debug3Func(): logFunc(
                        "tests requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setTestsRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "result-up"):
                    for logFunc in self._log("set-oper-data-requested-fields-resultup-requested").debug3Func(): logFunc(
                        "result-up requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setResultUpRequested()
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "result-down"):
            if operData.isResultDownRequested():
                 if operData.hasResultDown():
                     value.setInt64(operData.resultDown)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "result-unknown"):
            if operData.isResultUnknownRequested():
                 if operData.hasResultUnknown():
                     value.setInt64(operData.resultUnknown)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "tests"):
            if operData.isTestsRequested():
                 if operData.hasTests():
                     value.setInt64(operData.tests)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "result-up"):
            if operData.isResultUpRequested():
                 if operData.hasResultUp():
                     value.setInt64(operData.resultUp)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.link.counters.counters_oper_data_gen import CountersOperData", 
        ...
        "logModuleName": "a-sys-net-tech-interfaces-tech-interfaces-interface-link-counters-blinky-counters-oper-gen", 
        "importStatement": "from a.sys.net.tech_interfaces.tech.interfaces.interface.link.counters.blinky_counters_oper_gen import BlinkyOperCounters", 
        "callpointName": "tech-interfaces-interface-link-counters-callpoint", 

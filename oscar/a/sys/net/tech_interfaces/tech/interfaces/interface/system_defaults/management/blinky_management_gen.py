...
from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.system_defaults.management.management_data_gen import ManagementData





class BlinkyManagement(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkymanagement').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyManagement._validationPointId, BlinkyManagement._actionPointId)
        ...
        newNode=BlinkyManagement(logger)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkymanagement').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.system_defaults.management.blinky_management_gen import BlinkyManagement', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkymanagement-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.system_defaults.management.blinky_management_gen import BlinkyManagement', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = ManagementData()
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = ManagementData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkymanagement-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkymanagement-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.system_defaults.management.management_data_gen import ManagementData", 
        "moduleYangNamespacePrefix": "qt-if", 
        "validationPoint": null, 
        "yangName": "management", 
        "namespace": "management", 
        "logGroupName": "blinky-management", 
        "className": "BlinkyManagement", 
        "logModuleName": "a-sys-net-tech-interfaces-tech-interfaces-interface-system-defaults-management-blinky-management-gen", 

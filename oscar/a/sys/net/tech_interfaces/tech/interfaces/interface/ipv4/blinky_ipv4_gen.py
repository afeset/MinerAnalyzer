...
from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.ipv4.ipv4_data_gen import Ipv4Data


from a.sys.net.tech_interfaces.tech.interfaces.interface.ipv4.address.blinky_address_list_gen import BlinkyAddressList
...
class BlinkyIpv4(BlinkyContainer):
    ...
    ourXmlTagDefaultGateway="default-gateway"
    

    #descendants
    
    ourXmlTagAddressList="address"
    ...
    ADDRESSLIST_CREATE_FUNCTOR = 'ADDRESSLIST_CREATE_FUNCTOR'
    ADDRESSLIST_DELETE_FUNCTOR = 'ADDRESSLIST_DELETE_FUNCTOR'
    ...
    def __init__ (self, logger):
        ...
        self.myCreateAddressListFunctor=None
        self.myDeleteAddressListFunctor=None
        self.myAddressList=None
    ...
    def setCreateAddressListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-create-address-functor-active").errorFunc():
                logFunc("setCreateAddressListFunctor() is illegal when blinky node is active")
        self.myCreateAddressListFunctor = functor

    def setDeleteAddressListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-delete-address-functor-active").errorFunc():
                logFunc("setDeleteAddressListFunctor() is illegal when blinky node is active")
        self.myDeleteAddressListFunctor = functor
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagDefaultGateway)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.defaultGateway = (newValue.asIPv4())
                self.myCandidateData.setHasDefaultGateway()
                
            else:
                self.setDefaultGatewayDefaultValue(True)
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if (keyPath.isTagEqual(keyDepth_PBR.value(), self.ourNamespace, self.ourXmlTagAddressList)):
            return self.myAddressList
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyipv4').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.ipv4.blinky_ipv4_gen import BlinkyIpv4', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        if not found:
            if keyPath.getLen() > keyDepth+1:
                if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagAddressList)):
                    keyPathRegistered = BlinkyAddressList.isKeyPathRegistered(logger, keyPath, keyDepth+1)
                    found = True
            else:
        ...
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagDefaultGateway)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkyipv4-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.ipv4.blinky_ipv4_gen import BlinkyIpv4', keyPathRegistered, keyPath, keyDepth)
    ...
    def prepareMyBlinkyNode(self, keyPath, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagAddressList)):
            if (self.myAddressList):
                for logFunc in self._log("prepare-my-blinky-node-addresslist-already-exists").errorFunc():
                    logFunc("prepareMyBlinkyNode(): already exists, keyPath=%s, keyDepth=%s", keyPath, keyDepth)
                return None
            self.myAddressList = BlinkyAddressList(self._log)
            self.myAddressList.setParent(self)
            self.myAddressList.setKeyPath(keyPath)
            self.myAddressList.setDomain(self.myDomain)
    ...
    def handleTrxElementCreate(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagAddressList)):
                if (self.myCreateAddressListFunctor):
                    if (not self.myAddressList):
                        for logFunc in self._log("handle-trx-element-create-addresslist-not-exist").debug3Func():
                            logFunc("handleTrxElementCreate(): addresslist not exists, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    
                    timeoutGuard = TimeoutGuard(self._log, '%s-create-addresslist-functor-%s' % (self.myKeyPath, phase), 
                                                self.getFunctorTimeoutForPhase(self.ADDRESSLIST_CREATE_FUNCTOR, phase),
                                                self.getFunctorMildTimeoutForPhase(self.ADDRESSLIST_CREATE_FUNCTOR, phase))
                    try:
                        res = self.myCreateAddressListFunctor(phase, self.myAddressList)
                    except:
                        for logFunc in self._log("handle-trx-element-create-addresslist-functor-exception").exceptionFunc():
                            logFunc("AddressList's create functor raised an exception. trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        raise
                    timeoutGuard.checkAndLog("application problem: functor=%s" % self.myCreateAddressListFunctor.__name__)
                    if (res != ReturnCodes.kOk):
                        if (phase.getConfdPhase() == TrxPhase.kPrepare):
                            for logFunc in self._log("handle-trx-element-create-addresslist-functor-failed-prepare").noticeFunc():
                                logFunc("handleTrxElementCreate(): addresslist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                            self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        else:
                            for logFunc in self._log("handle-trx-element-create-addresslist-functor-failed").errorFunc():
                                logFunc("handleTrxElementCreate(): addresslist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    ...
    def notifyDescendantsPrepareBlinkyDelete(self):
        ...
        if (self.myAddressList):
            self.myAddressList.notifyDescendantsPrepareBlinkyDelete()
    ...
    def handleTrxElementDelete(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagAddressList)):
                res = self.activateDeleteAddressListFunctor(phase)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("handle-trx-element-delete-addresslist-functor-failed-prepare").noticeFunc():
                            logFunc("handleTrxElementDelete(): addresslist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("handle-trx-element-delete-addresslist-functor-failed").errorFunc():
                            logFunc("handleTrxElementDelete(): addresslist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                    return ReturnCodes.kGeneralError
    ...
    def removeAddressList(self):
        self.myAddressList = None

    def activateDeleteAddressListFunctor(self, phase):
        for logFunc in self._log("activate-delete-addresslist-functor").debug3Func(): logFunc("activateDeleteAddressListFunctor(): called, phase=%s", phase)
        if (self.myIsActive):
            if (self.myDeleteAddressListFunctor):
                timeoutGuard = TimeoutGuard(self._log, '%s-delete-addresslist-functor-%s' % (self.myKeyPath, phase), 
                                            self.getFunctorTimeoutForPhase(self.ADDRESSLIST_DELETE_FUNCTOR, phase), 
                                            self.getFunctorMildTimeoutForPhase(self.ADDRESSLIST_DELETE_FUNCTOR, phase))
                try:
                    res = self.myDeleteAddressListFunctor(phase)
                except:
                    for logFunc in self._log("activate-delete-addresslist-functor-exception").exceptionFunc():
                        logFunc("AddressList's delete functor raised an exception. phase=%s", phase)
                    raise
                timeoutGuard.checkAndLog("application problem: functor=%s" % self.myDeleteAddressListFunctor.__name__)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("activate-delete-addresslist-functor-functor-failed-prepare").noticeFunc():
                            logFunc("activateDeleteAddressListFunctor(): functor failed, phase=%s", phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("activate-delete-addresslist-functor-functor-failed").errorFunc():
                            logFunc("activateDeleteAddressListFunctor(): functor failed, phase=%s", phase)
                    return ReturnCodes.kGeneralError
        return ReturnCodes.kOk
    ...
    def allocMyCandidate(self):
        ...
        res = self.setDefaultGatewayDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-defaultgateway-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setDefaultGatewayDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setDefaultGatewayDefaultValue (self, setHas):
        for logFunc in self._log("set-defaultgateway-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.defaultGateway = None
    ...
    def handleInternalDestroy(self, phase):
        ...
        if (phase.getConfdPhase() == TrxPhase.kPrepare):
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myAddressList):
                    res = self.myAddressList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-addresslist-recursion-failed-prepare-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): delete-addresslist-recursion-failed-prepare-blinky, phase=%s",
                                  phase)
                        return ReturnCodes.kGeneralError
                ...
                if (self.myAddressList):
                    res = self.activateDeleteAddressListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-addresslist-functor-failed-prepare").noticeFunc():
                            logFunc("handleInternalDestroy(): activateDeleteAddressListFunctor() failed, res=%s, phase=%s",
                                   res, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        return ReturnCodes.kGeneralError
                if (self.myAddressList):
                    res = self.myAddressList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-addresslist-recursion-failed-prepare-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myAddressList.handleInternalDestroy() failed, res=%s, phase=%s",
                                   res, phase)
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myAddressList):
                    res = self.myAddressList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-addresslist-recursion-failed-commit-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myAddressList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myAddressList):
                    res = self.myAddressList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-addresslist-recursion-failed-commit-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myAddressList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                    ...
                    res = self.activateDeleteAddressListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-addresslist-functor-failed-commit").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteAddressListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("activateDeleteAddressListFunctor failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myAddressList):
                    res = self.myAddressList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-addresslist-recursion-failed-abort-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myAddressList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myAddressList):
                    res = self.myAddressList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-addresslist-recursion-failed-abort-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myAddressList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myAddressList):
                    res = self.activateDeleteAddressListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-addresslist-functor-failed-abort").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteAddressListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.ipv4.ipv4_data_gen import Ipv4Data", 
        ...
        "logModuleName": "a-sys-net-tech-interfaces-tech-interfaces-interface-ipv4-blinky-ipv4-gen", 
    ...
    "descendants": [
        {
            "className": "BlinkyAddressList", 
            "memberName": "AddressList", 
            "yangName": "address", 
            "importStatement": "from a.sys.net.tech_interfaces.tech.interfaces.interface.ipv4.address.blinky_address_list_gen import BlinkyAddressList"
        }
    ], 

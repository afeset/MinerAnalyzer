...
from a.sys.net.tech_interfaces.tech.interfaces.interface.blinky_interface_gen import BlinkyInterface

class BlinkyInterfaceList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyInterfaceList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyInterfaceList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyinterfacelist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.net.tech_interfaces.tech.interfaces.interface.blinky_interface_gen import BlinkyInterface', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyInterface.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyinterfacelist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.net.tech_interfaces.tech.interfaces.interface.blinky_interface_gen import BlinkyInterface', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-interface-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-interface-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-interface-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-interface").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyInterfaceList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-interface").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyInterface. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyInterface(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-interface-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyInterface not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-interface").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyInterface. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyInterface(self, key):
        for logFunc in self._log("get-blinky-blinky-interface").debug1Func(): logFunc("getBlinkyInterface(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyInterface(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkyInterface(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-interface-moved-failed").errorFunc(): logFunc("getBlinkyInterface(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkyInterface(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-interface-after-failed").errorFunc(): logFunc("getBlinkyInterface(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-interface-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyInterface-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyInterface(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyInterface", 
        "containerLogGroupName": "blinky-interface", 
        "namespace": "interface", 
        "containerImportStatement": "from a.sys.net.tech_interfaces.tech.interfaces.interface.blinky_interface_gen import BlinkyInterface", 
        "logGroupName": "blinky-interface-list", 
        "className": "BlinkyInterfaceList", 
        "logModuleName": "a-sys-net-tech-interfaces-tech-interfaces-interface-blinky-interface-list-gen", 
        "keyLeaf": {
            "varName": "interface", 

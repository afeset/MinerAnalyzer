...
from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.connectivity_check.ipv4.counters.counters_oper_data_gen import CountersOperData
...
class BlinkyOperCounters(BlinkyOperNode):
    ...
    _kCallpointName = "tech-interfaces-interface-connectivity-check-ipv4-counters-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-interfaces-interface-connectivity-check-ipv4-counters-callpoint"
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "arp-failures"):
                    for logFunc in self._log("set-oper-data-requested-fields-arpfailures-requested").debug3Func(): logFunc(
                        "arp-failures requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setArpFailuresRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "arp-requests-sent"):
                    for logFunc in self._log("set-oper-data-requested-fields-arprequestssent-requested").debug3Func(): logFunc(
                        "arp-requests-sent requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setArpRequestsSentRequested()
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "arp-successes"):
                    for logFunc in self._log("set-oper-data-requested-fields-arpsuccesses-requested").debug3Func(): logFunc(
                        "arp-successes requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setArpSuccessesRequested()
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "arp-timeouts"):
                    for logFunc in self._log("set-oper-data-requested-fields-arptimeouts-requested").debug3Func(): logFunc(
                        "arp-timeouts requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setArpTimeoutsRequested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isArpFailuresRequested() and operData.hasArpFailures():
            val = Value()
            val.setInt64(operData.arpFailures)
            tagValueList.push(("arp-failures", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces"), val)
        
        if operData.isArpRequestsSentRequested() and operData.hasArpRequestsSent():
            val = Value()
            val.setInt64(operData.arpRequestsSent)
            tagValueList.push(("arp-requests-sent", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces"), val)
        ...
        if operData.isArpSuccessesRequested() and operData.hasArpSuccesses():
            val = Value()
            val.setInt64(operData.arpSuccesses)
            tagValueList.push(("arp-successes", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces"), val)
        ...
        if operData.isArpTimeoutsRequested() and operData.hasArpTimeouts():
            val = Value()
            val.setInt64(operData.arpTimeouts)
            tagValueList.push(("arp-timeouts", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "arp-failures"):
            if operData.isArpFailuresRequested():
                 if operData.hasArpFailures():
                     value.setInt64(operData.arpFailures)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "arp-requests-sent"):
            if operData.isArpRequestsSentRequested():
                 if operData.hasArpRequestsSent():
                     value.setInt64(operData.arpRequestsSent)
                 else:
                     value.setEmpty()
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "arp-successes"):
            if operData.isArpSuccessesRequested():
                 if operData.hasArpSuccesses():
                     value.setInt64(operData.arpSuccesses)
                 else:
                     value.setEmpty()
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "arp-timeouts"):
            if operData.isArpTimeoutsRequested():
                 if operData.hasArpTimeouts():
                     value.setInt64(operData.arpTimeouts)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.connectivity_check.ipv4.counters.counters_oper_data_gen import CountersOperData", 
        ...
        "logModuleName": "a-sys-net-tech-interfaces-tech-interfaces-interface-connectivity-check-ipv4-counters-blinky-counters-oper-gen", 
        "importStatement": "from a.sys.net.tech_interfaces.tech.interfaces.interface.connectivity_check.ipv4.counters.blinky_counters_oper_gen import BlinkyOperCounters", 
        "callpointName": "tech-interfaces-interface-connectivity-check-ipv4-counters-callpoint", 

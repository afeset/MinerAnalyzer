...
from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.link.link_data_gen import LinkData





class BlinkyLink(BlinkyContainer):
    ...
    _actionPointId=None
    _actionPointId="tech-interfaces-interface-link-clear-counters-actionpoint"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkylink').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyLink._validationPointId, BlinkyLink._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagUpPeriod)):
        ...
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagTestTimeoutMsec)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkylink').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.link.blinky_link_gen import BlinkyLink', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkylink-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.link.blinky_link_gen import BlinkyLink', keyPathRegistered, keyPath, keyDepth)
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkylink-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkylink-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.link.link_data_gen import LinkData", 
        "moduleYangNamespacePrefix": "qt-if", 
        "validationPoint": null, 
        "yangName": "link", 
        "namespace": "link", 
        "logGroupName": "blinky-link", 
        "className": "BlinkyLink", 
        "logModuleName": "a-sys-net-tech-interfaces-tech-interfaces-interface-link-blinky-link-gen", 
        ...
        "actionPoint": "tech-interfaces-interface-link-clear-counters-actionpoint"
    }, 
    "ancestors": [
        {

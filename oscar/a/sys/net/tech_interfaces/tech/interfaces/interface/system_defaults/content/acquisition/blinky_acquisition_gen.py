...
from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.system_defaults.content.acquisition.acquisition_data_gen import AcquisitionData





class BlinkyAcquisition(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyacquisition').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyAcquisition._validationPointId, BlinkyAcquisition._actionPointId)

        confd_key=KeyPath()
        
        
        xmlVal = Value()
        xmlVal.setXmlTag(("acquisition", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", "qt-if"))
        ...
        newNode=BlinkyAcquisition(logger)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyacquisition').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.system_defaults.content.acquisition.blinky_acquisition_gen import BlinkyAcquisition', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkyacquisition-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_interfaces.tech.interfaces.interface.system_defaults.content.acquisition.blinky_acquisition_gen import BlinkyAcquisition', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = AcquisitionData()
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = AcquisitionData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyacquisition-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyacquisition-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_interfaces.tech.interfaces.interface.system_defaults.content.acquisition.acquisition_data_gen import AcquisitionData", 
        "moduleYangNamespacePrefix": "qt-if", 
        "validationPoint": null, 
        "yangName": "acquisition", 
        "namespace": "acquisition", 
        "logGroupName": "blinky-acquisition", 
        "className": "BlinkyAcquisition", 
        "logModuleName": "a-sys-net-tech-interfaces-tech-interfaces-interface-system-defaults-content-acquisition-blinky-acquisition-gen", 
        "importStatement": "from a.sys.net.tech_interfaces.tech.interfaces.interface.system_defaults.content.acquisition.blinky_acquisition_gen import BlinkyAcquisition", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-interfaces", 
        "dataClassName": "AcquisitionData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

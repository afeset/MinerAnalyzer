...
from a.api.yang.modules.tech.content.qwilt_tech_content_interfaces.tech.content.interfaces.interface.delivery.ipv6.status.status_oper_data_gen import StatusOperData
...
class BlinkyOperStatus(BlinkyOperNode):
    ...
    _kCallpointName = "tech-content-interfaces-interface-delivery-ipv6-status-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-content-interfaces-interface-delivery-ipv6-status-callpoint"
    ...
    def getOperRelativePath (self, operRelativePath):
        ...
        val = Value()
        val.setXmlTag(("status", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-interfaces", "qtc-if"))
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-interfaces", "actual-delivery-interface"):
                    for logFunc in self._log("set-oper-data-requested-fields-actualdeliveryinterface-requested").debug3Func(): logFunc(
                        "actual-delivery-interface requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setActualDeliveryInterfaceRequested()
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-interfaces", "actual-delivery-interface"):
            if operData.isActualDeliveryInterfaceRequested():
                 if operData.hasActualDeliveryInterface():
                     value.setString(operData.actualDeliveryInterface)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.content.qwilt_tech_content_interfaces.tech.content.interfaces.interface.delivery.ipv6.status.status_oper_data_gen import StatusOperData", 
        ...
        "logModuleName": "a-sys-net-tech-content-interfaces-tech-content-interfaces-interface-delivery-ipv6-status-blinky-status-oper-gen", 
        "importStatement": "from a.sys.net.tech_content_interfaces.tech.content.interfaces.interface.delivery.ipv6.status.blinky_status_oper_gen import BlinkyOperStatus", 
        "callpointName": "tech-content-interfaces-interface-delivery-ipv6-status-callpoint", 

...
from a.sys.net.tech_content_interfaces.tech.content.interfaces.interface.delivery.ipv4.blinky_ipv4_gen import BlinkyIpv4

from a.sys.net.tech_content_interfaces.tech.content.interfaces.interface.delivery.ipv6.blinky_ipv6_gen import BlinkyIpv6




class BlinkyDelivery(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-content-interfaces"
    #leaves
    
    ourXmlTagPreferredDeliveryInterface="preferred-delivery-interface"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkydelivery').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyDelivery._validationPointId, BlinkyDelivery._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagPreferredDeliveryInterface)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.preferredDeliveryInterface = newValue.asString()
                self.myCandidateData.setHasPreferredDeliveryInterface()
                
            else:
                self.setPreferredDeliveryInterfaceDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkydelivery').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_content_interfaces.tech.content.interfaces.interface.delivery.blinky_delivery_gen import BlinkyDelivery', keyPath, keyDepth)
        ...
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagPreferredDeliveryInterface)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkydelivery-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_content_interfaces.tech.content.interfaces.interface.delivery.blinky_delivery_gen import BlinkyDelivery', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = DeliveryData()
        
        res = self.setPreferredDeliveryInterfaceDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-preferreddeliveryinterface-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setPreferredDeliveryInterfaceDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setPreferredDeliveryInterfaceDefaultValue (self, setHas):
        for logFunc in self._log("set-preferreddeliveryinterface-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.preferredDeliveryInterface = ""
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = DeliveryData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkydelivery-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkydelivery-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.content.qwilt_tech_content_interfaces.tech.content.interfaces.interface.delivery.delivery_data_gen import DeliveryData", 
        "moduleYangNamespacePrefix": "qtc-if", 
        "validationPoint": null, 
        "yangName": "delivery", 
        "namespace": "delivery", 
        "logGroupName": "blinky-delivery", 
        "className": "BlinkyDelivery", 
        "logModuleName": "a-sys-net-tech-content-interfaces-tech-content-interfaces-interface-delivery-blinky-delivery-gen", 
        "importStatement": "from a.sys.net.tech_content_interfaces.tech.content.interfaces.interface.delivery.blinky_delivery_gen import BlinkyDelivery", 
    ...
    "descendants": [
        {
            ...
            "memberName": "Ipv4", 
            "yangName": "ipv4", 
            "importStatement": "from a.sys.net.tech_content_interfaces.tech.content.interfaces.interface.delivery.ipv4.blinky_ipv4_gen import BlinkyIpv4"
        }, 
        {
            "className": "BlinkyIpv6", 
            "memberName": "Ipv6", 
            "yangName": "ipv6", 
            "importStatement": "from a.sys.net.tech_content_interfaces.tech.content.interfaces.interface.delivery.ipv6.blinky_ipv6_gen import BlinkyIpv6"
        }
    ], 

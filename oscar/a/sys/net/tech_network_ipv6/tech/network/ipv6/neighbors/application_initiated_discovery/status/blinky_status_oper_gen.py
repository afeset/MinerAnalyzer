...
from a.api.yang.modules.tech.common.qwilt_tech_network_ipv6.tech.network.ipv6.neighbors.application_initiated_discovery.status.status_oper_data_gen import StatusOperData
...
class BlinkyOperStatus(BlinkyOperNode):
    ...
    _kCallpointName = "tech-network-ipv6-application-initiated-discovery-status-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-network-ipv6-application-initiated-discovery-status-callpoint"
    ...
    def getOperRelativePath (self, operRelativePath):
        ...
        val = Value()
        val.setXmlTag(("status", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-network-ipv6", "qt-net-ip6"))
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-network-ipv6", "application-entry-count"):
                    for logFunc in self._log("set-oper-data-requested-fields-applicationentrycount-requested").debug3Func(): logFunc(
                        "application-entry-count requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setApplicationEntryCountRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-network-ipv6", "concurrent-requests"):
                    for logFunc in self._log("set-oper-data-requested-fields-concurrentrequests-requested").debug3Func(): logFunc(
                        "concurrent-requests requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setConcurrentRequestsRequested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isApplicationEntryCountRequested() and operData.hasApplicationEntryCount():
            val = Value()
            val.setInt64(operData.applicationEntryCount)
            tagValueList.push(("application-entry-count", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-network-ipv6"), val)
        
        if operData.isConcurrentRequestsRequested() and operData.hasConcurrentRequests():
            val = Value()
            val.setInt64(operData.concurrentRequests)
            tagValueList.push(("concurrent-requests", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-network-ipv6"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-network-ipv6", "application-entry-count"):
            if operData.isApplicationEntryCountRequested():
                 if operData.hasApplicationEntryCount():
                     value.setInt64(operData.applicationEntryCount)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-network-ipv6", "concurrent-requests"):
            if operData.isConcurrentRequestsRequested():
                 if operData.hasConcurrentRequests():
                     value.setInt64(operData.concurrentRequests)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_network_ipv6.tech.network.ipv6.neighbors.application_initiated_discovery.status.status_oper_data_gen import StatusOperData", 
        ...
        "logModuleName": "a-sys-net-tech-network-ipv6-tech-network-ipv6-neighbors-application-initiated-discovery-status-blinky-status-oper-gen", 
        "importStatement": "from a.sys.net.tech_network_ipv6.tech.network.ipv6.neighbors.application_initiated_discovery.status.blinky_status_oper_gen import BlinkyOperStatus", 
        "callpointName": "tech-network-ipv6-application-initiated-discovery-status-callpoint", 

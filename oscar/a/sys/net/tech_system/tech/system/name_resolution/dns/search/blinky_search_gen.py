...
from a.api.yang.modules.tech.common.qwilt_tech_system.tech.system.name_resolution.dns.search.search_data_gen import SearchData





class BlinkySearch(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkysearch').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkySearch._validationPointId, BlinkySearch._actionPointId)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkysearch').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.name_resolution.dns.search.blinky_search_gen import BlinkySearch', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkysearch-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.name_resolution.dns.search.blinky_search_gen import BlinkySearch', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = SearchData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkysearch-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkysearch-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_system.tech.system.name_resolution.dns.search.search_data_gen import SearchData", 
        "moduleYangNamespacePrefix": "qt-sys", 
        "validationPoint": null, 
        "yangName": "search", 
        "namespace": "search", 
        "logGroupName": "blinky-search", 
        "className": "BlinkySearch", 
        "logModuleName": "a-sys-net-tech-system-tech-system-name-resolution-dns-search-blinky-search-gen", 
        "importStatement": "from a.sys.net.tech_system.tech.system.name_resolution.dns.search.blinky_search_gen import BlinkySearch", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system", 
        "dataClassName": "SearchData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

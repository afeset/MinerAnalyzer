...
from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.blinky_ipv4_gen import BlinkyIpv4

class BlinkyIpv4List(BlinkyList):
    ...
    def __str__ (self):
        ret="{BlinkyIpv4List: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyipv4list').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.blinky_ipv4_gen import BlinkyIpv4', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkyipv4list-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.blinky_ipv4_gen import BlinkyIpv4', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-ipv4-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-ipv4-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-ipv4").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyIpv4List. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-ipv4").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyIpv4. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyIpv4(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-ipv4-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyIpv4 not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-ipv4").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyIpv4. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyIpv4(self, key):
        for logFunc in self._log("get-blinky-blinky-ipv4").debug1Func(): logFunc("getBlinkyIpv4(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyIpv4(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-ipv4-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyIpv4-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyIpv4(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyIpv4", 
        "containerLogGroupName": "blinky-ipv4", 
        "namespace": "ipv4", 
        "containerImportStatement": "from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.blinky_ipv4_gen import BlinkyIpv4", 
        "logGroupName": "blinky-ipv4-list", 
        "className": "BlinkyIpv4List", 
        "logModuleName": "a-sys-net-tech-system-tech-system-name-resolution-static-resolution-hosts-ipv4-blinky-ipv4-list-gen", 
        "keyLeaf": {
            "varName": "ipv4", 

...
from a.sys.net.tech_system.tech.system.name_resolution.dns.search.blinky_search_gen import BlinkySearch

class BlinkySearchList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkySearchList(logger)
    ...
    def __str__ (self):
        ret="{BlinkySearchList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkysearchlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.net.tech_system.tech.system.name_resolution.dns.search.blinky_search_gen import BlinkySearch', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkySearch.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkysearchlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.net.tech_system.tech.system.name_resolution.dns.search.blinky_search_gen import BlinkySearch', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-search-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-search-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-search-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-search").debug1Func(): logFunc("findBlinkyNode(): handling BlinkySearchList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-search").debug1Func(): logFunc("findBlinkyNode(): getting BlinkySearch. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkySearch(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-search-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkySearch not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-search").debug1Func(): logFunc("findBlinkyNode(): querying BlinkySearch. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkySearch(self, key):
        for logFunc in self._log("get-blinky-blinky-search").debug1Func(): logFunc("getBlinkySearch(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkySearch(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkySearch(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-search-moved-failed").errorFunc(): logFunc("getBlinkySearch(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkySearch(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-search-after-failed").errorFunc(): logFunc("getBlinkySearch(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-search-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkySearch-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkySearch(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkySearch", 
        "containerLogGroupName": "blinky-search", 
        "namespace": "search", 
        "containerImportStatement": "from a.sys.net.tech_system.tech.system.name_resolution.dns.search.blinky_search_gen import BlinkySearch", 
        "logGroupName": "blinky-search-list", 
        "className": "BlinkySearchList", 
        "logModuleName": "a-sys-net-tech-system-tech-system-name-resolution-dns-search-blinky-search-list-gen", 
        "keyLeaf": {
            "varName": "search", 

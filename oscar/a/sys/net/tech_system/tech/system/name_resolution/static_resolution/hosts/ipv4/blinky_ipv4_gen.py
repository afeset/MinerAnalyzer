...
from a.api.yang.modules.tech.common.qwilt_tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.ipv4_data_gen import Ipv4Data


from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.host.blinky_host_list_gen import BlinkyHostList
...
class BlinkyIpv4(BlinkyContainer):
    ...
    ourXmlTagAddress="address"
    

    #descendants
    
    ourXmlTagHostList="host"
    ...
    HOSTLIST_CREATE_FUNCTOR = 'HOSTLIST_CREATE_FUNCTOR'
    HOSTLIST_DELETE_FUNCTOR = 'HOSTLIST_DELETE_FUNCTOR'
    ...
    def __init__ (self, logger):
        ...
        self.myCreateHostListFunctor=None
        self.myDeleteHostListFunctor=None
        self.myHostList=None
    ...
    def setCreateHostListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-create-host-functor-active").errorFunc():
                logFunc("setCreateHostListFunctor() is illegal when blinky node is active")
        self.myCreateHostListFunctor = functor

    def setDeleteHostListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-delete-host-functor-active").errorFunc():
                logFunc("setDeleteHostListFunctor() is illegal when blinky node is active")
        self.myDeleteHostListFunctor = functor
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagAddress)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.address = (newValue.asIPv4())
                self.myCandidateData.setHasAddress()
                
            else:
                self.setAddressDefaultValue(True)
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if (keyPath.isTagEqual(keyDepth_PBR.value(), self.ourNamespace, self.ourXmlTagHostList)):
            return self.myHostList
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyipv4').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.blinky_ipv4_gen import BlinkyIpv4', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        if not found:
            if keyPath.getLen() > keyDepth+1:
                if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagHostList)):
                    keyPathRegistered = BlinkyHostList.isKeyPathRegistered(logger, keyPath, keyDepth+1)
                    found = True
            else:
        ...
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagAddress)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkyipv4-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.blinky_ipv4_gen import BlinkyIpv4', keyPathRegistered, keyPath, keyDepth)
    ...
    def prepareMyBlinkyNode(self, keyPath, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagHostList)):
            if (self.myHostList):
                for logFunc in self._log("prepare-my-blinky-node-hostlist-already-exists").errorFunc():
                    logFunc("prepareMyBlinkyNode(): already exists, keyPath=%s, keyDepth=%s", keyPath, keyDepth)
                return None
            self.myHostList = BlinkyHostList(self._log)
            self.myHostList.setParent(self)
            self.myHostList.setKeyPath(keyPath)
            self.myHostList.setDomain(self.myDomain)
    ...
    def handleTrxElementCreate(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagHostList)):
                if (self.myCreateHostListFunctor):
                    if (not self.myHostList):
                        for logFunc in self._log("handle-trx-element-create-hostlist-not-exist").debug3Func():
                            logFunc("handleTrxElementCreate(): hostlist not exists, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    
                    timeoutGuard = TimeoutGuard(self._log, '%s-create-hostlist-functor-%s' % (self.myKeyPath, phase), 
                                                self.getFunctorTimeoutForPhase(self.HOSTLIST_CREATE_FUNCTOR, phase),
                                                self.getFunctorMildTimeoutForPhase(self.HOSTLIST_CREATE_FUNCTOR, phase))
                    try:
                        res = self.myCreateHostListFunctor(phase, self.myHostList)
                    except:
                        for logFunc in self._log("handle-trx-element-create-hostlist-functor-exception").exceptionFunc():
                            logFunc("HostList's create functor raised an exception. trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        raise
                    timeoutGuard.checkAndLog("application problem: functor=%s" % self.myCreateHostListFunctor.__name__)
                    if (res != ReturnCodes.kOk):
                        if (phase.getConfdPhase() == TrxPhase.kPrepare):
                            for logFunc in self._log("handle-trx-element-create-hostlist-functor-failed-prepare").noticeFunc():
                                logFunc("handleTrxElementCreate(): hostlist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                            self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        else:
                            for logFunc in self._log("handle-trx-element-create-hostlist-functor-failed").errorFunc():
                                logFunc("handleTrxElementCreate(): hostlist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    ...
    def notifyDescendantsPrepareBlinkyDelete(self):
        ...
        if (self.myHostList):
            self.myHostList.notifyDescendantsPrepareBlinkyDelete()
    ...
    def handleTrxElementDelete(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagHostList)):
                res = self.activateDeleteHostListFunctor(phase)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("handle-trx-element-delete-hostlist-functor-failed-prepare").noticeFunc():
                            logFunc("handleTrxElementDelete(): hostlist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("handle-trx-element-delete-hostlist-functor-failed").errorFunc():
                            logFunc("handleTrxElementDelete(): hostlist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                    return ReturnCodes.kGeneralError
    ...
    def removeHostList(self):
        self.myHostList = None

    def activateDeleteHostListFunctor(self, phase):
        for logFunc in self._log("activate-delete-hostlist-functor").debug3Func(): logFunc("activateDeleteHostListFunctor(): called, phase=%s", phase)
        if (self.myIsActive):
            if (self.myDeleteHostListFunctor):
                timeoutGuard = TimeoutGuard(self._log, '%s-delete-hostlist-functor-%s' % (self.myKeyPath, phase), 
                                            self.getFunctorTimeoutForPhase(self.HOSTLIST_DELETE_FUNCTOR, phase), 
                                            self.getFunctorMildTimeoutForPhase(self.HOSTLIST_DELETE_FUNCTOR, phase))
                try:
                    res = self.myDeleteHostListFunctor(phase)
                except:
                    for logFunc in self._log("activate-delete-hostlist-functor-exception").exceptionFunc():
                        logFunc("HostList's delete functor raised an exception. phase=%s", phase)
                    raise
                timeoutGuard.checkAndLog("application problem: functor=%s" % self.myDeleteHostListFunctor.__name__)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("activate-delete-hostlist-functor-functor-failed-prepare").noticeFunc():
                            logFunc("activateDeleteHostListFunctor(): functor failed, phase=%s", phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("activate-delete-hostlist-functor-functor-failed").errorFunc():
                            logFunc("activateDeleteHostListFunctor(): functor failed, phase=%s", phase)
                    return ReturnCodes.kGeneralError
        return ReturnCodes.kOk
    ...
    def allocMyCandidate(self):
        ...
        res = self.setAddressDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-address-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setAddressDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setAddressDefaultValue (self, setHas):
        for logFunc in self._log("set-address-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.address = None
    ...
    def handleInternalDestroy(self, phase):
        ...
        if (phase.getConfdPhase() == TrxPhase.kPrepare):
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myHostList):
                    res = self.myHostList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-hostlist-recursion-failed-prepare-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): delete-hostlist-recursion-failed-prepare-blinky, phase=%s",
                                  phase)
                        return ReturnCodes.kGeneralError
                ...
                if (self.myHostList):
                    res = self.activateDeleteHostListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-hostlist-functor-failed-prepare").noticeFunc():
                            logFunc("handleInternalDestroy(): activateDeleteHostListFunctor() failed, res=%s, phase=%s",
                                   res, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        return ReturnCodes.kGeneralError
                if (self.myHostList):
                    res = self.myHostList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-hostlist-recursion-failed-prepare-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myHostList.handleInternalDestroy() failed, res=%s, phase=%s",
                                   res, phase)
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myHostList):
                    res = self.myHostList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-hostlist-recursion-failed-commit-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myHostList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myHostList):
                    res = self.myHostList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-hostlist-recursion-failed-commit-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myHostList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                    ...
                    res = self.activateDeleteHostListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-hostlist-functor-failed-commit").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteHostListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("activateDeleteHostListFunctor failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myHostList):
                    res = self.myHostList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-hostlist-recursion-failed-abort-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myHostList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myHostList):
                    res = self.myHostList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-hostlist-recursion-failed-abort-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myHostList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myHostList):
                    res = self.activateDeleteHostListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-hostlist-functor-failed-abort").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteHostListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.ipv4_data_gen import Ipv4Data", 
        ...
        "logModuleName": "a-sys-net-tech-system-tech-system-name-resolution-static-resolution-hosts-ipv4-blinky-ipv4-gen", 
        "importStatement": "from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.blinky_ipv4_gen import BlinkyIpv4", 
    ...
    "descendants": [
        {
            ...
            "memberName": "HostList", 
            "yangName": "host", 
            "importStatement": "from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.host.blinky_host_list_gen import BlinkyHostList"
        }
    ], 

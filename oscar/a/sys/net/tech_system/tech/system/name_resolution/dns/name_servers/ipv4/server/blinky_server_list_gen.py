...
from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.server.blinky_server_gen import BlinkyServer

class BlinkyServerList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyServerList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyServerList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyserverlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.server.blinky_server_gen import BlinkyServer', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyServer.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyserverlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.server.blinky_server_gen import BlinkyServer', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-server-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-server-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-server-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-server").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyServerList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-server").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyServer. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyServer(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-server-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyServer not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-server").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyServer. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyServer(self, key):
        for logFunc in self._log("get-blinky-blinky-server").debug1Func(): logFunc("getBlinkyServer(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyServer(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkyServer(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-server-moved-failed").errorFunc(): logFunc("getBlinkyServer(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkyServer(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-server-after-failed").errorFunc(): logFunc("getBlinkyServer(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-server-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyServer-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
        ...
        if trxElement.getOpCode() == TrxElement.kCreate:
            ...
            if trxElement.getNewVal():
                ...
                movedAfterKey = (trxElement.getNewVal().asIPv4())
                if res != ReturnCodes.kOk:
                    return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyServer(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyServer", 
        "containerLogGroupName": "blinky-server", 
        "namespace": "server", 
        "containerImportStatement": "from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.server.blinky_server_gen import BlinkyServer", 
        "logGroupName": "blinky-server-list", 
        "className": "BlinkyServerList", 
        "logModuleName": "a-sys-net-tech-system-tech-system-name-resolution-dns-name-servers-ipv4-server-blinky-server-list-gen", 
        "keyLeaf": {
            "varName": "server", 

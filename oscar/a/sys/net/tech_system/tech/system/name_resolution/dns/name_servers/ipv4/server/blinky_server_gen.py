...
from a.api.yang.modules.tech.common.qwilt_tech_system.tech.system.name_resolution.dns.name_servers.ipv4.server.server_data_gen import ServerData
...
class BlinkyServer(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyserver').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyServer._validationPointId, BlinkyServer._actionPointId)

        confd_key=KeyPath()
        
        
        ancestorVal = Value()
        ancestorVal.setString(server);
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyserver').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.server.blinky_server_gen import BlinkyServer', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkyserver-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.server.blinky_server_gen import BlinkyServer', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = ServerData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyserver-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyserver-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_system.tech.system.name_resolution.dns.name_servers.ipv4.server.server_data_gen import ServerData", 
        "moduleYangNamespacePrefix": "qt-sys", 
        "validationPoint": null, 
        "yangName": "server", 
        "namespace": "server", 
        "logGroupName": "blinky-server", 
        "className": "BlinkyServer", 
        "logModuleName": "a-sys-net-tech-system-tech-system-name-resolution-dns-name-servers-ipv4-server-blinky-server-gen", 
        "importStatement": "from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.server.blinky_server_gen import BlinkyServer", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system", 
        "dataClassName": "ServerData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

...
from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.server.blinky_server_list_gen import BlinkyServerList
...
class BlinkyIpv4(BlinkyContainer):
    ...
    SERVERLIST_CREATE_FUNCTOR = 'SERVERLIST_CREATE_FUNCTOR'
    SERVERLIST_DELETE_FUNCTOR = 'SERVERLIST_DELETE_FUNCTOR'
    ...
    def __init__ (self, logger):
        ...
        self.myCreateServerListFunctor=None
        self.myDeleteServerListFunctor=None
        self.myServerList=None
    ...
    def setCreateServerListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-create-server-functor-active").errorFunc():
                logFunc("setCreateServerListFunctor() is illegal when blinky node is active")
        self.myCreateServerListFunctor = functor

    def setDeleteServerListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-delete-server-functor-active").errorFunc():
                logFunc("setDeleteServerListFunctor() is illegal when blinky node is active")
        self.myDeleteServerListFunctor = functor
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if (keyPath.isTagEqual(keyDepth_PBR.value(), self.ourNamespace, self.ourXmlTagServerList)):
            return self.myServerList
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyipv4').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.blinky_ipv4_gen import BlinkyIpv4', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        if not found:
            if keyPath.getLen() > keyDepth+1:
                if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagServerList)):
                    keyPathRegistered = BlinkyServerList.isKeyPathRegistered(logger, keyPath, keyDepth+1)
                    found = True
            else:
        ...
        logger('is-key-path-registered-blinkyipv4-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.blinky_ipv4_gen import BlinkyIpv4', keyPathRegistered, keyPath, keyDepth)
    ...
    def prepareMyBlinkyNode(self, keyPath, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagServerList)):
            if (self.myServerList):
                for logFunc in self._log("prepare-my-blinky-node-serverlist-already-exists").errorFunc():
                    logFunc("prepareMyBlinkyNode(): already exists, keyPath=%s, keyDepth=%s", keyPath, keyDepth)
                return None
            self.myServerList = BlinkyServerList(self._log)
            self.myServerList.setParent(self)
            self.myServerList.setKeyPath(keyPath)
            self.myServerList.setDomain(self.myDomain)
    ...
    def handleTrxElementCreate(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagServerList)):
                if (self.myCreateServerListFunctor):
                    if (not self.myServerList):
                        for logFunc in self._log("handle-trx-element-create-serverlist-not-exist").debug3Func():
                            logFunc("handleTrxElementCreate(): serverlist not exists, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    
                    timeoutGuard = TimeoutGuard(self._log, '%s-create-serverlist-functor-%s' % (self.myKeyPath, phase), 
                                                self.getFunctorTimeoutForPhase(self.SERVERLIST_CREATE_FUNCTOR, phase),
                                                self.getFunctorMildTimeoutForPhase(self.SERVERLIST_CREATE_FUNCTOR, phase))
                    try:
                        res = self.myCreateServerListFunctor(phase, self.myServerList)
                    except:
                        for logFunc in self._log("handle-trx-element-create-serverlist-functor-exception").exceptionFunc():
                            logFunc("ServerList's create functor raised an exception. trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        raise
                    timeoutGuard.checkAndLog("application problem: functor=%s" % self.myCreateServerListFunctor.__name__)
                    if (res != ReturnCodes.kOk):
                        if (phase.getConfdPhase() == TrxPhase.kPrepare):
                            for logFunc in self._log("handle-trx-element-create-serverlist-functor-failed-prepare").noticeFunc():
                                logFunc("handleTrxElementCreate(): serverlist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                            self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        else:
                            for logFunc in self._log("handle-trx-element-create-serverlist-functor-failed").errorFunc():
                                logFunc("handleTrxElementCreate(): serverlist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    ...
    def notifyDescendantsPrepareBlinkyDelete(self):
        ...
        if (self.myServerList):
            self.myServerList.notifyDescendantsPrepareBlinkyDelete()
    ...
    def handleTrxElementDelete(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagServerList)):
                res = self.activateDeleteServerListFunctor(phase)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("handle-trx-element-delete-serverlist-functor-failed-prepare").noticeFunc():
                            logFunc("handleTrxElementDelete(): serverlist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("handle-trx-element-delete-serverlist-functor-failed").errorFunc():
                            logFunc("handleTrxElementDelete(): serverlist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                    return ReturnCodes.kGeneralError
    ...
    def removeServerList(self):
        self.myServerList = None

    def activateDeleteServerListFunctor(self, phase):
        for logFunc in self._log("activate-delete-serverlist-functor").debug3Func(): logFunc("activateDeleteServerListFunctor(): called, phase=%s", phase)
        if (self.myIsActive):
            if (self.myDeleteServerListFunctor):
                timeoutGuard = TimeoutGuard(self._log, '%s-delete-serverlist-functor-%s' % (self.myKeyPath, phase), 
                                            self.getFunctorTimeoutForPhase(self.SERVERLIST_DELETE_FUNCTOR, phase), 
                                            self.getFunctorMildTimeoutForPhase(self.SERVERLIST_DELETE_FUNCTOR, phase))
                try:
                    res = self.myDeleteServerListFunctor(phase)
                except:
                    for logFunc in self._log("activate-delete-serverlist-functor-exception").exceptionFunc():
                        logFunc("ServerList's delete functor raised an exception. phase=%s", phase)
                    raise
                timeoutGuard.checkAndLog("application problem: functor=%s" % self.myDeleteServerListFunctor.__name__)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("activate-delete-serverlist-functor-functor-failed-prepare").noticeFunc():
                            logFunc("activateDeleteServerListFunctor(): functor failed, phase=%s", phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("activate-delete-serverlist-functor-functor-failed").errorFunc():
                            logFunc("activateDeleteServerListFunctor(): functor failed, phase=%s", phase)
                    return ReturnCodes.kGeneralError
        return ReturnCodes.kOk
    ...
    def handleInternalDestroy(self, phase):
        ...
        if (phase.getConfdPhase() == TrxPhase.kPrepare):
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myServerList):
                    res = self.myServerList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-serverlist-recursion-failed-prepare-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): delete-serverlist-recursion-failed-prepare-blinky, phase=%s",
                                  phase)
                        return ReturnCodes.kGeneralError
                ...
                if (self.myServerList):
                    res = self.activateDeleteServerListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-serverlist-functor-failed-prepare").noticeFunc():
                            logFunc("handleInternalDestroy(): activateDeleteServerListFunctor() failed, res=%s, phase=%s",
                                   res, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        return ReturnCodes.kGeneralError
                if (self.myServerList):
                    res = self.myServerList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-serverlist-recursion-failed-prepare-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myServerList.handleInternalDestroy() failed, res=%s, phase=%s",
                                   res, phase)
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myServerList):
                    res = self.myServerList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-serverlist-recursion-failed-commit-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myServerList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("handle internal destroy failed in commit phase (blinky)")
                        return ReturnCodes.kGeneralError
                    self.removeServerList()
                ...
                if (self.myServerList):
                    res = self.myServerList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-serverlist-recursion-failed-commit-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myServerList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                    ...
                    res = self.activateDeleteServerListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-serverlist-functor-failed-commit").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteServerListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("activateDeleteServerListFunctor failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myServerList):
                    res = self.myServerList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-serverlist-recursion-failed-abort-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myServerList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myServerList):
                    res = self.myServerList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-serverlist-recursion-failed-abort-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myServerList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myServerList):
                    res = self.activateDeleteServerListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-serverlist-functor-failed-abort").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteServerListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_system.tech.system.name_resolution.dns.name_servers.ipv4.ipv4_data_gen import Ipv4Data", 
        ...
        "logModuleName": "a-sys-net-tech-system-tech-system-name-resolution-dns-name-servers-ipv4-blinky-ipv4-gen", 
        "importStatement": "from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.blinky_ipv4_gen import BlinkyIpv4", 
    ...
    "descendants": [
        {
            "className": "BlinkyServerList", 
            "memberName": "ServerList", 
            "yangName": "server", 
            "importStatement": "from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.server.blinky_server_list_gen import BlinkyServerList"
        }
    ], 

...
from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.ipv4.blinky_ipv4_gen import BlinkyIpv4




class BlinkyNameServers(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkynameservers').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyNameServers._validationPointId, BlinkyNameServers._actionPointId)
        ...
        newNode=BlinkyNameServers(logger)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkynameservers').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.blinky_name_servers_gen import BlinkyNameServers', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkynameservers-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.name_resolution.dns.name_servers.blinky_name_servers_gen import BlinkyNameServers', keyPathRegistered, keyPath, keyDepth)
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkynameservers-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkynameservers-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_system.tech.system.name_resolution.dns.name_servers.name_servers_data_gen import NameServersData", 
        "moduleYangNamespacePrefix": "qt-sys", 
        "validationPoint": null, 
        "yangName": "name-servers", 
        "namespace": "name_servers", 
        "logGroupName": "blinky-name-servers", 
        "className": "BlinkyNameServers", 
        "logModuleName": "a-sys-net-tech-system-tech-system-name-resolution-dns-name-servers-blinky-name-servers-gen", 

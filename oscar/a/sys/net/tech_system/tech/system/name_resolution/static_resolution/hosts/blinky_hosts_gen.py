...
from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.blinky_ipv4_list_gen import BlinkyIpv4List




class BlinkyHosts(BlinkyContainer):
    ...
    IPV4LIST_CREATE_FUNCTOR = 'IPV4LIST_CREATE_FUNCTOR'
    IPV4LIST_DELETE_FUNCTOR = 'IPV4LIST_DELETE_FUNCTOR'
    ...
    def __init__ (self, logger):
        ...
        self.myCreateIpv4ListFunctor=None
        self.myDeleteIpv4ListFunctor=None
        self.myIpv4List=None
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyhosts').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyHosts._validationPointId, BlinkyHosts._actionPointId)
    ...
    def setCreateIpv4ListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-create-ipv4-functor-active").errorFunc():
                logFunc("setCreateIpv4ListFunctor() is illegal when blinky node is active")
        self.myCreateIpv4ListFunctor = functor

    def setDeleteIpv4ListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-delete-ipv4-functor-active").errorFunc():
                logFunc("setDeleteIpv4ListFunctor() is illegal when blinky node is active")
        self.myDeleteIpv4ListFunctor = functor
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if (keyPath.isTagEqual(keyDepth_PBR.value(), self.ourNamespace, self.ourXmlTagIpv4List)):
            return self.myIpv4List
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyhosts').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.blinky_hosts_gen import BlinkyHosts', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        if not found:
            if keyPath.getLen() > keyDepth+1:
                if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagIpv4List)):
                    keyPathRegistered = BlinkyIpv4List.isKeyPathRegistered(logger, keyPath, keyDepth+1)
                    found = True
            else:
        ...
        logger('is-key-path-registered-blinkyhosts-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.blinky_hosts_gen import BlinkyHosts', keyPathRegistered, keyPath, keyDepth)
    ...
    def prepareMyBlinkyNode(self, keyPath, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagIpv4List)):
            if (self.myIpv4List):
                for logFunc in self._log("prepare-my-blinky-node-ipv4list-already-exists").errorFunc():
                    logFunc("prepareMyBlinkyNode(): already exists, keyPath=%s, keyDepth=%s", keyPath, keyDepth)
                return None
            self.myIpv4List = BlinkyIpv4List(self._log)
            self.myIpv4List.setParent(self)
            self.myIpv4List.setKeyPath(keyPath)
            self.myIpv4List.setDomain(self.myDomain)
    ...
    def handleTrxElementCreate(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagIpv4List)):
                if (self.myCreateIpv4ListFunctor):
                    if (not self.myIpv4List):
                        for logFunc in self._log("handle-trx-element-create-ipv4list-not-exist").debug3Func():
                            logFunc("handleTrxElementCreate(): ipv4list not exists, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    
                    timeoutGuard = TimeoutGuard(self._log, '%s-create-ipv4list-functor-%s' % (self.myKeyPath, phase), 
                                                self.getFunctorTimeoutForPhase(self.IPV4LIST_CREATE_FUNCTOR, phase),
                                                self.getFunctorMildTimeoutForPhase(self.IPV4LIST_CREATE_FUNCTOR, phase))
                    try:
                        res = self.myCreateIpv4ListFunctor(phase, self.myIpv4List)
                    except:
                        for logFunc in self._log("handle-trx-element-create-ipv4list-functor-exception").exceptionFunc():
                            logFunc("Ipv4List's create functor raised an exception. trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        raise
                    timeoutGuard.checkAndLog("application problem: functor=%s" % self.myCreateIpv4ListFunctor.__name__)
                    if (res != ReturnCodes.kOk):
                        if (phase.getConfdPhase() == TrxPhase.kPrepare):
                            for logFunc in self._log("handle-trx-element-create-ipv4list-functor-failed-prepare").noticeFunc():
                                logFunc("handleTrxElementCreate(): ipv4list functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                            self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        else:
                            for logFunc in self._log("handle-trx-element-create-ipv4list-functor-failed").errorFunc():
                                logFunc("handleTrxElementCreate(): ipv4list functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    ...
    def notifyDescendantsPrepareBlinkyDelete(self):
        ...
        if (self.myIpv4List):
            self.myIpv4List.notifyDescendantsPrepareBlinkyDelete()
    ...
    def handleTrxElementDelete(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagIpv4List)):
                res = self.activateDeleteIpv4ListFunctor(phase)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("handle-trx-element-delete-ipv4list-functor-failed-prepare").noticeFunc():
                            logFunc("handleTrxElementDelete(): ipv4list functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("handle-trx-element-delete-ipv4list-functor-failed").errorFunc():
                            logFunc("handleTrxElementDelete(): ipv4list functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                    return ReturnCodes.kGeneralError
    ...
    def removeIpv4List(self):
        self.myIpv4List = None

    def activateDeleteIpv4ListFunctor(self, phase):
        for logFunc in self._log("activate-delete-ipv4list-functor").debug3Func(): logFunc("activateDeleteIpv4ListFunctor(): called, phase=%s", phase)
        if (self.myIsActive):
            if (self.myDeleteIpv4ListFunctor):
                timeoutGuard = TimeoutGuard(self._log, '%s-delete-ipv4list-functor-%s' % (self.myKeyPath, phase), 
                                            self.getFunctorTimeoutForPhase(self.IPV4LIST_DELETE_FUNCTOR, phase), 
                                            self.getFunctorMildTimeoutForPhase(self.IPV4LIST_DELETE_FUNCTOR, phase))
                try:
                    res = self.myDeleteIpv4ListFunctor(phase)
                except:
                    for logFunc in self._log("activate-delete-ipv4list-functor-exception").exceptionFunc():
                        logFunc("Ipv4List's delete functor raised an exception. phase=%s", phase)
                    raise
                timeoutGuard.checkAndLog("application problem: functor=%s" % self.myDeleteIpv4ListFunctor.__name__)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("activate-delete-ipv4list-functor-functor-failed-prepare").noticeFunc():
                            logFunc("activateDeleteIpv4ListFunctor(): functor failed, phase=%s", phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("activate-delete-ipv4list-functor-functor-failed").errorFunc():
                            logFunc("activateDeleteIpv4ListFunctor(): functor failed, phase=%s", phase)
                    return ReturnCodes.kGeneralError
        return ReturnCodes.kOk
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyhosts-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyhosts-failed").errorFunc():
    ...
    def handleInternalDestroy(self, phase):
        ...
        if (phase.getConfdPhase() == TrxPhase.kPrepare):
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myIpv4List):
                    res = self.myIpv4List.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-ipv4list-recursion-failed-prepare-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): delete-ipv4list-recursion-failed-prepare-blinky, phase=%s",
                                  phase)
                        return ReturnCodes.kGeneralError
                ...
                if (self.myIpv4List):
                    res = self.activateDeleteIpv4ListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-ipv4list-functor-failed-prepare").noticeFunc():
                            logFunc("handleInternalDestroy(): activateDeleteIpv4ListFunctor() failed, res=%s, phase=%s",
                                   res, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        return ReturnCodes.kGeneralError
                if (self.myIpv4List):
                    res = self.myIpv4List.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-ipv4list-recursion-failed-prepare-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myIpv4List.handleInternalDestroy() failed, res=%s, phase=%s",
                                   res, phase)
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myIpv4List):
                    res = self.myIpv4List.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-ipv4list-recursion-failed-commit-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myIpv4List.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myIpv4List):
                    res = self.myIpv4List.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-ipv4list-recursion-failed-commit-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myIpv4List.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                    ...
                    res = self.activateDeleteIpv4ListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-ipv4list-functor-failed-commit").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteIpv4ListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("activateDeleteIpv4ListFunctor failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myIpv4List):
                    res = self.myIpv4List.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-ipv4list-recursion-failed-abort-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myIpv4List.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myIpv4List):
                    res = self.myIpv4List.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-ipv4list-recursion-failed-abort-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myIpv4List.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myIpv4List):
                    res = self.activateDeleteIpv4ListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-ipv4list-functor-failed-abort").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteIpv4ListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_system.tech.system.name_resolution.static_resolution.hosts.hosts_data_gen import HostsData", 
        "moduleYangNamespacePrefix": "qt-sys", 
        "validationPoint": null, 
        "yangName": "hosts", 
        "namespace": "hosts", 
        "logGroupName": "blinky-hosts", 
        "className": "BlinkyHosts", 
        "logModuleName": "a-sys-net-tech-system-tech-system-name-resolution-static-resolution-hosts-blinky-hosts-gen", 
    ...
    "descendants": [
        {
            "className": "BlinkyIpv4List", 
            "memberName": "Ipv4List", 
            "yangName": "ipv4", 
            "importStatement": "from a.sys.net.tech_system.tech.system.name_resolution.static_resolution.hosts.ipv4.blinky_ipv4_list_gen import BlinkyIpv4List"
        }
    ], 

...
from a.api.yang.modules.tech.common.qwilt_tech_system.tech.system.system_data_gen import SystemData


from a.sys.net.tech_system.tech.system.system_defaults.blinky_system_defaults_gen import BlinkySystemDefaults

from a.sys.net.tech_system.tech.system.name_resolution.blinky_name_resolution_gen import BlinkyNameResolution




class BlinkySystem(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/device/tech/qwilt-tech-system"
    #leaves
    
    ourXmlTagHostname="hostname"
    ...
    ourXmlTagNameResolution="name-resolution"
    ...
    NAMERESOLUTION_CREATE_FUNCTOR = 'NAMERESOLUTION_CREATE_FUNCTOR'
    NAMERESOLUTION_DELETE_FUNCTOR = 'NAMERESOLUTION_DELETE_FUNCTOR'
    ...
    def __init__ (self, logger):
        ...
        self.myCreateNameResolutionFunctor=None
        self.myDeleteNameResolutionFunctor=None
        self.myNameResolution=None
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkysystem').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkySystem._validationPointId, BlinkySystem._actionPointId)
    ...
    def setCreateNameResolutionFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-create-name-resolution-functor-active").errorFunc():
                logFunc("setCreateNameResolutionFunctor() is illegal when blinky node is active")
        self.myCreateNameResolutionFunctor = functor

    def setDeleteNameResolutionFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-delete-name-resolution-functor-active").errorFunc():
                logFunc("setDeleteNameResolutionFunctor() is illegal when blinky node is active")
        self.myDeleteNameResolutionFunctor = functor
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagHostname)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.hostname = newValue.asString()
                self.myCandidateData.setHasHostname()
                
            else:
                self.setHostnameDefaultValue(True)
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if (keyPath.isTagEqual(keyDepth_PBR.value(), self.ourNamespace, self.ourXmlTagNameResolution)):
            return self.myNameResolution
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkysystem').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.blinky_system_gen import BlinkySystem', keyPath, keyDepth)
        ...
        if not found:
            if keyPath.getLen() > keyDepth+1:
                if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagNameResolution)):
                    keyPathRegistered = BlinkyNameResolution.isKeyPathRegistered(logger, keyPath, keyDepth+1)
                    found = True
            else:
                keyPathRegistered = True
                found = True
        ...
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagHostname)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkysystem-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.net.tech_system.tech.system.blinky_system_gen import BlinkySystem', keyPathRegistered, keyPath, keyDepth)
    ...
    def prepareMyBlinkyNode(self, keyPath, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagNameResolution)):
            if (self.myNameResolution):
                for logFunc in self._log("prepare-my-blinky-node-nameresolution-already-exists").errorFunc():
                    logFunc("prepareMyBlinkyNode(): already exists, keyPath=%s, keyDepth=%s", keyPath, keyDepth)
                return None
            self.myNameResolution = BlinkyNameResolution(self._log)
            self.myNameResolution.setParent(self)
            self.myNameResolution.setKeyPath(keyPath)
            self.myNameResolution.setDomain(self.myDomain)
    ...
    def handleTrxElementCreate(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagNameResolution)):
                if (self.myCreateNameResolutionFunctor):
                    if (not self.myNameResolution):
                        for logFunc in self._log("handle-trx-element-create-nameresolution-not-exist").debug3Func():
                            logFunc("handleTrxElementCreate(): nameresolution not exists, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    
                    timeoutGuard = TimeoutGuard(self._log, '%s-create-nameresolution-functor-%s' % (self.myKeyPath, phase), 
                                                self.getFunctorTimeoutForPhase(self.NAMERESOLUTION_CREATE_FUNCTOR, phase),
                                                self.getFunctorMildTimeoutForPhase(self.NAMERESOLUTION_CREATE_FUNCTOR, phase))
                    try:
                        res = self.myCreateNameResolutionFunctor(phase, self.myNameResolution)
                    except:
                        for logFunc in self._log("handle-trx-element-create-nameresolution-functor-exception").exceptionFunc():
                            logFunc("NameResolution's create functor raised an exception. trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        raise
                    timeoutGuard.checkAndLog("application problem: functor=%s" % self.myCreateNameResolutionFunctor.__name__)
                    if (res != ReturnCodes.kOk):
                        if (phase.getConfdPhase() == TrxPhase.kPrepare):
                            for logFunc in self._log("handle-trx-element-create-nameresolution-functor-failed-prepare").noticeFunc():
                                logFunc("handleTrxElementCreate(): nameresolution functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                            self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        else:
                            for logFunc in self._log("handle-trx-element-create-nameresolution-functor-failed").errorFunc():
                                logFunc("handleTrxElementCreate(): nameresolution functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    ...
    def notifyDescendantsPrepareBlinkyDelete(self):
        ...
        if (self.myNameResolution):
            self.myNameResolution.notifyDescendantsPrepareBlinkyDelete()
    ...
    def handleTrxElementDelete(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagNameResolution)):
                res = self.activateDeleteNameResolutionFunctor(phase)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("handle-trx-element-delete-nameresolution-functor-failed-prepare").noticeFunc():
                            logFunc("handleTrxElementDelete(): nameresolution functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("handle-trx-element-delete-nameresolution-functor-failed").errorFunc():
                            logFunc("handleTrxElementDelete(): nameresolution functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                    return ReturnCodes.kGeneralError
    ...
    def removeNameResolution(self):
        self.myNameResolution = None

    def activateDeleteNameResolutionFunctor(self, phase):
        for logFunc in self._log("activate-delete-nameresolution-functor").debug3Func(): logFunc("activateDeleteNameResolutionFunctor(): called, phase=%s", phase)
        if (self.myIsActive):
            if (self.myDeleteNameResolutionFunctor):
                timeoutGuard = TimeoutGuard(self._log, '%s-delete-nameresolution-functor-%s' % (self.myKeyPath, phase), 
                                            self.getFunctorTimeoutForPhase(self.NAMERESOLUTION_DELETE_FUNCTOR, phase), 
                                            self.getFunctorMildTimeoutForPhase(self.NAMERESOLUTION_DELETE_FUNCTOR, phase))
                try:
                    res = self.myDeleteNameResolutionFunctor(phase)
                except:
                    for logFunc in self._log("activate-delete-nameresolution-functor-exception").exceptionFunc():
                        logFunc("NameResolution's delete functor raised an exception. phase=%s", phase)
                    raise
                timeoutGuard.checkAndLog("application problem: functor=%s" % self.myDeleteNameResolutionFunctor.__name__)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("activate-delete-nameresolution-functor-functor-failed-prepare").noticeFunc():
                            logFunc("activateDeleteNameResolutionFunctor(): functor failed, phase=%s", phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("activate-delete-nameresolution-functor-functor-failed").errorFunc():
                            logFunc("activateDeleteNameResolutionFunctor(): functor failed, phase=%s", phase)
                    return ReturnCodes.kGeneralError
        return ReturnCodes.kOk
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = SystemData()
        
        res = self.setHostnameDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-hostname-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setHostnameDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setHostnameDefaultValue (self, setHas):
        for logFunc in self._log("set-hostname-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.hostname = ""
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkysystem-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkysystem-failed").errorFunc():
    ...
    def handleInternalDestroy(self, phase):
        ...
        if (phase.getConfdPhase() == TrxPhase.kPrepare):
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myNameResolution):
                    res = self.myNameResolution.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-nameresolution-recursion-failed-prepare-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): delete-nameresolution-recursion-failed-prepare-blinky, phase=%s",
                                  phase)
                        return ReturnCodes.kGeneralError
                ...
                if (self.myNameResolution):
                    res = self.activateDeleteNameResolutionFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-nameresolution-functor-failed-prepare").noticeFunc():
                            logFunc("handleInternalDestroy(): activateDeleteNameResolutionFunctor() failed, res=%s, phase=%s",
                                   res, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        return ReturnCodes.kGeneralError
                if (self.myNameResolution):
                    res = self.myNameResolution.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-nameresolution-recursion-failed-prepare-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myNameResolution.handleInternalDestroy() failed, res=%s, phase=%s",
                                   res, phase)
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myNameResolution):
                    res = self.myNameResolution.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-nameresolution-recursion-failed-commit-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myNameResolution.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("handle internal destroy failed in commit phase (blinky)")
                        return ReturnCodes.kGeneralError
                    self.removeNameResolution()
                ...
                if (self.myNameResolution):
                    res = self.myNameResolution.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-nameresolution-recursion-failed-commit-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myNameResolution.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("handle internal destroy failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
                    res = self.activateDeleteNameResolutionFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-nameresolution-functor-failed-commit").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteNameResolutionFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("activateDeleteNameResolutionFunctor failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myNameResolution):
                    res = self.myNameResolution.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-nameresolution-recursion-failed-abort-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myNameResolution.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("internal destroy failed in abort (blinky)")
                        return ReturnCodes.kOk
                ...
                if (self.myNameResolution):
                    res = self.myNameResolution.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-nameresolution-recursion-failed-abort-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myNameResolution.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("internal destroy failed in abort (private/public)")
                        return ReturnCodes.kOk
                if (self.myNameResolution):
                    res = self.activateDeleteNameResolutionFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-nameresolution-functor-failed-abort").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteNameResolutionFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("delete functor failed in abort (blinky)")
                        return ReturnCodes.kOk
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_system.tech.system.system_data_gen import SystemData", 
        "moduleYangNamespacePrefix": "qt-sys", 
        "validationPoint": null, 
        "yangName": "system", 
        "namespace": "system", 
        "logGroupName": "blinky-system", 
        "className": "BlinkySystem", 
        "logModuleName": "a-sys-net-tech-system-tech-system-blinky-system-gen", 
        "importStatement": "from a.sys.net.tech_system.tech.system.blinky_system_gen import BlinkySystem", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system", 
        "dataClassName": "SystemData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {
    ...
    "descendants": [
        {
            ...
            "importStatement": "from a.sys.net.tech_system.tech.system.system_defaults.blinky_system_defaults_gen import BlinkySystemDefaults"
        }, 
        {

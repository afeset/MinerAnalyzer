...
class BlinkyOperStatus(BlinkyOperNode):
    ...
    _kCallpointName = "tech-system-clock-status-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "tech-system-clock-status-callpoint"
    ...
    def getOperRelativePath (self, operRelativePath):
        ...
        val = Value()
        val.setXmlTag(("status", "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-clock", "qt-sys-clock"))
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-clock", "local-time-string"):
                    for logFunc in self._log("set-oper-data-requested-fields-localtimestring-requested").debug3Func(): logFunc(
                        "local-time-string requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setLocalTimeStringRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-clock", "utc-time-string"):
                    for logFunc in self._log("set-oper-data-requested-fields-utctimestring-requested").debug3Func(): logFunc(
                        "utc-time-string requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setUtcTimeStringRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-clock", "daylight-saving-time"):
                    for logFunc in self._log("set-oper-data-requested-fields-daylightsavingtime-requested").debug3Func(): logFunc(
                        "daylight-saving-time requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setDaylightSavingTimeRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-clock", "epoch"):
                    for logFunc in self._log("set-oper-data-requested-fields-epoch-requested").debug3Func(): logFunc(
                        "epoch requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setEpochRequested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-clock", "utc-offset-minutes"):
                    for logFunc in self._log("set-oper-data-requested-fields-utcoffsetminutes-requested").debug3Func(): logFunc(
                        "utc-offset-minutes requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setUtcOffsetMinutesRequested()
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-clock", "local-time-string"):
            if operData.isLocalTimeStringRequested():
                 if operData.hasLocalTimeString():
                     value.setString(operData.localTimeString)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-clock", "utc-time-string"):
            if operData.isUtcTimeStringRequested():
                 if operData.hasUtcTimeString():
                     value.setString(operData.utcTimeString)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-clock", "daylight-saving-time"):
            if operData.isDaylightSavingTimeRequested():
                 if operData.hasDaylightSavingTime():
                     value.setBool(operData.daylightSavingTime)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-clock", "epoch"):
            if operData.isEpochRequested():
                 if operData.hasEpoch():
                     value.setInt64(operData.epoch)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/ns/yang/device/tech/qwilt-tech-system-clock", "utc-offset-minutes"):
            if operData.isUtcOffsetMinutesRequested():
                 if operData.hasUtcOffsetMinutes():
                     value.setInt64(operData.utcOffsetMinutes)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.tech.common.qwilt_tech_system_clock.tech.system.clock.status.status_oper_data_gen import StatusOperData", 
        ...
        "logModuleName": "a-sys-clock-tech-system-clock-tech-system-clock-status-blinky-status-oper-gen", 
        "importStatement": "from a.sys.clock.tech_system_clock.tech.system.clock.status.blinky_status_oper_gen import BlinkyOperStatus", 
        "callpointName": "tech-system-clock-status-callpoint", 

...
class BlinkyBlinkyValidation(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/blinky/validation"
    #leaves
    ...
    _validationPointId="blinky-validation-validation-point"
    _actionPointId=None
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyblinkyvalidation').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyBlinkyValidation._validationPointId, BlinkyBlinkyValidation._actionPointId)

        confd_key=KeyPath()
        
        
        xmlVal = Value()
        xmlVal.setXmlTag(("blinky-validation", "http://qwilt.com/ns/blinky/validation", "blinky-val"))
        confd_key.addKeyPathPrefix(xmlVal)
        
        newNode=BlinkyBlinkyValidation(logger)
    ...
    def setValidateTrxFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-validate-trx-functor-active").errorFunc():
                logFunc("illegal when blinky node is active")
        self.validateTrxFunctor = functor
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyblinkyvalidation').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.config_validation.validation.blinky_validation.blinky_blinky_validation_gen import BlinkyBlinkyValidation', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        logger('is-key-path-registered-blinkyblinkyvalidation-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.config_validation.validation.blinky_validation.blinky_blinky_validation_gen import BlinkyBlinkyValidation', keyPathRegistered, keyPath, keyDepth)
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyblinkyvalidation-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyblinkyvalidation-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.api.yang.modules.sys.blinky_validation.blinky_validation.blinky_validation.blinky_validation_data_gen import BlinkyValidationData", 
        "moduleYangNamespacePrefix": "blinky-val", 
        "validationPoint": "blinky-validation-validation-point", 
        "yangName": "blinky-validation", 
        "namespace": "blinky_validation", 
        "logGroupName": "blinky-blinky-validation", 
        "className": "BlinkyBlinkyValidation", 
        "logModuleName": "a-sys-blinky-config-validation-validation-blinky-validation-blinky-blinky-validation-gen", 
        "importStatement": "from a.sys.blinky.config_validation.validation.blinky_validation.blinky_blinky_validation_gen import BlinkyBlinkyValidation", 
        "moduleYangNamespace": "http://qwilt.com/ns/blinky/validation", 
        "dataClassName": "BlinkyValidationData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

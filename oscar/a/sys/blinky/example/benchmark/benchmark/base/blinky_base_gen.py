...
from a.sys.blinky.example.benchmark.benchmark.base.lll.blinky_lll_list_gen import BlinkyLllList




class BlinkyBase(BlinkyContainer):
    ...
    LLLLIST_CREATE_FUNCTOR = 'LLLLIST_CREATE_FUNCTOR'
    LLLLIST_DELETE_FUNCTOR = 'LLLLIST_DELETE_FUNCTOR'
    ...
    def __init__ (self, logger):
        ...
        self.myCreateLllListFunctor=None
        self.myDeleteLllListFunctor=None
        self.myLllList=None
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkybase').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyBase._validationPointId, BlinkyBase._actionPointId)
    ...
    def setCreateLllListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-create-lll-functor-active").errorFunc():
                logFunc("setCreateLllListFunctor() is illegal when blinky node is active")
        self.myCreateLllListFunctor = functor

    def setDeleteLllListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-delete-lll-functor-active").errorFunc():
                logFunc("setDeleteLllListFunctor() is illegal when blinky node is active")
        self.myDeleteLllListFunctor = functor
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if (keyPath.isTagEqual(keyDepth_PBR.value(), self.ourNamespace, self.ourXmlTagLllList)):
            return self.myLllList
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkybase').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.benchmark.benchmark.base.blinky_base_gen import BlinkyBase', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        if not found:
            if keyPath.getLen() > keyDepth+1:
                if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagLllList)):
                    keyPathRegistered = BlinkyLllList.isKeyPathRegistered(logger, keyPath, keyDepth+1)
                    found = True
            else:
        ...
        logger('is-key-path-registered-blinkybase-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.benchmark.benchmark.base.blinky_base_gen import BlinkyBase', keyPathRegistered, keyPath, keyDepth)
    ...
    def prepareMyBlinkyNode(self, keyPath, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagLllList)):
            if (self.myLllList):
                for logFunc in self._log("prepare-my-blinky-node-llllist-already-exists").errorFunc():
                    logFunc("prepareMyBlinkyNode(): already exists, keyPath=%s, keyDepth=%s", keyPath, keyDepth)
                return None
            self.myLllList = BlinkyLllList(self._log)
            self.myLllList.setParent(self)
            self.myLllList.setKeyPath(keyPath)
            self.myLllList.setDomain(self.myDomain)
    ...
    def handleTrxElementCreate(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagLllList)):
                if (self.myCreateLllListFunctor):
                    if (not self.myLllList):
                        for logFunc in self._log("handle-trx-element-create-llllist-not-exist").debug3Func():
                            logFunc("handleTrxElementCreate(): llllist not exists, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    
                    timeoutGuard = TimeoutGuard(self._log, '%s-create-llllist-functor-%s' % (self.myKeyPath, phase), 
                                                self.getFunctorTimeoutForPhase(self.LLLLIST_CREATE_FUNCTOR, phase),
                                                self.getFunctorMildTimeoutForPhase(self.LLLLIST_CREATE_FUNCTOR, phase))
                    try:
                        res = self.myCreateLllListFunctor(phase, self.myLllList)
                    except:
                        for logFunc in self._log("handle-trx-element-create-llllist-functor-exception").exceptionFunc():
                            logFunc("LllList's create functor raised an exception. trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        raise
                    timeoutGuard.checkAndLog("application problem: functor=%s" % self.myCreateLllListFunctor.__name__)
                    if (res != ReturnCodes.kOk):
                        if (phase.getConfdPhase() == TrxPhase.kPrepare):
                            for logFunc in self._log("handle-trx-element-create-llllist-functor-failed-prepare").noticeFunc():
                                logFunc("handleTrxElementCreate(): llllist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                            self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        else:
                            for logFunc in self._log("handle-trx-element-create-llllist-functor-failed").errorFunc():
                                logFunc("handleTrxElementCreate(): llllist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    ...
    def notifyDescendantsPrepareBlinkyDelete(self):
        ...
        if (self.myLllList):
            self.myLllList.notifyDescendantsPrepareBlinkyDelete()
    ...
    def handleTrxElementDelete(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagLllList)):
                res = self.activateDeleteLllListFunctor(phase)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("handle-trx-element-delete-llllist-functor-failed-prepare").noticeFunc():
                            logFunc("handleTrxElementDelete(): llllist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("handle-trx-element-delete-llllist-functor-failed").errorFunc():
                            logFunc("handleTrxElementDelete(): llllist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                    return ReturnCodes.kGeneralError
    ...
    def removeLllList(self):
        self.myLllList = None

    def activateDeleteLllListFunctor(self, phase):
        for logFunc in self._log("activate-delete-llllist-functor").debug3Func(): logFunc("activateDeleteLllListFunctor(): called, phase=%s", phase)
        if (self.myIsActive):
            if (self.myDeleteLllListFunctor):
                timeoutGuard = TimeoutGuard(self._log, '%s-delete-llllist-functor-%s' % (self.myKeyPath, phase), 
                                            self.getFunctorTimeoutForPhase(self.LLLLIST_DELETE_FUNCTOR, phase), 
                                            self.getFunctorMildTimeoutForPhase(self.LLLLIST_DELETE_FUNCTOR, phase))
                try:
                    res = self.myDeleteLllListFunctor(phase)
                except:
                    for logFunc in self._log("activate-delete-llllist-functor-exception").exceptionFunc():
                        logFunc("LllList's delete functor raised an exception. phase=%s", phase)
                    raise
                timeoutGuard.checkAndLog("application problem: functor=%s" % self.myDeleteLllListFunctor.__name__)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("activate-delete-llllist-functor-functor-failed-prepare").noticeFunc():
                            logFunc("activateDeleteLllListFunctor(): functor failed, phase=%s", phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("activate-delete-llllist-functor-functor-failed").errorFunc():
                            logFunc("activateDeleteLllListFunctor(): functor failed, phase=%s", phase)
                    return ReturnCodes.kGeneralError
        return ReturnCodes.kOk
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkybase-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkybase-failed").errorFunc():
    ...
    def handleInternalDestroy(self, phase):
        ...
        if (phase.getConfdPhase() == TrxPhase.kPrepare):
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myLllList):
                    res = self.myLllList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-llllist-recursion-failed-prepare-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): delete-llllist-recursion-failed-prepare-blinky, phase=%s",
                                  phase)
                        return ReturnCodes.kGeneralError
                ...
                if (self.myLllList):
                    res = self.activateDeleteLllListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-llllist-functor-failed-prepare").noticeFunc():
                            logFunc("handleInternalDestroy(): activateDeleteLllListFunctor() failed, res=%s, phase=%s",
                                   res, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        return ReturnCodes.kGeneralError
                if (self.myLllList):
                    res = self.myLllList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-llllist-recursion-failed-prepare-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myLllList.handleInternalDestroy() failed, res=%s, phase=%s",
                                   res, phase)
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myLllList):
                    res = self.myLllList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-llllist-recursion-failed-commit-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myLllList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myLllList):
                    res = self.myLllList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-llllist-recursion-failed-commit-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myLllList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                    ...
                    res = self.activateDeleteLllListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-llllist-functor-failed-commit").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteLllListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("activateDeleteLllListFunctor failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myLllList):
                    res = self.myLllList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-llllist-recursion-failed-abort-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myLllList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myLllList):
                    res = self.myLllList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-llllist-recursion-failed-abort-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myLllList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myLllList):
                    res = self.activateDeleteLllListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-llllist-functor-failed-abort").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteLllListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.benchmark.benchmark.base.base_data_gen import BaseData", 
        "moduleYangNamespacePrefix": "bnch", 
        "validationPoint": null, 
        "yangName": "base", 
        "namespace": "base", 
        "logGroupName": "blinky-base", 
        "className": "BlinkyBase", 
        "logModuleName": "a-sys-blinky-example-benchmark-benchmark-base-blinky-base-gen", 
        "importStatement": "from a.sys.blinky.example.benchmark.benchmark.base.blinky_base_gen import BlinkyBase", 
        "moduleYangNamespace": "http://qwilt.com/model/benchmark", 
        "dataClassName": "BaseData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {
    ...
    "descendants": [
        {
            "className": "BlinkyLllList", 
            "memberName": "LllList", 
            "yangName": "lll", 
            "importStatement": "from a.sys.blinky.example.benchmark.benchmark.base.lll.blinky_lll_list_gen import BlinkyLllList"
        }
    ], 

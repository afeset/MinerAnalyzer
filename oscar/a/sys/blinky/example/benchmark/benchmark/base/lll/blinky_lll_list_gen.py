...
from a.sys.blinky.example.benchmark.benchmark.base.lll.blinky_lll_gen import BlinkyLll

class BlinkyLllList(BlinkyList):
    ...
    def __str__ (self):
        ret="{BlinkyLllList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyllllist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.benchmark.benchmark.base.lll.blinky_lll_gen import BlinkyLll', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyLll.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyllllist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.benchmark.benchmark.base.lll.blinky_lll_gen import BlinkyLll', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-lll-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-lll-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-lll").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyLllList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-lll").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyLll. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyLll(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-lll-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyLll not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-lll").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyLll. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyLll(self, key):
        for logFunc in self._log("get-blinky-blinky-lll").debug1Func(): logFunc("getBlinkyLll(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyLll(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-lll-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyLll-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyLll(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyLll", 
        "containerLogGroupName": "blinky-lll", 
        "namespace": "lll", 
        "containerImportStatement": "from a.sys.blinky.example.benchmark.benchmark.base.lll.blinky_lll_gen import BlinkyLll", 
        "logGroupName": "blinky-lll-list", 
        "className": "BlinkyLllList", 
        "logModuleName": "a-sys-blinky-example-benchmark-benchmark-base-lll-blinky-lll-list-gen", 
        "keyLeaf": {
            "varName": "lll", 

...
from a.sys.blinky.example.benchmark.benchmark.base.lll.ccc.ccc_data_gen import CccData





class BlinkyCcc(BlinkyContainer):
    ourNamespace="http://qwilt.com/model/benchmark"
    #leaves
    
    ourXmlTagC9str="c9str"
    
    ourXmlTagC2str="c2str"
    
    ourXmlTagC8str="c8str"
    
    ourXmlTagC6str="c6str"
    
    ourXmlTagC5str="c5str"
    
    ourXmlTagC1str="c1str"
    
    ourXmlTagC7str="c7str"
    
    ourXmlTagC10str="c10str"
    
    ourXmlTagC3str="c3str"
    
    ourXmlTagC4str="c4str"
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagC9str)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.c9str = newValue.asString()
                self.myCandidateData.setHasC9str()
                
            else:
                self.setC9strDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagC2str)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.c2str = newValue.asString()
                self.myCandidateData.setHasC2str()
                
            else:
                self.setC2strDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagC8str)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.c8str = newValue.asString()
                self.myCandidateData.setHasC8str()
                
            else:
                self.setC8strDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagC6str)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.c6str = newValue.asString()
                self.myCandidateData.setHasC6str()
                
            else:
                self.setC6strDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagC5str)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.c5str = newValue.asString()
                self.myCandidateData.setHasC5str()
                
            else:
                self.setC5strDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagC1str)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.c1str = newValue.asString()
                self.myCandidateData.setHasC1str()
                
            else:
                self.setC1strDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagC7str)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.c7str = newValue.asString()
                self.myCandidateData.setHasC7str()
                
            else:
                self.setC7strDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagC10str)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.c10str = newValue.asString()
                self.myCandidateData.setHasC10str()
                
            else:
                self.setC10strDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagC3str)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.c3str = newValue.asString()
                self.myCandidateData.setHasC3str()
                
            else:
                self.setC3strDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagC4str)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.c4str = newValue.asString()
                self.myCandidateData.setHasC4str()
                
            else:
                self.setC4strDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyccc').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.benchmark.benchmark.base.lll.ccc.blinky_ccc_gen import BlinkyCcc', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagC9str)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagC2str)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagC8str)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagC6str)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagC5str)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagC1str)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagC7str)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagC10str)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagC3str)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagC4str)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkyccc-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.benchmark.benchmark.base.lll.ccc.blinky_ccc_gen import BlinkyCcc', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = CccData()
        
        res = self.setC9strDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-c9str-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setC9strDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setC2strDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-c2str-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setC2strDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setC8strDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-c8str-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setC8strDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setC6strDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-c6str-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setC6strDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setC5strDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-c5str-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setC5strDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setC1strDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-c1str-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setC1strDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setC7strDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-c7str-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setC7strDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setC10strDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-c10str-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setC10strDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setC3strDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-c3str-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setC3strDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setC4strDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-c4str-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setC4strDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setC9strDefaultValue (self, setHas):
        for logFunc in self._log("set-c9str-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.c9str = ""
    ...
    def setC2strDefaultValue (self, setHas):
        for logFunc in self._log("set-c2str-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.c2str = ""
    ...
    def setC8strDefaultValue (self, setHas):
        for logFunc in self._log("set-c8str-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.c8str = ""
    ...
    def setC6strDefaultValue (self, setHas):
        for logFunc in self._log("set-c6str-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.c6str = ""
    ...
    def setC5strDefaultValue (self, setHas):
        for logFunc in self._log("set-c5str-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.c5str = ""
    ...
    def setC1strDefaultValue (self, setHas):
        for logFunc in self._log("set-c1str-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.c1str = ""
    ...
    def setC7strDefaultValue (self, setHas):
        for logFunc in self._log("set-c7str-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.c7str = ""
    ...
    def setC10strDefaultValue (self, setHas):
        for logFunc in self._log("set-c10str-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.c10str = ""
    ...
    def setC3strDefaultValue (self, setHas):
        for logFunc in self._log("set-c3str-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.c3str = ""
    ...
    def setC4strDefaultValue (self, setHas):
        for logFunc in self._log("set-c4str-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.c4str = ""
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyccc-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyccc-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.benchmark.benchmark.base.lll.ccc.ccc_data_gen import CccData", 
        "moduleYangNamespacePrefix": "bnch", 
        "validationPoint": null, 
        "yangName": "ccc", 
        "namespace": "ccc", 
        "logGroupName": "blinky-ccc", 
        "className": "BlinkyCcc", 
        "logModuleName": "a-sys-blinky-example-benchmark-benchmark-base-lll-ccc-blinky-ccc-gen", 

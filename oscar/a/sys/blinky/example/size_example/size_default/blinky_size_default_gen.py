...
from a.sys.blinky.example.lake_example.size_default.size_default_data_gen import SizeDefaultData





class BlinkySizeDefault(BlinkyContainer):
    ourNamespace="http://qwilt.com/model/lake-example"
    #leaves
    
    ourXmlTagWidth="width"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkysizedefault').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkySizeDefault._validationPointId, BlinkySizeDefault._actionPointId)

        confd_key=KeyPath()
        
        
        xmlVal = Value()
        xmlVal.setXmlTag(("size-default", "http://qwilt.com/model/lake-example", "lake-example"))
        confd_key.addKeyPathPrefix(xmlVal)
        
        newNode=BlinkySizeDefault(logger)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagWidth)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.width = newValue.asInt64()
                self.myCandidateData.setHasWidth()
                
            else:
                self.setWidthDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkysizedefault').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.size_example.size_default.blinky_size_default_gen import BlinkySizeDefault', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagWidth)):
        ...
        logger('is-key-path-registered-blinkysizedefault-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.size_example.size_default.blinky_size_default_gen import BlinkySizeDefault', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = SizeDefaultData()
        
        res = self.setWidthDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-width-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setWidthDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
    ...
    def setWidthDefaultValue (self, setHas):
        for logFunc in self._log("set-width-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.width = 30
    ...
    def setLengthDefaultValue (self, setHas):
        ...
        self.myCandidateData.length = 20
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = SizeDefaultData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkysizedefault-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkysizedefault-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.lake_example.size_default.size_default_data_gen import SizeDefaultData", 
        "moduleYangNamespacePrefix": "lake-example", 
        "validationPoint": null, 
        "yangName": "size-default", 
        "namespace": "size_default", 
        "logGroupName": "blinky-size-default", 
        "className": "BlinkySizeDefault", 
        "logModuleName": "a-sys-blinky-example-size-example-size-default-blinky-size-default-gen", 
        "importStatement": "from a.sys.blinky.example.size_example.size_default.blinky_size_default_gen import BlinkySizeDefault", 
        "moduleYangNamespace": "http://qwilt.com/model/lake-example", 
        "dataClassName": "SizeDefaultData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

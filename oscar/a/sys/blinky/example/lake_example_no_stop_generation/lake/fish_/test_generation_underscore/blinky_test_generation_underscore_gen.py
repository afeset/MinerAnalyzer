...
from a.sys.blinky.example.lake_example.lake.fish_.test_generation_underscore.test_generation_underscore_data_gen import TestGenerationUnderscoreData





class BlinkyTestGenerationUnderscore(BlinkyContainer):
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = TestGenerationUnderscoreData()
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = TestGenerationUnderscoreData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkytestgenerationunderscore-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkytestgenerationunderscore-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.lake_example.lake.fish_.test_generation_underscore.test_generation_underscore_data_gen import TestGenerationUnderscoreData", 
        "moduleYangNamespacePrefix": "lake-example", 
        "validationPoint": null, 
        "yangName": "test-generation_underscore", 
        "namespace": "test_generation_underscore", 
        "logGroupName": "blinky-test-generation-underscore", 
        "className": "BlinkyTestGenerationUnderscore", 
        "logModuleName": "a-sys-blinky-example-lake-example-no-stop-generation-lake-fish--test-generation-underscore-blinky-test-generation-underscore-gen", 
        "importStatement": "from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.test_generation_underscore.blinky_test_generation_underscore_gen import BlinkyTestGenerationUnderscore", 
        "moduleYangNamespace": "http://qwilt.com/model/lake-example", 
        "dataClassName": "TestGenerationUnderscoreData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

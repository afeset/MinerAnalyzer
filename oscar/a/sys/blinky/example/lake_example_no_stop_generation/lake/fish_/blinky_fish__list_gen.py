...
from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.blinky_fish__gen import BlinkyFish

class BlinkyFishList(BlinkyList):
    ...
    def __str__ (self):
        ret="{BlinkyFishList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyfishlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.blinky_fish__gen import BlinkyFish', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyFish.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyfishlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.blinky_fish__gen import BlinkyFish', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-fish-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-fish-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-fish-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-fish").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyFishList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-fish").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyFish. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyFish(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-fish-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyFish not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-fish").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyFish. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyFish(self, key):
        for logFunc in self._log("get-blinky-blinky-fish").debug1Func(): logFunc("getBlinkyFish(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyFish(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkyFish(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-fish-moved-failed").errorFunc(): logFunc("getBlinkyFish(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkyFish(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-fish-after-failed").errorFunc(): logFunc("getBlinkyFish(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-fish-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyFish-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyFish(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyFish", 
        "containerLogGroupName": "blinky-fish", 
        "namespace": "fish_", 
        "containerImportStatement": "from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.blinky_fish__gen import BlinkyFish", 
        "logGroupName": "blinky-fish-list", 
        "className": "BlinkyFishList", 
        "logModuleName": "a-sys-blinky-example-lake-example-no-stop-generation-lake-fish--blinky-fish--list-gen", 
        "keyLeaf": {
            "varName": "fish_", 

...
from a.sys.blinky.example.lake_example.lake.fish_.mood.mood_oper_data_gen import MoodOperData
...
class BlinkyOperMood(BlinkyOperNode):

    _kCallpointName = "lake-fish-mood-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "lake-fish-mood-callpoint"
    ...
    def getOperRelativePath (self, operRelativePath):
        ...
        val = Value()
        val.setXmlTag(("mood", "http://qwilt.com/model/lake-example", "lake-example"))
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/model/lake-example", "current"):
                    for logFunc in self._log("set-oper-data-requested-fields-current-requested").debug3Func(): logFunc(
                        "current requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setCurrentRequested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isCurrentRequested() and operData.hasCurrent():
            val = Value()
            val.setString(operData.current)
            tagValueList.push(("current", "http://qwilt.com/model/lake-example"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/model/lake-example", "current"):
            if operData.isCurrentRequested():
                 if operData.hasCurrent():
                     value.setString(operData.current)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.lake_example.lake.fish_.mood.mood_oper_data_gen import MoodOperData", 
        ...
        "yangName": "mood", 
        "namespace": "mood", 
        "logGroupName": "blinky-mood-oper", 
        "className": "BlinkyOperMood", 
        "logModuleName": "a-sys-blinky-example-lake-example-no-stop-generation-lake-fish--mood-blinky-mood-oper-gen", 
        "importStatement": "from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.mood.blinky_mood_oper_gen import BlinkyOperMood", 
        "callpointName": "lake-fish-mood-callpoint", 
        "moduleYangNamespace": "http://qwilt.com/model/lake-example", 
        "dataClassName": "MoodOperData", 
        "getObjArgsNum": 2, 
        "actionPoint": null
    }, 
    "ancestors": [

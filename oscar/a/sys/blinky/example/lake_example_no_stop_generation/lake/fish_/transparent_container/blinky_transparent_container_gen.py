...
from a.sys.blinky.example.lake_example.lake.fish_.transparent_container.transparent_container_data_gen import TransparentContainerData





class BlinkyTransparentContainer(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkytransparentcontainer').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyTransparentContainer._validationPointId, BlinkyTransparentContainer._actionPointId)
        ...
        newNode=BlinkyTransparentContainer(logger)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagVal)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.val = newValue.asInt64()
                self.myCandidateData.setHasVal()
                
            else:
                self.setValDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkytransparentcontainer').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.transparent_container.blinky_transparent_container_gen import BlinkyTransparentContainer', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagVal)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkytransparentcontainer-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.transparent_container.blinky_transparent_container_gen import BlinkyTransparentContainer', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = TransparentContainerData()
        
        res = self.setValDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-val-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setValDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setValDefaultValue (self, setHas):
        for logFunc in self._log("set-val-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.val = 0
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = TransparentContainerData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkytransparentcontainer-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkytransparentcontainer-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.lake_example.lake.fish_.transparent_container.transparent_container_data_gen import TransparentContainerData", 
        "moduleYangNamespacePrefix": "lake-example", 
        "validationPoint": null, 
        "yangName": "transparent-container", 
        "namespace": "transparent_container", 
        "logGroupName": "blinky-transparent-container", 
        "className": "BlinkyTransparentContainer", 
        "logModuleName": "a-sys-blinky-example-lake-example-no-stop-generation-lake-fish--transparent-container-blinky-transparent-container-gen", 
        "importStatement": "from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.transparent_container.blinky_transparent_container_gen import BlinkyTransparentContainer", 
        "moduleYangNamespace": "http://qwilt.com/model/lake-example", 
        "dataClassName": "TransparentContainerData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

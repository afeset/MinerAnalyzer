...
from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.test_generation_underscore.blinky_test_generation_underscore_gen import BlinkyTestGenerationUnderscore

class BlinkyTestGenerationUnderscoreList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyTestGenerationUnderscoreList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyTestGenerationUnderscoreList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkytestgenerationunderscorelist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.test_generation_underscore.blinky_test_generation_underscore_gen import BlinkyTestGenerationUnderscore', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyTestGenerationUnderscore.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkytestgenerationunderscorelist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.test_generation_underscore.blinky_test_generation_underscore_gen import BlinkyTestGenerationUnderscore', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-test-generation-underscore-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-test-generation-underscore-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-test-generation-underscore").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyTestGenerationUnderscoreList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-test-generation-underscore").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyTestGenerationUnderscore. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyTestGenerationUnderscore(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-test-generation-underscore-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyTestGenerationUnderscore not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-test-generation-underscore").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyTestGenerationUnderscore. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyTestGenerationUnderscore(self, key):
        for logFunc in self._log("get-blinky-blinky-test-generation-underscore").debug1Func(): logFunc("getBlinkyTestGenerationUnderscore(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyTestGenerationUnderscore(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-test-generation-underscore-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyTestGenerationUnderscore-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyTestGenerationUnderscore(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyTestGenerationUnderscore", 
        "containerLogGroupName": "blinky-test-generation-underscore", 
        "namespace": "test_generation_underscore", 
        "containerImportStatement": "from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.test_generation_underscore.blinky_test_generation_underscore_gen import BlinkyTestGenerationUnderscore", 
        "logGroupName": "blinky-test-generation-underscore-list", 
        "className": "BlinkyTestGenerationUnderscoreList", 
        "logModuleName": "a-sys-blinky-example-lake-example-no-stop-generation-lake-fish--test-generation-underscore-blinky-test-generation-underscore-list-gen", 
        "keyLeaf": {

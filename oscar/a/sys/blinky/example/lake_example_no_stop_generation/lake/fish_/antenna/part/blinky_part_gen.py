...
from a.sys.blinky.example.lake_example.lake.fish_.antenna.part.part_data_gen import PartData





class BlinkyPart(BlinkyContainer):
    ourNamespace="http://qwilt.com/model/lake-example"
    #leaves
    
    ourXmlTagCatalogId="catalog-id"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkypart').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyPart._validationPointId, BlinkyPart._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagCatalogId)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.catalogId = newValue.asInt64()
                self.myCandidateData.setHasCatalogId()
                
            else:
                self.setCatalogIdDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkypart').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.antenna.part.blinky_part_gen import BlinkyPart', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagCatalogId)):
        ...
        logger('is-key-path-registered-blinkypart-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.antenna.part.blinky_part_gen import BlinkyPart', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = PartData()
        
        res = self.setCatalogIdDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-catalogid-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setCatalogIdDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
    ...
    def setCatalogIdDefaultValue (self, setHas):
        for logFunc in self._log("set-catalogid-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.catalogId = 0
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkypart-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkypart-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.lake_example.lake.fish_.antenna.part.part_data_gen import PartData", 
        "moduleYangNamespacePrefix": "lake-example", 
        "validationPoint": null, 
        "yangName": "parts", 
        "namespace": "part", 
        "logGroupName": "blinky-part", 
        "className": "BlinkyPart", 
        "logModuleName": "a-sys-blinky-example-lake-example-no-stop-generation-lake-fish--antenna-part-blinky-part-gen", 
        "importStatement": "from a.sys.blinky.example.lake_example_no_stop_generation.lake.fish_.antenna.part.blinky_part_gen import BlinkyPart", 
        "moduleYangNamespace": "http://qwilt.com/model/lake-example", 
        "dataClassName": "PartData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

...
from a.sys.blinky.example.python.generator_test.ut.generator_test.person.blinky_person_gen import BlinkyPerson

class BlinkyPersonList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyPersonList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyPersonList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkypersonlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.python.generator_test.ut.generator_test.person.blinky_person_gen import BlinkyPerson', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyPerson.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkypersonlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.python.generator_test.ut.generator_test.person.blinky_person_gen import BlinkyPerson', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-person-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-person-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-person-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-person").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyPersonList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-person").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyPerson. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyPerson(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-person-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyPerson not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-person").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyPerson. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyPerson(self, key):
        for logFunc in self._log("get-blinky-blinky-person").debug1Func(): logFunc("getBlinkyPerson(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyPerson(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkyPerson(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-person-moved-failed").errorFunc(): logFunc("getBlinkyPerson(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkyPerson(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-person-after-failed").errorFunc(): logFunc("getBlinkyPerson(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-person-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyPerson-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyPerson(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyPerson", 
        "containerLogGroupName": "blinky-person", 
        "namespace": "person", 
        "containerImportStatement": "from a.sys.blinky.example.python.generator_test.ut.generator_test.person.blinky_person_gen import BlinkyPerson", 
        "logGroupName": "blinky-person-list", 
        "className": "BlinkyPersonList", 
        "logModuleName": "a-sys-blinky-example-python-generator-test-ut-generator-test-person-blinky-person-list-gen", 
        "keyLeaf": {
            "varName": "person", 

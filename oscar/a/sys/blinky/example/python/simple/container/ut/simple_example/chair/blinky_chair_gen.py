...
from a.sys.blinky.example.python.simple.container.ut.simple_example.chair.chair_data_gen import ChairData





class BlinkyChair(BlinkyContainer):
    ourNamespace="http://qwilt.com/ns/yang/ut/sys/blinky/example/python/simple-example"
    #leaves
    
    ourXmlTagColor="color"
    
    ourXmlTagPretty="pretty"
    
    ourXmlTagNumOfLegs="num-of-legs"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkychair').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyChair._validationPointId, BlinkyChair._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagColor)):
            if trxElement.getNewVal():
                ...
                self.myCandidateData.color = newValue.asString()
                self.myCandidateData.setHasColor()
                
            else:
                self.setColorDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagPretty)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.pretty = newValue.asBool()
                self.myCandidateData.setHasPretty()
                
            else:
                self.setPrettyDefaultValue(True)
            
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagNumOfLegs)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.numOfLegs = newValue.asInt64()
                self.myCandidateData.setHasNumOfLegs()
                
            else:
                self.setNumOfLegsDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkychair').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.python.simple.container.ut.simple_example.chair.blinky_chair_gen import BlinkyChair', keyPath, keyDepth)
        ...
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagPretty)):
                keyPathRegistered = True
                found = True
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagNumOfLegs)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkychair-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.python.simple.container.ut.simple_example.chair.blinky_chair_gen import BlinkyChair', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = ChairData()
        ...
        res = self.setPrettyDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-pretty-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setPrettyDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        res = self.setNumOfLegsDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-numoflegs-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setNumOfLegsDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
    ...
    def setColorDefaultValue (self, setHas):
        ...
        self.myCandidateData.color = "yellow"
    ...
    def setPrettyDefaultValue (self, setHas):
        for logFunc in self._log("set-pretty-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.pretty = False
    ...
    def setNumOfLegsDefaultValue (self, setHas):
        for logFunc in self._log("set-numoflegs-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.numOfLegs = 0
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkychair-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkychair-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.python.simple.container.ut.simple_example.chair.chair_data_gen import ChairData", 
        "moduleYangNamespacePrefix": "se", 
        "validationPoint": null, 
        "yangName": "chair", 
        "namespace": "chair", 
        "logGroupName": "blinky-chair", 
        "className": "BlinkyChair", 
        "logModuleName": "a-sys-blinky-example-python-simple-container-ut-simple-example-chair-blinky-chair-gen", 
        "importStatement": "from a.sys.blinky.example.python.simple.container.ut.simple_example.chair.blinky_chair_gen import BlinkyChair", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/ut/sys/blinky/example/python/simple-example", 
        "dataClassName": "ChairData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

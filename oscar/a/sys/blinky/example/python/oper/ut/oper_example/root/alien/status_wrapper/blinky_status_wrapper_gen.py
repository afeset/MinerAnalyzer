...
from a.sys.blinky.example.python.oper.ut.oper_example.root.alien.status_wrapper.status_wrapper_data_gen import StatusWrapperData





class BlinkyStatusWrapper(BlinkyContainer):
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkystatuswrapper').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.python.oper.ut.oper_example.root.alien.status_wrapper.blinky_status_wrapper_gen import BlinkyStatusWrapper', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkystatuswrapper-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.python.oper.ut.oper_example.root.alien.status_wrapper.blinky_status_wrapper_gen import BlinkyStatusWrapper', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = StatusWrapperData()
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = StatusWrapperData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkystatuswrapper-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkystatuswrapper-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.python.oper.ut.oper_example.root.alien.status_wrapper.status_wrapper_data_gen import StatusWrapperData", 
        "moduleYangNamespacePrefix": "oe", 
        "validationPoint": null, 
        "yangName": "status-wrapper", 
        "namespace": "status_wrapper", 
        "logGroupName": "blinky-status-wrapper", 
        "className": "BlinkyStatusWrapper", 
        "logModuleName": "a-sys-blinky-example-python-oper-ut-oper-example-root-alien-status-wrapper-blinky-status-wrapper-gen", 
        ...
        "dataClassName": "StatusWrapperData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

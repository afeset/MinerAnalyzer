...
from a.sys.blinky.example.python.oper.ut.oper_example.root.alien.blinky_alien_gen import BlinkyAlien

class BlinkyAlienList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyAlienList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyAlienList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyalienlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.python.oper.ut.oper_example.root.alien.blinky_alien_gen import BlinkyAlien', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyAlien.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyalienlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.python.oper.ut.oper_example.root.alien.blinky_alien_gen import BlinkyAlien', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-alien-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-alien-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-alien-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-alien").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyAlienList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-alien").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyAlien. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyAlien(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-alien-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyAlien not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-alien").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyAlien. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyAlien(self, key):
        for logFunc in self._log("get-blinky-blinky-alien").debug1Func(): logFunc("getBlinkyAlien(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyAlien(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkyAlien(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-alien-moved-failed").errorFunc(): logFunc("getBlinkyAlien(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkyAlien(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-alien-after-failed").errorFunc(): logFunc("getBlinkyAlien(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-alien-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyAlien-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyAlien(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyAlien", 
        "containerLogGroupName": "blinky-alien", 
        "namespace": "alien", 
        "containerImportStatement": "from a.sys.blinky.example.python.oper.ut.oper_example.root.alien.blinky_alien_gen import BlinkyAlien", 
        "logGroupName": "blinky-alien-list", 
        "className": "BlinkyAlienList", 
        "logModuleName": "a-sys-blinky-example-python-oper-ut-oper-example-root-alien-blinky-alien-list-gen", 
        "keyLeaf": {
            "varName": "alien", 

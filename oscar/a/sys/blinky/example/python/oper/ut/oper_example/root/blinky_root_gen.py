...
from a.sys.blinky.example.python.oper.ut.oper_example.root.alien.blinky_alien_list_gen import BlinkyAlienList




class BlinkyRoot(BlinkyContainer):
    ...
    ALIENLIST_CREATE_FUNCTOR = 'ALIENLIST_CREATE_FUNCTOR'
    ALIENLIST_DELETE_FUNCTOR = 'ALIENLIST_DELETE_FUNCTOR'
    ...
    def __init__ (self, logger):
        ...
        self.myCreateAlienListFunctor=None
        self.myDeleteAlienListFunctor=None
        self.myAlienList=None
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyroot').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyRoot._validationPointId, BlinkyRoot._actionPointId)
    ...
    def setCreateAlienListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-create-alien-functor-active").errorFunc():
                logFunc("setCreateAlienListFunctor() is illegal when blinky node is active")
        self.myCreateAlienListFunctor = functor

    def setDeleteAlienListFunctor(self, functor):
        if (self.myIsActive):
            for logFunc in self._log("set-delete-alien-functor-active").errorFunc():
                logFunc("setDeleteAlienListFunctor() is illegal when blinky node is active")
        self.myDeleteAlienListFunctor = functor
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if (keyPath.isTagEqual(keyDepth_PBR.value(), self.ourNamespace, self.ourXmlTagAlienList)):
            return self.myAlienList
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyroot').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.python.oper.ut.oper_example.root.blinky_root_gen import BlinkyRoot', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        if not found:
            if keyPath.getLen() > keyDepth+1:
                if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagAlienList)):
                    keyPathRegistered = BlinkyAlienList.isKeyPathRegistered(logger, keyPath, keyDepth+1)
                    found = True
            else:
        ...
        logger('is-key-path-registered-blinkyroot-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.python.oper.ut.oper_example.root.blinky_root_gen import BlinkyRoot', keyPathRegistered, keyPath, keyDepth)
    ...
    def prepareMyBlinkyNode(self, keyPath, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagAlienList)):
            if (self.myAlienList):
                for logFunc in self._log("prepare-my-blinky-node-alienlist-already-exists").errorFunc():
                    logFunc("prepareMyBlinkyNode(): already exists, keyPath=%s, keyDepth=%s", keyPath, keyDepth)
                return None
            self.myAlienList = BlinkyAlienList(self._log)
            self.myAlienList.setParent(self)
            self.myAlienList.setKeyPath(keyPath)
            self.myAlienList.setDomain(self.myDomain)
    ...
    def handleTrxElementCreate(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagAlienList)):
                if (self.myCreateAlienListFunctor):
                    if (not self.myAlienList):
                        for logFunc in self._log("handle-trx-element-create-alienlist-not-exist").debug3Func():
                            logFunc("handleTrxElementCreate(): alienlist not exists, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    
                    timeoutGuard = TimeoutGuard(self._log, '%s-create-alienlist-functor-%s' % (self.myKeyPath, phase), 
                                                self.getFunctorTimeoutForPhase(self.ALIENLIST_CREATE_FUNCTOR, phase),
                                                self.getFunctorMildTimeoutForPhase(self.ALIENLIST_CREATE_FUNCTOR, phase))
                    try:
                        res = self.myCreateAlienListFunctor(phase, self.myAlienList)
                    except:
                        for logFunc in self._log("handle-trx-element-create-alienlist-functor-exception").exceptionFunc():
                            logFunc("AlienList's create functor raised an exception. trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        raise
                    timeoutGuard.checkAndLog("application problem: functor=%s" % self.myCreateAlienListFunctor.__name__)
                    if (res != ReturnCodes.kOk):
                        if (phase.getConfdPhase() == TrxPhase.kPrepare):
                            for logFunc in self._log("handle-trx-element-create-alienlist-functor-failed-prepare").noticeFunc():
                                logFunc("handleTrxElementCreate(): alienlist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                            self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        else:
                            for logFunc in self._log("handle-trx-element-create-alienlist-functor-failed").errorFunc():
                                logFunc("handleTrxElementCreate(): alienlist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                       trxElement, keyDepth, phase)
                        return ReturnCodes.kGeneralError
    ...
    def notifyDescendantsPrepareBlinkyDelete(self):
        ...
        if (self.myAlienList):
            self.myAlienList.notifyDescendantsPrepareBlinkyDelete()
    ...
    def handleTrxElementDelete(self, trxElement, keyDepth, phase):
        ...
        if (self.myIsActive):
            ...
            if (trxElement.getKeyPath().isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagAlienList)):
                res = self.activateDeleteAlienListFunctor(phase)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("handle-trx-element-delete-alienlist-functor-failed-prepare").noticeFunc():
                            logFunc("handleTrxElementDelete(): alienlist functor-failed-prepare, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("handle-trx-element-delete-alienlist-functor-failed").errorFunc():
                            logFunc("handleTrxElementDelete(): alienlist functor-failed, trxElement=%s, keyDepth=%s, phase=%s",
                                   trxElement, keyDepth, phase)
                    return ReturnCodes.kGeneralError
    ...
    def removeAlienList(self):
        self.myAlienList = None

    def activateDeleteAlienListFunctor(self, phase):
        for logFunc in self._log("activate-delete-alienlist-functor").debug3Func(): logFunc("activateDeleteAlienListFunctor(): called, phase=%s", phase)
        if (self.myIsActive):
            if (self.myDeleteAlienListFunctor):
                timeoutGuard = TimeoutGuard(self._log, '%s-delete-alienlist-functor-%s' % (self.myKeyPath, phase), 
                                            self.getFunctorTimeoutForPhase(self.ALIENLIST_DELETE_FUNCTOR, phase), 
                                            self.getFunctorMildTimeoutForPhase(self.ALIENLIST_DELETE_FUNCTOR, phase))
                try:
                    res = self.myDeleteAlienListFunctor(phase)
                except:
                    for logFunc in self._log("activate-delete-alienlist-functor-exception").exceptionFunc():
                        logFunc("AlienList's delete functor raised an exception. phase=%s", phase)
                    raise
                timeoutGuard.checkAndLog("application problem: functor=%s" % self.myDeleteAlienListFunctor.__name__)
                if (res != ReturnCodes.kOk):
                    if (phase.getConfdPhase() == TrxPhase.kPrepare):
                        for logFunc in self._log("activate-delete-alienlist-functor-functor-failed-prepare").noticeFunc():
                            logFunc("activateDeleteAlienListFunctor(): functor failed, phase=%s", phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                    else:
                        for logFunc in self._log("activate-delete-alienlist-functor-functor-failed").errorFunc():
                            logFunc("activateDeleteAlienListFunctor(): functor failed, phase=%s", phase)
                    return ReturnCodes.kGeneralError
        return ReturnCodes.kOk
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyroot-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyroot-failed").errorFunc():
    ...
    def handleInternalDestroy(self, phase):
        ...
        if (phase.getConfdPhase() == TrxPhase.kPrepare):
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myAlienList):
                    res = self.myAlienList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-alienlist-recursion-failed-prepare-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): delete-alienlist-recursion-failed-prepare-blinky, phase=%s",
                                  phase)
                        return ReturnCodes.kGeneralError
                ...
                if (self.myAlienList):
                    res = self.activateDeleteAlienListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-alienlist-functor-failed-prepare").noticeFunc():
                            logFunc("handleInternalDestroy(): activateDeleteAlienListFunctor() failed, res=%s, phase=%s",
                                   res, phase)
                        self.myDomain.setConfigErrorStr(self.myConfigErrorStr)
                        return ReturnCodes.kGeneralError
                if (self.myAlienList):
                    res = self.myAlienList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-alienlist-recursion-failed-prepare-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myAlienList.handleInternalDestroy() failed, res=%s, phase=%s",
                                   res, phase)
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myAlienList):
                    res = self.myAlienList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-alienlist-recursion-failed-commit-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myAlienList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("handle internal destroy failed in commit phase (blinky)")
                        return ReturnCodes.kGeneralError
                    self.removeAlienList()
                ...
                if (self.myAlienList):
                    res = self.myAlienList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-alienlist-recursion-failed-commit-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myAlienList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                    ...
                    res = self.activateDeleteAlienListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-alienlist-functor-failed-commit").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteAlienListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
                        a.infra.process.processFatal("activateDeleteAlienListFunctor failed in commit phase (private/public)")
                        return ReturnCodes.kGeneralError
            ...
            if (phase.getBlinkyPhase() == TrxPhase.kBlinky):
                ...
                if (self.myAlienList):
                    res = self.myAlienList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-alienlist-recursion-failed-abort-blinky").errorFunc():
                            logFunc("handleInternalDestroy(): myAlienList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myAlienList):
                    res = self.myAlienList.handleInternalDestroy(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-alienlist-recursion-failed-abort-private-or-public").errorFunc():
                            logFunc("handleInternalDestroy(): myAlienList.handleInternalDestroy() failed, res=%s, phase=%s",
                                  res, phase)
                ...
                if (self.myAlienList):
                    res = self.activateDeleteAlienListFunctor(phase)
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-internal-destroy-delete-alienlist-functor-failed-abort").errorFunc():
                            logFunc("handleInternalDestroy(): activateDeleteAlienListFunctor() failed, res=%s, phase=%s",
                                  res, phase)
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.python.oper.ut.oper_example.root.root_data_gen import RootData", 
        "moduleYangNamespacePrefix": "oe", 
        "validationPoint": null, 
        "yangName": "root", 
        "namespace": "root", 
        "logGroupName": "blinky-root", 
        "className": "BlinkyRoot", 
        "logModuleName": "a-sys-blinky-example-python-oper-ut-oper-example-root-blinky-root-gen", 
        "importStatement": "from a.sys.blinky.example.python.oper.ut.oper_example.root.blinky_root_gen import BlinkyRoot", 
        "moduleYangNamespace": "http://qwilt.com/ns/yang/ut/sys/blinky/example/python/oper-example", 
        "dataClassName": "RootData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {
    ...
    "descendants": [
        {
            "className": "BlinkyAlienList", 
            "memberName": "AlienList", 
            "yangName": "alien", 
            "importStatement": "from a.sys.blinky.example.python.oper.ut.oper_example.root.alien.blinky_alien_list_gen import BlinkyAlienList"
        }
    ], 

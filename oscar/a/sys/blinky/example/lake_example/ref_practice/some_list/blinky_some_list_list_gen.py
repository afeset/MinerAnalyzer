...
from a.sys.blinky.example.lake_example.ref_practice.some_list.blinky_some_list_gen import BlinkySomeList

class BlinkySomeListList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkySomeListList(logger)
    ...
    def __str__ (self):
        ret="{BlinkySomeListList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkysomelistlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.lake_example.ref_practice.some_list.blinky_some_list_gen import BlinkySomeList', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkySomeList.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkysomelistlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.lake_example.ref_practice.some_list.blinky_some_list_gen import BlinkySomeList', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-some-list-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-some-list-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-some-list").debug1Func(): logFunc("findBlinkyNode(): handling BlinkySomeListList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-some-list").debug1Func(): logFunc("findBlinkyNode(): getting BlinkySomeList. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkySomeList(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-some-list-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkySomeList not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-some-list").debug1Func(): logFunc("findBlinkyNode(): querying BlinkySomeList. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkySomeList(self, key):
        for logFunc in self._log("get-blinky-blinky-some-list").debug1Func(): logFunc("getBlinkySomeList(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkySomeList(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-some-list-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkySomeList-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkySomeList(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkySomeList", 
        "containerLogGroupName": "blinky-some-list", 
        "namespace": "some_list", 
        "containerImportStatement": "from a.sys.blinky.example.lake_example.ref_practice.some_list.blinky_some_list_gen import BlinkySomeList", 
        "logGroupName": "blinky-some-list-list", 
        "className": "BlinkySomeListList", 
        "logModuleName": "a-sys-blinky-example-lake-example-ref-practice-some-list-blinky-some-list-list-gen", 
        "keyLeaf": {
            "varName": "someList", 

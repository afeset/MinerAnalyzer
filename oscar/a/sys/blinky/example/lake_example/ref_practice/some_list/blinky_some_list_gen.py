...
from a.sys.blinky.example.lake_example.ref_practice.some_list.some_list_data_gen import SomeListData





class BlinkySomeList(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkysomelist').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkySomeList._validationPointId, BlinkySomeList._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagName)):
            ...
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.value = newValue.asInt64()
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkysomelist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.lake_example.ref_practice.some_list.blinky_some_list_gen import BlinkySomeList', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkysomelist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.lake_example.ref_practice.some_list.blinky_some_list_gen import BlinkySomeList', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = SomeListData()
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = SomeListData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkysomelist-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkysomelist-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.lake_example.ref_practice.some_list.some_list_data_gen import SomeListData", 
        "moduleYangNamespacePrefix": "lake-example", 
        "validationPoint": null, 
        "yangName": "some-list", 
        "namespace": "some_list", 
        "logGroupName": "blinky-some-list", 
        "className": "BlinkySomeList", 
        "logModuleName": "a-sys-blinky-example-lake-example-ref-practice-some-list-blinky-some-list-gen", 
        "importStatement": "from a.sys.blinky.example.lake_example.ref_practice.some_list.blinky_some_list_gen import BlinkySomeList", 
        "moduleYangNamespace": "http://qwilt.com/model/lake-example", 
        "dataClassName": "SomeListData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

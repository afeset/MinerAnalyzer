...
from a.sys.blinky.example.lake_example.list_ordered_by_user.ordered_list.ordered_list_data_gen import OrderedListData





class BlinkyOrderedList(BlinkyContainer):
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyorderedlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.user_ordered.list_ordered_by_user.ordered_list.blinky_ordered_list_gen import BlinkyOrderedList', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkyorderedlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.user_ordered.list_ordered_by_user.ordered_list.blinky_ordered_list_gen import BlinkyOrderedList', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = OrderedListData()
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = OrderedListData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyorderedlist-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyorderedlist-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.lake_example.list_ordered_by_user.ordered_list.ordered_list_data_gen import OrderedListData", 
        "moduleYangNamespacePrefix": "lake-example", 
        "validationPoint": null, 
        "yangName": "ordered-list", 
        "namespace": "ordered_list", 
        "logGroupName": "blinky-ordered-list", 
        "className": "BlinkyOrderedList", 
        "logModuleName": "a-sys-blinky-example-user-ordered-list-ordered-by-user-ordered-list-blinky-ordered-list-gen", 
        "importStatement": "from a.sys.blinky.example.user_ordered.list_ordered_by_user.ordered_list.blinky_ordered_list_gen import BlinkyOrderedList", 
        "moduleYangNamespace": "http://qwilt.com/model/lake-example", 
        "dataClassName": "OrderedListData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

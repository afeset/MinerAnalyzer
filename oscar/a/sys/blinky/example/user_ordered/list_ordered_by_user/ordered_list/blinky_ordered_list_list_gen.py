...
from a.sys.blinky.example.user_ordered.list_ordered_by_user.ordered_list.blinky_ordered_list_gen import BlinkyOrderedList

class BlinkyOrderedListList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyOrderedListList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyOrderedListList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyorderedlistlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.user_ordered.list_ordered_by_user.ordered_list.blinky_ordered_list_gen import BlinkyOrderedList', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyOrderedList.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyorderedlistlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.user_ordered.list_ordered_by_user.ordered_list.blinky_ordered_list_gen import BlinkyOrderedList', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-ordered-list-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-ordered-list-list is active.")
        self.myDeleteFunctor=functor

    def setMovedAfterFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-moved-after-functor-not-active").errorFunc(): logFunc("setMovedAfterFunctor is illegal when blinky-ordered-list-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-ordered-list").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyOrderedListList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-ordered-list").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyOrderedList. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyOrderedList(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-ordered-list-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyOrderedList not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-ordered-list").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyOrderedList. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyOrderedList(self, key):
        for logFunc in self._log("get-blinky-blinky-ordered-list").debug1Func(): logFunc("getBlinkyOrderedList(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyOrderedList(self._log)
    ...
    def moveAfter (self, movedKey, afterKey):
        for logFunc in self._log("move-after-details").debug1Func(): logFunc("moveAfter (): details: movedKey=%s, afterKey=%s",
                                               movedKey, afterKey)
        movedNode = self.getBlinkyOrderedList(movedKey)
        if not movedNode:
            for logFunc in self._log("moved-after-get-blinky-ordered-list-moved-failed").errorFunc(): logFunc("getBlinkyOrderedList(%s) failed. movedKey=%s, afterKey=%s",
        ...
        if afterKey:
            afterNode = self.getBlinkyOrderedList(afterKey)
            if not afterNode:
                for logFunc in self._log("moved-after-get-blinky-ordered-list-after-failed").errorFunc(): logFunc("getBlinkyOrderedList(%s) failed. movedKey=%s, afterKey=%s",
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-ordered-list-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyOrderedList-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyOrderedList(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyOrderedList", 
        "containerLogGroupName": "blinky-ordered-list", 
        "namespace": "ordered_list", 
        "containerImportStatement": "from a.sys.blinky.example.user_ordered.list_ordered_by_user.ordered_list.blinky_ordered_list_gen import BlinkyOrderedList", 
        "logGroupName": "blinky-ordered-list-list", 
        "className": "BlinkyOrderedListList", 
        "logModuleName": "a-sys-blinky-example-user-ordered-list-ordered-by-user-ordered-list-blinky-ordered-list-list-gen", 
        "keyLeaf": {
            "varName": "orderedList", 

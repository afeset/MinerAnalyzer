...
from a.sys.blinky.example.oper.oper.generation_dummy.generation_dummy_data_gen import GenerationDummyData





class BlinkyGenerationDummy(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkygenerationdummy').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyGenerationDummy._validationPointId, BlinkyGenerationDummy._actionPointId)
        ...
        newNode=BlinkyGenerationDummy(logger)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagDummy)):
            if trxElement.getNewVal():
                ...
                self.myCandidateData.dummy = newValue.asInt64()
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkygenerationdummy').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.oper.oper.generation_dummy.blinky_generation_dummy_gen import BlinkyGenerationDummy', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkygenerationdummy-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.oper.oper.generation_dummy.blinky_generation_dummy_gen import BlinkyGenerationDummy', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = GenerationDummyData()
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = GenerationDummyData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkygenerationdummy-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkygenerationdummy-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.oper.oper.generation_dummy.generation_dummy_data_gen import GenerationDummyData", 
        "moduleYangNamespacePrefix": "oper", 
        "validationPoint": null, 
        "yangName": "generation-dummy", 
        "namespace": "generation_dummy", 
        "logGroupName": "blinky-generation-dummy", 
        "className": "BlinkyGenerationDummy", 
        "logModuleName": "a-sys-blinky-example-oper-oper-generation-dummy-blinky-generation-dummy-gen", 
        "importStatement": "from a.sys.blinky.example.oper.oper.generation_dummy.blinky_generation_dummy_gen import BlinkyGenerationDummy", 
        "moduleYangNamespace": "http://qwilt.com/model/oper", 
        "dataClassName": "GenerationDummyData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

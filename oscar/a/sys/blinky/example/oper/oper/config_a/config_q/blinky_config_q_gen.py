...
from a.sys.blinky.example.oper.oper.config_a.config_q.config_q_data_gen import ConfigQData





class BlinkyConfigQ(BlinkyContainer):
    ourNamespace="http://qwilt.com/model/oper"
    #leaves
    
    ourXmlTagValueConfigQ="value-config-q"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyconfigq').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyConfigQ._validationPointId, BlinkyConfigQ._actionPointId)

        confd_key=KeyPath()
        
        
        xmlVal = Value()
        xmlVal.setXmlTag(("config-q", "http://qwilt.com/model/oper", "oper"))
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagValueConfigQ)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.valueConfigQ = newValue.asInt8()
                self.myCandidateData.setHasValueConfigQ()
                
            else:
                self.setValueConfigQDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyconfigq').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.oper.oper.config_a.config_q.blinky_config_q_gen import BlinkyConfigQ', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagValueConfigQ)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkyconfigq-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.oper.oper.config_a.config_q.blinky_config_q_gen import BlinkyConfigQ', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = ConfigQData()
        
        res = self.setValueConfigQDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-valueconfigq-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setValueConfigQDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setValueConfigQDefaultValue (self, setHas):
        for logFunc in self._log("set-valueconfigq-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.valueConfigQ = 0
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = ConfigQData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyconfigq-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyconfigq-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.oper.oper.config_a.config_q.config_q_data_gen import ConfigQData", 
        "moduleYangNamespacePrefix": "oper", 
        "validationPoint": null, 
        "yangName": "config-q", 
        "namespace": "config_q", 
        "logGroupName": "blinky-config-q", 
        "className": "BlinkyConfigQ", 
        "logModuleName": "a-sys-blinky-example-oper-oper-config-a-config-q-blinky-config-q-gen", 

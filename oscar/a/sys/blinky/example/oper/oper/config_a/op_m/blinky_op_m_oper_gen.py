...
from a.sys.blinky.example.oper.oper.config_a.op_m.op_m_oper_data_gen import OpMOperData
...
class BlinkyOperOpM(BlinkyOperNode):

    _kCallpointName = "op-m-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "op-m-callpoint"
    ...
    def getOperRelativePath (self, operRelativePath):
        ...
        val = Value()
        val.setXmlTag(("op-m", "http://qwilt.com/model/oper", "oper"))
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/model/oper", "value-op-m1"):
                    for logFunc in self._log("set-oper-data-requested-fields-valueopm1-requested").debug3Func(): logFunc(
                        "value-op-m1 requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setValueOpM1Requested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isValueOpM1Requested() and operData.hasValueOpM1():
            val = Value()
            val.setString(operData.valueOpM1)
            tagValueList.push(("value-op-m1", "http://qwilt.com/model/oper"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/model/oper", "value-op-m1"):
            if operData.isValueOpM1Requested():
                 if operData.hasValueOpM1():
                     value.setString(operData.valueOpM1)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.oper.oper.config_a.op_m.op_m_oper_data_gen import OpMOperData", 
        ...
        "yangName": "op-m", 
        "namespace": "op_m", 
        "logGroupName": "blinky-op-m-oper", 
        "className": "BlinkyOperOpM", 
        "logModuleName": "a-sys-blinky-example-oper-oper-config-a-op-m-blinky-op-m-oper-gen", 
        "importStatement": "from a.sys.blinky.example.oper.oper.config_a.op_m.blinky_op_m_oper_gen import BlinkyOperOpM", 
        "callpointName": "op-m-callpoint", 
        "moduleYangNamespace": "http://qwilt.com/model/oper", 
        "dataClassName": "OpMOperData", 
        "getObjArgsNum": 2, 
        "actionPoint": null
    }, 
    "ancestors": [

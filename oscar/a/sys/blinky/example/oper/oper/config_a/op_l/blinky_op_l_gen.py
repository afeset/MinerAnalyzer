...
from a.sys.blinky.example.oper.oper.config_a.op_l.op_l_data_gen import OpLData





class BlinkyOpL(BlinkyContainer):
    ourNamespace="http://qwilt.com/model/oper"
    #leaves
    
    ourXmlTagConfigDummy="config-dummy"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyopl').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyOpL._validationPointId, BlinkyOpL._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagConfigDummy)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.configDummy = newValue.asInt64()
                self.myCandidateData.setHasConfigDummy()
                
            else:
                self.setConfigDummyDefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyopl').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.oper.oper.config_a.op_l.blinky_op_l_gen import BlinkyOpL', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagConfigDummy)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkyopl-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.oper.oper.config_a.op_l.blinky_op_l_gen import BlinkyOpL', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = OpLData()
        
        res = self.setConfigDummyDefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-configdummy-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setConfigDummyDefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setConfigDummyDefaultValue (self, setHas):
        for logFunc in self._log("set-configdummy-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.configDummy = 0
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyopl-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyopl-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.oper.oper.config_a.op_l.op_l_data_gen import OpLData", 
        "moduleYangNamespacePrefix": "oper", 
        "validationPoint": null, 
        "yangName": "op-l", 
        "namespace": "op_l", 
        "logGroupName": "blinky-op-l", 
        "className": "BlinkyOpL", 
        "logModuleName": "a-sys-blinky-example-oper-oper-config-a-op-l-blinky-op-l-gen", 

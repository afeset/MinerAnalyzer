...
from a.sys.blinky.example.oper.oper.config_a.config_u.config_u_data_gen import ConfigUData





class BlinkyConfigU(BlinkyContainer):
    ourNamespace="http://qwilt.com/model/oper"
    #leaves
    
    ourXmlTagValueConfigU1="value-config-u1"
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyconfigu').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyConfigU._validationPointId, BlinkyConfigU._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagValueConfigU1)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
                self.myCandidateData.valueConfigU1 = newValue.asString()
                self.myCandidateData.setHasValueConfigU1()
                
            else:
                self.setValueConfigU1DefaultValue(True)
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyconfigu').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.oper.oper.config_a.config_u.blinky_config_u_gen import BlinkyConfigU', keyPath, keyDepth)
        keyPathRegistered = False
        found = False
        
        
        if not found:
            if (keyPath.isTagEqual(keyDepth, cls.ourNamespace, cls.ourXmlTagValueConfigU1)):
                keyPathRegistered = True
                found = True
        
        logger('is-key-path-registered-blinkyconfigu-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.oper.oper.config_a.config_u.blinky_config_u_gen import BlinkyConfigU', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = ConfigUData()
        
        res = self.setValueConfigU1DefaultValue(True)
        if res != ReturnCodes.kOk:
            for logFunc in self._log("alloc-my-candidate-set-valueconfigu1-default-value-failed").errorFunc():
                logFunc("allocMyCandidate(): setValueConfigU1DefaultValue failed. res=%s",
                      res)
            return ReturnCodes.kGeneralError
        
        return ReturnCodes.kOk

    def setValueConfigU1DefaultValue (self, setHas):
        for logFunc in self._log("set-valueconfigu1-default-value").debug3Func():
            logFunc("called. setHas=%s", setHas)
        
        
        self.myCandidateData.valueConfigU1 = ""
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = ConfigUData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyconfigu-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyconfigu-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.oper.oper.config_a.config_u.config_u_data_gen import ConfigUData", 
        "moduleYangNamespacePrefix": "oper", 
        "validationPoint": null, 
        "yangName": "config-u", 
        "namespace": "config_u", 
        "logGroupName": "blinky-config-u", 
        "className": "BlinkyConfigU", 
        "logModuleName": "a-sys-blinky-example-oper-oper-config-a-config-u-blinky-config-u-gen", 
        "importStatement": "from a.sys.blinky.example.oper.oper.config_a.config_u.blinky_config_u_gen import BlinkyConfigU", 
        "moduleYangNamespace": "http://qwilt.com/model/oper", 
        "dataClassName": "ConfigUData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

...
from a.sys.blinky.example.oper.oper.config_a.config_p.op_r.op_r_oper_data_gen import OpROperData
...
class BlinkyOperOpR(BlinkyOperNode):

    _kCallpointName = "op-r-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "op-r-callpoint"
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/model/oper", "op-value-r"):
                    for logFunc in self._log("set-oper-data-requested-fields-opvaluer-requested").debug3Func(): logFunc(
                        "op-value-r requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setOpValueRRequested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isOpValueRRequested() and operData.hasOpValueR():
            val = Value()
            val.setString(operData.opValueR)
            tagValueList.push(("op-value-r", "http://qwilt.com/model/oper"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/model/oper", "op-value-r"):
            if operData.isOpValueRRequested():
                 if operData.hasOpValueR():
                     value.setString(operData.opValueR)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.oper.oper.config_a.config_p.op_r.op_r_oper_data_gen import OpROperData", 
        ...
        "yangName": "op-r", 
        "namespace": "op_r", 
        "logGroupName": "blinky-op-r-oper", 
        "className": "BlinkyOperOpR", 
        "logModuleName": "a-sys-blinky-example-oper-oper-config-a-config-p-op-r-blinky-op-r-oper-gen", 
        "importStatement": "from a.sys.blinky.example.oper.oper.config_a.config_p.op_r.blinky_op_r_oper_gen import BlinkyOperOpR", 
        "callpointName": "op-r-callpoint", 
        "moduleYangNamespace": "http://qwilt.com/model/oper", 
        "dataClassName": "OpROperData", 
        "getObjArgsNum": 2, 
        "actionPoint": null
    }, 
    "ancestors": [

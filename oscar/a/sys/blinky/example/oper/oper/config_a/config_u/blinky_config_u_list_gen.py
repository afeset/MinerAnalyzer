...
from a.sys.blinky.example.oper.oper.config_a.config_u.blinky_config_u_gen import BlinkyConfigU

class BlinkyConfigUList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyConfigUList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyConfigUList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyconfigulist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.oper.oper.config_a.config_u.blinky_config_u_gen import BlinkyConfigU', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyConfigU.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyconfigulist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.oper.oper.config_a.config_u.blinky_config_u_gen import BlinkyConfigU', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-config-u-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-config-u-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-config-u").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyConfigUList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-config-u").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyConfigU. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyConfigU(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-config-u-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyConfigU not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-config-u").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyConfigU. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyConfigU(self, key):
        for logFunc in self._log("get-blinky-blinky-config-u").debug1Func(): logFunc("getBlinkyConfigU(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyConfigU(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-config-u-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyConfigU-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyConfigU(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyConfigU", 
        "containerLogGroupName": "blinky-config-u", 
        "namespace": "config_u", 
        "containerImportStatement": "from a.sys.blinky.example.oper.oper.config_a.config_u.blinky_config_u_gen import BlinkyConfigU", 
        "logGroupName": "blinky-config-u-list", 
        "className": "BlinkyConfigUList", 
        "logModuleName": "a-sys-blinky-example-oper-oper-config-a-config-u-blinky-config-u-list-gen", 
        "keyLeaf": {
            "varName": "configU", 

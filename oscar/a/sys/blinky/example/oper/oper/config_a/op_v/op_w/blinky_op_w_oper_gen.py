...
from a.sys.blinky.example.oper.oper.config_a.op_v.op_w.op_w_oper_data_gen import OpWOperData
...
class BlinkyOperOpW(BlinkyOperNode):

    _kCallpointName = "op-w-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "op-w-callpoint"
    ...
    def getOperRelativePath (self, operRelativePath):
        ...
        val = Value()
        val.setXmlTag(("op-w", "http://qwilt.com/model/oper", "oper"))
        operRelativePath.addKeyPathPostfix(val)
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/model/oper", "op-w-value"):
                    for logFunc in self._log("set-oper-data-requested-fields-opwvalue-requested").debug3Func(): logFunc(
                        "op-w-value requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setOpWValueRequested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isOpWValueRequested() and operData.hasOpWValue():
            val = Value()
            val.setString(operData.opWValue)
            tagValueList.push(("op-w-value", "http://qwilt.com/model/oper"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/model/oper", "op-w-value"):
            if operData.isOpWValueRequested():
                 if operData.hasOpWValue():
                     value.setString(operData.opWValue)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.oper.oper.config_a.op_v.op_w.op_w_oper_data_gen import OpWOperData", 
        ...
        "yangName": "op-w", 
        "namespace": "op_w", 
        "logGroupName": "blinky-op-w-oper", 
        "className": "BlinkyOperOpW", 
        "logModuleName": "a-sys-blinky-example-oper-oper-config-a-op-v-op-w-blinky-op-w-oper-gen", 
        "importStatement": "from a.sys.blinky.example.oper.oper.config_a.op_v.op_w.blinky_op_w_oper_gen import BlinkyOperOpW", 
        "callpointName": "op-w-callpoint", 
        "moduleYangNamespace": "http://qwilt.com/model/oper", 
        "dataClassName": "OpWOperData", 
        "getObjArgsNum": 3, 
        "actionPoint": null
    }, 
    "ancestors": [

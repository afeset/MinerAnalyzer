...
from a.sys.blinky.example.oper.oper.config_a.config_p.blinky_config_p_gen import BlinkyConfigP

class BlinkyConfigPList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyConfigPList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyConfigPList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyconfigplist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.oper.oper.config_a.config_p.blinky_config_p_gen import BlinkyConfigP', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyConfigP.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyconfigplist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.sys.blinky.example.oper.oper.config_a.config_p.blinky_config_p_gen import BlinkyConfigP', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-config-p-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-config-p-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-config-p").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyConfigPList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-config-p").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyConfigP. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyConfigP(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-config-p-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyConfigP not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-config-p").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyConfigP. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyConfigP(self, key):
        for logFunc in self._log("get-blinky-blinky-config-p").debug1Func(): logFunc("getBlinkyConfigP(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyConfigP(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-config-p-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyConfigP-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyConfigP(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyConfigP", 
        "containerLogGroupName": "blinky-config-p", 
        "namespace": "config_p", 
        "containerImportStatement": "from a.sys.blinky.example.oper.oper.config_a.config_p.blinky_config_p_gen import BlinkyConfigP", 
        "logGroupName": "blinky-config-p-list", 
        "className": "BlinkyConfigPList", 
        "logModuleName": "a-sys-blinky-example-oper-oper-config-a-config-p-blinky-config-p-list-gen", 
        "keyLeaf": {
            "varName": "configP", 

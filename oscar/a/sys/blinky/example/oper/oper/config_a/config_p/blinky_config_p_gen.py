...
from a.sys.blinky.example.oper.oper.config_a.config_p.config_p_data_gen import ConfigPData





class BlinkyConfigP(BlinkyContainer):
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyconfigp').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyConfigP._validationPointId, BlinkyConfigP._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagId)):
            if trxElement.getNewVal():
                ...
                self.myCandidateData.id = newValue.asInt64()
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyconfigp').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.oper.oper.config_a.config_p.blinky_config_p_gen import BlinkyConfigP', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkyconfigp-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.sys.blinky.example.oper.oper.config_a.config_p.blinky_config_p_gen import BlinkyConfigP', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = ConfigPData()
    ...
    def activateCandidate(self):
        ...
        if (self.myState.isPhaseDone(phase)):
            ...
            return
        self.myRunningData = ConfigPData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyconfigp-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyconfigp-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.oper.oper.config_a.config_p.config_p_data_gen import ConfigPData", 
        "moduleYangNamespacePrefix": "oper", 
        "validationPoint": null, 
        "yangName": "config-p", 
        "namespace": "config_p", 
        "logGroupName": "blinky-config-p", 
        "className": "BlinkyConfigP", 
        "logModuleName": "a-sys-blinky-example-oper-oper-config-a-config-p-blinky-config-p-gen", 
        "importStatement": "from a.sys.blinky.example.oper.oper.config_a.config_p.blinky_config_p_gen import BlinkyConfigP", 
        "moduleYangNamespace": "http://qwilt.com/model/oper", 
        "dataClassName": "ConfigPData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {

...
from a.sys.blinky.example.oper.oper.config_a.op_l.op_l_oper_data_gen import OpLOperData
...
class BlinkyOperOpL(BlinkyOperNode):

    _kCallpointName = "op-l-callpoint"
    ...
    def __init__ (self, logger):
        ...
        self.kCallpointName = "op-l-callpoint"
    ...
    def setOperDataRequestedFields (self, operData, keyPath):
        ...
        if requestedKeyPath.isEqual(flattenedSelfKeyPath) or (requestedKeyPath.getLen() <= flattenedSelfKeyPath.getLen()):
            ...
            if requestedKeyPath.getLen() > flattenedSelfKeyPath.getLen():
                ...
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/model/oper", "value-op-l1"):
                    for logFunc in self._log("set-oper-data-requested-fields-valueopl1-requested").debug3Func(): logFunc(
                        "value-op-l1 requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setValueOpL1Requested()
                
                if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/model/oper", "value-op-l2"):
                    for logFunc in self._log("set-oper-data-requested-fields-valueopl2-requested").debug3Func(): logFunc(
                        "value-op-l2 requested. requestedKeyPath=%s, operData=%s", requestedKeyPath, operData.debugStr(True))
                    operData.setValueOpL2Requested()
    ...
    def fillTagValues (self, keyPath, tagValueList, operData):
        ...
        if operData.isValueOpL1Requested() and operData.hasValueOpL1():
            val = Value()
            val.setString(operData.valueOpL1)
            tagValueList.push(("value-op-l1", "http://qwilt.com/model/oper"), val)
        
        if operData.isValueOpL2Requested() and operData.hasValueOpL2():
            val = Value()
            val.setString(operData.valueOpL2)
            tagValueList.push(("value-op-l2", "http://qwilt.com/model/oper"), val)
    ...
    def fillValue (self, value, keyPath, operData):
        ...
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/model/oper", "value-op-l1"):
            if operData.isValueOpL1Requested():
                 if operData.hasValueOpL1():
                     value.setString(operData.valueOpL1)
                 else:
                     value.setEmpty()
        
        if requestedKeyPath.isTagEqual(flattenedSelfKeyPath.getLen(), "http://qwilt.com/model/oper", "value-op-l2"):
            if operData.isValueOpL2Requested():
                 if operData.hasValueOpL2():
                     value.setString(operData.valueOpL2)
                 else:
                     value.setEmpty()
...
{
    "node": {
        "dataImportStatement": "from a.sys.blinky.example.oper.oper.config_a.op_l.op_l_oper_data_gen import OpLOperData", 
        ...
        "yangName": "op-l", 
        "namespace": "op_l", 
        "logGroupName": "blinky-op-l-oper", 
        "className": "BlinkyOperOpL", 
        "logModuleName": "a-sys-blinky-example-oper-oper-config-a-op-l-blinky-op-l-oper-gen", 
        "importStatement": "from a.sys.blinky.example.oper.oper.config_a.op_l.blinky_op_l_oper_gen import BlinkyOperOpL", 
        "callpointName": "op-l-callpoint", 
        "moduleYangNamespace": "http://qwilt.com/model/oper", 
        "dataClassName": "OpLOperData", 
        "getObjArgsNum": 2, 
        "actionPoint": null
    }, 
    "ancestors": [

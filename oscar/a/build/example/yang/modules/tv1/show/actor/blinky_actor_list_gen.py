...
from a.build.example.yang.modules.tv1.show.actor.blinky_actor_gen import BlinkyActor

class BlinkyActorList(BlinkyList):
    ...
    def s_create(cls, logger, 
        ...
        newNode=BlinkyActorList(logger)
    ...
    def __str__ (self):
        ret="{BlinkyActorList: self.myContainersMap=%s, BlinkyList=%s}" % \
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyactorlist').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am list of: from a.build.example.yang.modules.tv1.show.actor.blinky_actor_gen import BlinkyActor', keyPath, keyDepth)
        keyPathRegistered = False
        if keyPath.getLen() > keyDepth+1:
            keyPathRegistered = BlinkyActor.isKeyPathRegistered(logger, keyPath, keyDepth+1)
        else:
            keyPathRegistered = True
        logger('is-key-path-registered-blinkyactorlist-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am list of: from a.build.example.yang.modules.tv1.show.actor.blinky_actor_gen import BlinkyActor', keyPathRegistered, keyPath, keyDepth)
        return keyPathRegistered

    def setCreateFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-create-functor-not-active").errorFunc(): logFunc("setCreateFunctor is illegal when blinky-actor-list is active.")
        self.myCreateFunctor=functor

    def setDeleteFunctor(self, functor):
        if self.myIsActive:
            for logFunc in self._log("set-delete-functor-not-active").errorFunc(): logFunc("setDeleteFunctor is illegal when blinky-actor-list is active.")
    ...
    def findBlinkyNode (self, keyPath, keyDepth_PBR, foundAtkeyDepth_PBR):
        ...
        if keyRemainderLen==1:
            for logFunc in self._log("find-blinky-node-handle-blinky-actor").debug1Func(): logFunc("findBlinkyNode(): handling BlinkyActorList. keyPath=%s, keyDepth=%s",
                                                         keyPath, keyDepth_PBR)
        ...
        if res != ReturnCodes.kOk:
            return None
        for logFunc in self._log("find-blinky-node-getting-blinky-actor").debug1Func(): logFunc("findBlinkyNode(): getting BlinkyActor. listKey=%s, self=%s", listKey, self)
        blinkyContainer = self.getBlinkyActor(listKey)
        if blinkyContainer==None:
            for logFunc in self._log("find-blinky-node-blinky-actor-not-found").debug1Func(): logFunc("findBlinkyNode(): BlinkyActor not found. listKey=%s", listKey)
            foundAtkeyDepth_PBR.setValue(keyDepth_PBR.value())
            return self
        for logFunc in self._log("find-blinky-node-querying-blinky-actor").debug1Func(): logFunc("findBlinkyNode(): querying BlinkyActor. keyPath=%s, keyDepth=%s, blinkyContainer=%s",
    ...
    def getBlinkyActor(self, key):
        for logFunc in self._log("get-blinky-blinky-actor").debug1Func(): logFunc("getBlinkyActor(): key=%s", key)
        keyString=str(key)
    ...
    def handleTrxPrepareBlinkyCreate(self, keyPath, listKey, blinkyContainer):
        ...
        newBlinkyNode=BlinkyActor(self._log)
    ...
    def trxElementHelper(self, trxElement, keyDepth, phase):
        ...
        if (blinkyContainer == None):
            if (not ((phase.getConfdPhase() == TrxPhase.kPrepare) and (phase.getBlinkyPhase() == TrxPhase.kBlinky)) ):
                for logFunc in self._log("trx-element-helper-blinky-actor-not-exist").errorFunc(): logFunc("trxElementHelper(): BlinkyActor-not-exist: phase=%s, listKey=%s", phase, listKey)
                return ReturnCodes.kGeneralError
    ...
    def getDescendant(self, keyPath, keyDepth_PBR):
        ...
        if res != ReturnCodes.kOk:
            return None
        descendant = self.getBlinkyActor(listKey)
        return descendant
...
{
    "node": {
        "containerClassName": "BlinkyActor", 
        "containerLogGroupName": "blinky-actor", 
        "namespace": "actor", 
        "containerImportStatement": "from a.build.example.yang.modules.tv1.show.actor.blinky_actor_gen import BlinkyActor", 
        "logGroupName": "blinky-actor-list", 
        "className": "BlinkyActorList", 
        "logModuleName": "a-build-example-yang-modules-tv1-show-actor-blinky-actor-list-gen", 
        "keyLeaf": {
            "varName": "actor", 

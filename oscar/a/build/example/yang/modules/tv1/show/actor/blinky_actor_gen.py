...
from a.build.example.yang.modules.tv.show.actor.actor_data_gen import ActorData





class BlinkyActor(BlinkyContainer):
    ourNamespace="http://qwilt.com/model/tv"
    #leaves
    ...
    def s_create (cls, logger, 
        ...
        logger('s-create-blinkyactor').info('called. domain=%s, _validationPointId=%s, _actionPointId=%s', domain, BlinkyActor._validationPointId, BlinkyActor._actionPointId)
    ...
    def trxElementUpdateCandidate(self, trxElement, keyDepth):
        ...
        if (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagEyeNumber)):
        ...
        elif (keyPath.isTagEqual(keyDepth, self.ourNamespace, self.ourXmlTagTransparentField)):
            if trxElement.getNewVal():
                newValue = trxElement.getNewVal()
    ...
    def isKeyPathRegistered (cls, logger, keyPath, keyDepth):
        ...
        logger('is-key-path-registered-blinkyactor').debug3('isKeyPathRegistered() called. keyPath=%s, keyDepth=%d, I am: from a.build.example.yang.modules.tv1.show.actor.blinky_actor_gen import BlinkyActor', keyPath, keyDepth)
        ...
        logger('is-key-path-registered-blinkyactor-done').debug3('isKeyPathRegistered() done. registered=%s. keyPath=%s, keyDepth=%d, I am: from a.build.example.yang.modules.tv1.show.actor.blinky_actor_gen import BlinkyActor', keyPathRegistered, keyPath, keyDepth)
    ...
    def allocMyCandidate(self):
        for logFunc in self._log("alloc-my-candidate").debug3Func():
            logFunc("called")
        self.myCandidateData = ActorData()
        ...
        if progress.isCommitBlinkyBefore() or progress.isAbortBlinkyBefore():
            if self.isInDestroy:
                if self._validationPointId and self.validateRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-validation-point-blinkyactor-failed").errorFunc():
                ...
                if self._actionPointId and self.actionRegistrationDone:
                    ...
                    if (res != ReturnCodes.kOk):
                        for logFunc in self._log("handle-trx-progress-notification-specitic-unregister-action-point-blinkyactor-failed").errorFunc():
...
{
    "node": {
        "dataImportStatement": "from a.build.example.yang.modules.tv.show.actor.actor_data_gen import ActorData", 
        "moduleYangNamespacePrefix": "tv", 
        "validationPoint": null, 
        "yangName": "actor", 
        "namespace": "actor", 
        "logGroupName": "blinky-actor", 
        "className": "BlinkyActor", 
        "logModuleName": "a-build-example-yang-modules-tv1-show-actor-blinky-actor-gen", 
        "importStatement": "from a.build.example.yang.modules.tv1.show.actor.blinky_actor_gen import BlinkyActor", 
        "moduleYangNamespace": "http://qwilt.com/model/tv", 
        "dataClassName": "ActorData", 
        "actionPoint": null
    }, 
    "ancestors": [
        {
